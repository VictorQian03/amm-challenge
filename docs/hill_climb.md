...
1. Topology proposer: drafts topology/interface contracts, not coefficient tweaks or renamed incumbent terms. It should use the six-layer scaffold, review the suggestions, ideas, anchor compatibility notes, saturated failure modes, and productivity rules in [`docs/combination_anchor_map.md`](/Users/victorqian/Desktop/opt_arena/simple_amm/docs/combination_anchor_map.md), identify allowed and forbidden downstream consumers, state expected outcome-space movement, and include at least one outside or public-evidence-motivated idea when the local search is saturated.
2. Saturation/entropy critic: reviews the proposed batch against retained history, active run notes, and the suggestions, ideas, anchor compatibility notes, saturated failure modes, and productivity rules in [`docs/combination_anchor_map.md`](/Users/victorqian/Desktop/opt_arena/simple_amm/docs/combination_anchor_map.md). It should reject same-spine batches, renamed incumbent vocabulary, weak positive stacking, hidden floor-slice coupling, missing kill signatures, or any plan whose novelty is only code-space novelty.
