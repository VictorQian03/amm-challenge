...
contract Strategy is AMMStrategyBase {
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256 bidFee, uint256 askFee) {
        ...
        uint256 priorCommon = (prevBid + prevAsk) / 2;
        uint256 common = (priorCommon + (BASE_FEE * 3)) / 4;

        uint256 tilt = 0;
        tilt = clampFee(tilt + wmul(spotSignal, bpsToWad(5)));
        tilt = clampFee(tilt + wmul(_cap01(_absSigned(nextFlow)), bpsToWad(4)));
        tilt = clampFee(tilt + wmul(interactionSignal, bpsToWad(2)));
        tilt = clampFee(tilt + wmul(_streakSignal(nextStreak), bpsToWad(1)));
        tilt = clampFee(tilt + wmul(nextRegime, bpsToWad(2)));
        common = clampFee(common + wmul(cooldownSignal, bpsToWad(1)));

        if (trade.isBuy) {
            bidFee = clampFee(common + tilt);
            askFee = common > tilt ? common - tilt : MIN_FEE;
        } else {
            bidFee = common > tilt ? common - tilt : MIN_FEE;
            askFee = clampFee(common + tilt);
        }
