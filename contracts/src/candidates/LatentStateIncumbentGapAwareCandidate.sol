...
/// @title Latent State Incumbent Gap-Aware Candidate
/// @notice Start from the recorded incumbent and add mild gap-aware recovery back to the resting state.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        if (gap >= 3 && cooldown < 5 * BPS) {
            common = common > BPS ? common - BPS : MIN_FEE;
        }
    ...
    function getName() external pure override returns (string memory) {
        return "LatentStateIncumbentGapAware";
    }
