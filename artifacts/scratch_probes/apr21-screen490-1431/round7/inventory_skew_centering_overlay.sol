...
contract Strategy is AMMStrategyBase {
    ...
    uint256 internal constant ALPHA_INVENTORY = 16 * BPS;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // inventory skew memory
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 reserveYInX = latentSpot == 0 ? trade.reserveX : wdiv(trade.reserveY, latentSpot);
        uint256 totalInventory = trade.reserveX + reserveYInX;
        uint256 inventorySkewObservation =
            totalInventory == 0 ? 0 : wdiv(absDiff(trade.reserveX, reserveYInX), totalInventory);
        uint256 inventorySkewMemory = _blend(
            slots[11],
            inventorySkewObservation,
            ALPHA_INVENTORY
        );
        if (gap >= 4) {
            inventorySkewMemory = wmul(inventorySkewMemory, 9700 * BPS);
        } else if (gap >= 2) {
            inventorySkewMemory = wmul(inventorySkewMemory, 9300 * BPS);
        }
        ...
        uint256 inventorySkewGate = _oneMinus(
            clamp(
                wmul(hazardMemory, 1800 * BPS) +
                    wmul(flowPressure, 1600 * BPS) +
                    wmul(oneSidedFlow, 1400 * BPS),
                0,
                WAD
            )
        );
        uint256 inventorySupport = _max(
            _max(passiveRecaptureMemory, calmMemory),
            wmul(inventorySkewMemory, inventorySkewGate)
        );
        uint256 inventoryCenteringOffset = wmul(
            wmul(inventorySupport, inventoryGate),
            760 * BPS
        );
        uint256 centerCapScale = 1400 * BPS + wmul(inventorySkewMemory, 300 * BPS);
        uint256 centerCap = wmul(widthFlex, centerCapScale);
        ...
        slots[11] = inventorySkewMemory;

        return (bidFee, askFee);
    }

    function getName() external pure override returns (string memory) {
        return "InventorySkewCenteringOverlay";
    }
