...
/// @title Retail Continuity Shock Absorber
/// @notice Tests a one-way blocker for sudden width contraction after public shock.
contract Strategy is AMMStrategyBase {
    ...
    uint256 internal constant DECAY_LULL = 9300 * BPS;
    ...
    uint256 internal constant ALPHA_LULL = 14 * BPS;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[5] = 0; // lull / mean-reversion memory
        ...
        slots[10] = 0; // passive fill memory
        slots[11] = 0; // previous bid-side width
        slots[12] = 0; // previous ask-side width
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 lullMemory = wmul(
            slots[5],
            _gapAdjustedDecay(DECAY_LULL, gapLong, 700 * BPS)
        );
        ...
        lullMemory = _blend(lullMemory, lullObservation, ALPHA_LULL);
        ...
        if (gap >= 4) {
            ...
            uint256 benignGapGate = wmul(gapLong, lullMemory);
            uint256 quietDamping = wmul(wmul(quietGate, reconvergenceGate), benignGapGate);
            divergenceMemory = wmul(
                divergenceMemory,
                _oneMinus(clamp(wmul(quietDamping, 1200 * BPS), 0, WAD))
            );
        }
        ...
        uint256 passiveFillObservation = wmul(
            gapLong,
            _oneMinus(
                clamp(
        ...
        if (gap >= 4) {
            ...
            passiveFillObservation = wmul(
                passiveFillObservation,
                wmul(quietGate, passiveRecenterGate)
            );
        }
        uint256 passiveFillMemory = _blend(slots[10], passiveFillObservation, ALPHA_PASSIVE);
        if (gap >= 4) {
            passiveFillMemory = wmul(passiveFillMemory, 9600 * BPS);
        } else if (gap >= 2) {
            passiveFillMemory = wmul(passiveFillMemory, 9000 * BPS);
        }
        ...
        uint256 tailProtectionLimit = tailBucket == 0
            ? 11 * BPS
            : tailBucket == 1
                ? 17 * BPS
                : tailBucket == 2
                    ? 26 * BPS
                    : 38 * BPS;
        if (bidProtection > tailProtectionLimit) {
            bidProtection = tailProtectionLimit;
        }
        if (askProtection > tailProtectionLimit) {
            askProtection = tailProtectionLimit;
        }
        ...
        uint256 inventoryCenteringOffset = wmul(
            wmul(_max(passiveFillMemory, lullMemory), inventoryGate),
            700 * BPS
        );
        uint256 centerLimit = wmul(
            sharedSpread > BASE_FEE ? sharedSpread - BASE_FEE : 0,
            1400 * BPS
        );
        if (inventoryCenteringOffset > centerLimit) {
            inventoryCenteringOffset = centerLimit;
        }
        ...
        uint256 bidPrevWidth = slots[11];
        uint256 askPrevWidth = slots[12];
        uint256 shockEvidence = _max(
            hazardObservation,
            _max(monotoneEvidenceFloor, flowDirectionalRisk)
        );
        bool doNotInvertWidth = gap <= 2 && shockEvidence > 7 * BPS;
        if (doNotInvertWidth && bidProtection > 0 && bidPrevWidth > bidProtection) {
            uint256 bidAllowedDrop = wmul(
                bidPrevWidth,
                shockEvidence > 13 * BPS ? 1800 * BPS : 3000 * BPS
            );
            uint256 bidFloor = bidPrevWidth > bidAllowedDrop ? bidPrevWidth - bidAllowedDrop : 0;
            if (bidProtection < bidFloor) {
                bidProtection = bidFloor;
            }
        }
        if (doNotInvertWidth && askProtection > 0 && askPrevWidth > askProtection) {
            uint256 askAllowedDrop = wmul(
                askPrevWidth,
                shockEvidence > 13 * BPS ? 1800 * BPS : 3000 * BPS
            );
            uint256 askFloor = askPrevWidth > askAllowedDrop ? askPrevWidth - askAllowedDrop : 0;
            if (askProtection < askFloor) {
                askProtection = askFloor;
            }
        }
        ...
        slots[10] = passiveFillMemory;
        slots[11] = bidProtection;
        slots[12] = askProtection;

        return (bidFee, askFee);
    }

    function getName() external pure override returns (string memory) {
        return "RetailContinuityShockAbsorber";
    }
