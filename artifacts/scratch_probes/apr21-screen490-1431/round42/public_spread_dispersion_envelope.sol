...
/// @title Public Spread Dispersion Envelope
/// @notice Tests a public dispersion/no-arbitrage label that only reinforces weak event feasibility.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 dispersionLabel = 1; // 0 dispersion_compressed, 1 dispersion_normal, 2 dispersion_wide
        uint256 spreadCore = _max(tradeSize, spotJumpVol);
        uint256 spreadWing = divergenceVol;
        uint256 spreadDispersion = spreadCore > spreadWing
            ? spreadCore - spreadWing
            : spreadWing - spreadCore;
        uint256 publicNoArbPressure = _max(
            spreadDispersion,
            _max(spotJump, divergence)
        );
        if (
            publicNoArbPressure > 9 * BPS &&
            spreadDispersion > wmul(_max(spreadCore, spreadWing), 3800 * BPS)
        ) {
            dispersionLabel = 2;
        } else if (
            publicNoArbPressure < 3 * BPS &&
            spreadDispersion < wmul(_max(spreadCore, spreadWing), 1400 * BPS)
        ) {
            dispersionLabel = 0;
        }
        ...
        if (dispersionLabel == 2 && feasibilityResidual > 0) {
            uint256 dispersionUncertainty = clamp(
                wmul(publicNoArbPressure, 1700 * BPS) +
                    wmul(spreadDispersion, 900 * BPS),
                0,
                6 * BPS
            );
            hazardObservation = clamp(hazardObservation + dispersionUncertainty, 0, WAD);
        }
    ...
    function getName() external pure override returns (string memory) {
        return "PublicSpreadDispersionEnvelope";
    }
