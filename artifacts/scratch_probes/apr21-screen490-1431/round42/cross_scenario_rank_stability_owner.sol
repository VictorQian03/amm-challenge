...
/// @title Cross Scenario Rank Stability Owner
/// @notice Uses unstable relative state ranks as uncertainty that can only preserve incumbent width.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // previous cross-scenario top-rank label
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 eventPathRank = _max(spotJumpVol, divergenceVol);
        uint256 flowPathRank = wmul(flowImbalance, _max(volObservation, divergenceVol));
        uint256 sizePathRank = tradeSize + wmul(clusterObservation, 1800 * BPS);
        uint256 topRank = _topRank(eventPathRank, flowPathRank, sizePathRank);
        uint256 priorTopRank = slots[11];
        uint256 topRankGap = _rankGap(eventPathRank, flowPathRank, sizePathRank, topRank);
        bool rankUnstable =
            priorTopRank != 0 &&
            priorTopRank != topRank &&
            topRankGap < 5 * BPS &&
            _max(eventPathRank, _max(flowPathRank, sizePathRank)) > 4 * BPS;
        ...
        if (rankUnstable && weaklyInconsistentEvent) {
            bidOpportunityCut = 0;
            askOpportunityCut = 0;
        }
    ...
    function getName() external pure override returns (string memory) {
        return "CrossScenarioRankStabilityOwner";
    }
    ...
    function _topRank(uint256 eventPath, uint256 flowPath, uint256 sizePath)
        internal
        pure
        returns (uint256)
    {
        if (eventPath >= flowPath && eventPath >= sizePath) {
            return 1;
        }
        if (flowPath >= sizePath) {
            return 2;
        }
        return 3;
    }
    ...
    function _rankGap(
        ...
        uint256 top = topRank == 1 ? eventPath : topRank == 2 ? flowPath : sizePath;
        uint256 next = 0;
        if (topRank != 1 && eventPath > next) {
            next = eventPath;
        }
        if (topRank != 2 && flowPath > next) {
            next = flowPath;
        }
        if (topRank != 3 && sizePath > next) {
            next = sizePath;
        }
        return top > next ? top - next : 0;
    }
}
