...
/// @title Execution Cost Convexity Surface
/// @notice Tests bounded convex execution-cost pressure as one-way protection evidence.
contract Strategy is AMMStrategyBase {
    ...
    uint256 internal constant ALPHA_EXEC_COST = 20 * BPS;
    uint256 internal constant ALPHA_EXEC_RISING = 24 * BPS;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // bounded convex execution-cost memory
        slots[12] = 0; // one-way rising execution-cost evidence
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 executionCostMemory = wmul(
            slots[11],
            _gapAdjustedDecay(9000 * BPS, gapShort, 1400 * BPS)
        );
        uint256 convexCostRisingMemory = wmul(
            slots[12],
            _gapAdjustedDecay(8600 * BPS, gapShort, 1800 * BPS)
        );
        ...
        uint256 convexExecutionCost = tradeSize;
        if (tradeSize > 4 * BPS) {
            convexExecutionCost += wmul(tradeSize - 4 * BPS, tradeSize * 5);
        }
        if (tradeSize > 12 * BPS) {
            convexExecutionCost += wmul(tradeSize - 12 * BPS, tradeSize * 9);
        }
        convexExecutionCost = clamp(convexExecutionCost, 0, 42 * BPS);
        uint256 convexCostRising = 0;
        if (convexExecutionCost > executionCostMemory) {
            convexCostRising = convexExecutionCost - executionCostMemory;
            if (convexCostRising > 18 * BPS) {
                convexCostRising = 18 * BPS;
            }
        }
        ...
        executionCostMemory = _blend(executionCostMemory, convexExecutionCost, ALPHA_EXEC_COST);
        convexCostRisingMemory = _blend(convexCostRisingMemory, convexCostRising, ALPHA_EXEC_RISING);
        ...
        uint256 executionCostProtection = wmul(
            wmul(convexCostRisingMemory, _oneMinus(gapShort)),
            6200 * BPS
        );
        if (executionCostProtection > 12 * BPS) {
            executionCostProtection = 12 * BPS;
        }
        if (trade.isBuy) {
            askProtection += executionCostProtection;
        } else {
            bidProtection += executionCostProtection;
        }
        ...
        slots[11] = executionCostMemory;
        slots[12] = convexCostRisingMemory;

        return (bidFee, askFee);
    }

    function getName() external pure override returns (string memory) {
        return "ExecutionCostConvexitySurface";
    }
