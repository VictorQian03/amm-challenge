...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 protectedServiceElasticity = 0;
        if (continuationAligned) {
            protectedServiceElasticity = wmul(flowPressure, _max(volMemory, divergenceMemory)) +
                wmul(oneSidedFlow, 1700 * BPS);
            if (gap <= 2) {
                protectedServiceElasticity += wmul(_oneMinus(gapShort), hazardMemory);
            }
            protectedServiceElasticity = clamp(protectedServiceElasticity, 0, WAD);
        }
        uint256 protectedElasticityAdd = wmul(protectedServiceElasticity, 950 * BPS);
        if (protectedElasticityAdd > 0) {
            if (currentSpot >= latentSpot) {
                bidProtection += protectedElasticityAdd;
            } else {
                askProtection += protectedElasticityAdd;
            }
        }
    ...
    function getName() external pure override returns (string memory) {
        return "ProtectedSideServiceElasticity";
    }
