...
/// @title Volatility Isoquant Service Map
/// @notice Adds bounded volatility-shape evidence to protected-side risk only.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 askRiskSignal =
            ...
            askFlowRisk;
        uint256 isoquantServiceEvidence = _volatilityIsoquantServiceEvidence(
            volMemory,
            hazardMemory,
            divergenceMemory,
            flowPressure,
        ...
        uint256 protectedSideEvidence = wmul(isoquantServiceEvidence, 3800 * BPS);
        if (toxicBidSide) {
            bidRiskSignal = clamp(bidRiskSignal + protectedSideEvidence, 0, WAD);
        } else {
            askRiskSignal = clamp(askRiskSignal + protectedSideEvidence, 0, WAD);
        }
    ...
    function getName() external pure override returns (string memory) {
        return "VolatilityIsoquantServiceMap";
    }
    ...
    function _volatilityIsoquantServiceEvidence(
        uint256 volMemory,
        uint256 hazardMemory,
        uint256 divergenceMemory,
        uint256 flowPressure,
        uint256 calmMemory,
        uint256 passiveRecaptureMemory
    ) internal pure returns (uint256) {
        uint256 isoquantMass = _max(
            wmul(volMemory, hazardMemory),
            wmul(_max(volMemory, divergenceMemory), flowPressure)
        );
        if (isoquantMass <= 250 * BPS) {
            return 0;
        }
        uint256 serviceRelief = clamp(
        ...
        return wmul(
            clamp(isoquantMass - 250 * BPS, 0, 5200 * BPS),
            _oneMinus(serviceRelief)
        );
    }
}
