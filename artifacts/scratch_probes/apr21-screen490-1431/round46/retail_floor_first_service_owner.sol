...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 retailFloorPreservation = _oneMinus(
            clamp(
                wmul(hazardMemory, 7600 * BPS) +
                    wmul(flowPressure, 3000 * BPS) +
                    wmul(oneSidedFlow, 3600 * BPS) +
        ...
        if (gap < 2) {
            retailFloorPreservation = 0;
        } else if (gap < 4) {
            retailFloorPreservation = wmul(retailFloorPreservation, 6200 * BPS);
        }
        uint256 serviceCapacityEvidence = wmul(
            wmul(_max(passiveRecaptureMemory, calmMemory), quietGate),
            retailFloorPreservation
        );
        serviceCapacityEvidence = wmul(
            serviceCapacityEvidence,
            _oneMinus(clamp(wmul(flowImbalance, 4200 * BPS), 0, WAD))
        );
        uint256 serviceRiskDebit = wmul(serviceCapacityEvidence, 900 * BPS);
        if (toxicBidSide) {
            bidRiskSignal = bidRiskSignal > serviceRiskDebit
                ? bidRiskSignal - serviceRiskDebit
                : 0;
        } else {
            askRiskSignal = askRiskSignal > serviceRiskDebit
                ? askRiskSignal - serviceRiskDebit
                : 0;
        }
    ...
    function getName() external pure override returns (string memory) {
        return "RetailFloorFirstServiceOwner";
    }
