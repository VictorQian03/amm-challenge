...
/// @title Isofloor Service Slope Splitter
/// @notice Splits benign service slope from floor-risking service slope before side-risk evidence assembly.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 floorProxy = wmul(
            quietGate,
            _oneMinus(
                clamp(
                    wmul(hazardMemory, 2800 * BPS) +
        ...
        uint256 isofloorServiceEvidence = 0;
        if (floorProxy >= 5600 * BPS && passiveRecaptureMemory > calmMemory) {
            uint256 serviceSlope = passiveRecaptureMemory - calmMemory;
            uint256 floorRiskingService = wmul(
                serviceSlope,
                clamp(
                    wmul(_max(flowPressure, divergenceMemory), 3600 * BPS) +
                        wmul(oneSidedFlow, 2600 * BPS),
                    0,
                    WAD
                )
            );
            isofloorServiceEvidence = wmul(
                floorRiskingService,
                wmul(floorProxy - 5600 * BPS, 2300 * BPS)
            );
        }
        ...
        uint256 askRiskSignal =
            ...
            askFlowRisk;
        if (isofloorServiceEvidence > 0) {
            if (toxicBidSide) {
                askRiskSignal += isofloorServiceEvidence;
            } else {
                bidRiskSignal += isofloorServiceEvidence;
            }
        }
    ...
    function getName() external pure override returns (string memory) {
        return "IsofloorServiceSlopeSplitter";
    }
