...
/// @title Supply Function Service Curve
/// @notice Adds a bounded service-capacity evidence curve before protection sizing.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 serviceLoad = clamp(
            wmul(volMemory, 3200 * BPS) +
                wmul(hazardMemory, 3800 * BPS) +
                wmul(flowPressure, 2600 * BPS) +
                wmul(divergenceMemory, 2200 * BPS),
            0,
            WAD
        );
        uint256 spareService = _oneMinus(serviceLoad);
        uint256 serviceCurvePressure = wmul(serviceLoad, serviceLoad);
        uint256 serviceFloorGate = _oneMinus(
            clamp(
                wmul(spareService, 6000 * BPS) +
                    wmul(calmMemory, 2200 * BPS),
                0,
                WAD
            )
        );
        uint256 serviceCapacityEvidence = clamp(
            wmul(serviceCurvePressure, serviceFloorGate),
            0,
            280 * BPS
        );
        ...
        uint256 askRiskSignal =
            ...
            askFlowRisk;
        if (toxicBidSide) {
            bidRiskSignal = clamp(bidRiskSignal + serviceCapacityEvidence, 0, WAD);
        } else {
            askRiskSignal = clamp(askRiskSignal + serviceCapacityEvidence, 0, WAD);
        }
    ...
    function getName() external pure override returns (string memory) {
        return "SupplyFunctionServiceCurve";
    }
