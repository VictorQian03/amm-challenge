...
/// @title Proper Score Reserve Ledger
/// @notice Adds a bounded reserve-adequacy evidence score before spread consumers see state.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // reserve-adequacy deficiency memory
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 expectedYForX = wmul(trade.reserveX, latentSpot);
        uint256 reserveDenom = _max(trade.reserveY, expectedYForX);
        uint256 reserveSkew = reserveDenom == 0
            ? 0
            : wdiv(absDiff(trade.reserveY, expectedYForX), reserveDenom);
        bool xReserveDeficient = trade.reserveY > expectedYForX;
        bool yReserveDeficient = expectedYForX > trade.reserveY;
        bool deficientReserveConsumed =
            (xReserveDeficient && !trade.isBuy) || (yReserveDeficient && trade.isBuy);
        bool sufficientReserveConsumed =
            (xReserveDeficient && trade.isBuy) || (yReserveDeficient && !trade.isBuy);
        uint256 reserveNeutralEvidence = reserveSkew <= 4 * BPS ? 4 * BPS - reserveSkew : 0;
        uint256 reserveSufficientEvidence = sufficientReserveConsumed
            ? clamp(wmul(reserveSkew, 2200 * BPS), 0, 5 * BPS)
            : 0;
        uint256 reserveDeficientEvidence = deficientReserveConsumed
            ? clamp(
                wmul(reserveSkew, 6400 * BPS) + wmul(tradeSize, 1100 * BPS),
                0,
                12 * BPS
            )
            : 0;
        if (reserveNeutralEvidence > 0 || reserveSufficientEvidence > 0) {
            reserveDeficientEvidence = 0;
        }
        ...
        uint256 reserveDeficientMemory = wmul(
            slots[11],
            _gapAdjustedDecay(DECAY_HAZARD, gapShort, 1500 * BPS)
        );
        ...
        if (hazardObservation < reserveDeficientEvidence) {
            hazardObservation = reserveDeficientEvidence;
        }
        ...
        reserveDeficientMemory = _blend(
            reserveDeficientMemory,
            reserveDeficientEvidence,
            ALPHA_HAZARD
        );
        ...
        if (continuationAligned) {
            continuationVeto = clamp(
                wmul(flowPressure, 2800 * BPS) +
                    ...
                    wmul(reserveDeficientMemory, 1800 * BPS),
                0,
                4500 * BPS
            );
        }
        ...
        uint256 reserveProtectionEvidence = clamp(
            reserveDeficientMemory + reserveDeficientEvidence,
            0,
            16 * BPS
        );
        uint256 bidReserveRisk = yReserveDeficient ? reserveProtectionEvidence : 0;
        uint256 askReserveRisk = xReserveDeficient ? reserveProtectionEvidence : 0;
        ...
        slots[11] = reserveDeficientMemory;

        return (bidFee, askFee);
    }

    function getName() external pure override returns (string memory) {
        return "ProperScoreReserveLedger";
    }
