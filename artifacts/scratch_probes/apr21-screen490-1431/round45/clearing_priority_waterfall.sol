...
/// @title Clearing Priority Waterfall
/// @notice Adds a small static obligation-priority layer before protection/opportunity fee assembly.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 askRiskSignal =
            ...
            askFlowRisk;
        uint256 clearingObligation =
            wmul(oneSidedFlow, 900 * BPS) +
            wmul(wmul(flowPressure, divergenceMemory), 650 * BPS);
        uint256 secondaryObligation = wmul(wmul(tradeSize, flowImbalance), 450 * BPS);
        if (toxicBidSide) {
            bidRiskSignal += clearingObligation;
            if (buyFlow >= sellFlow) {
                bidRiskSignal += secondaryObligation;
            } else {
                askRiskSignal += secondaryObligation;
            }
        } else {
            askRiskSignal += clearingObligation;
            if (sellFlow > buyFlow) {
                askRiskSignal += secondaryObligation;
            } else {
                bidRiskSignal += secondaryObligation;
    ...
    function getName() external pure override returns (string memory) {
        return "ClearingPriorityWaterfall";
    }
