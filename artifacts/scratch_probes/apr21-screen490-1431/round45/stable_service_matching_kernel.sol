...
/// @title Stable Service Matching Kernel
/// @notice Adds a bounded layer-3 match between existing side-risk demand and safe-side service.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        {
            uint256 bidStableService = _stableServiceMatch(
                bidRiskSignal,
                askOpportunitySignal,
                richSignal,
                cheapSignal
            );
            uint256 askStableService = _stableServiceMatch(
                askRiskSignal,
                bidOpportunitySignal,
                cheapSignal,
                richSignal
            );
            bidRiskSignal = clamp(bidRiskSignal + bidStableService, 0, WAD);
            askRiskSignal = clamp(askRiskSignal + askStableService, 0, WAD);
            askOpportunitySignal = clamp(
                askOpportunitySignal + wmul(bidStableService, 1800 * BPS),
                0,
                WAD
            );
            bidOpportunitySignal = clamp(
                bidOpportunitySignal + wmul(askStableService, 1800 * BPS),
                0,
                WAD
            );
        }
    ...
    function getName() external pure override returns (string memory) {
        return "StableServiceMatchingKernel";
    }
    ...
    function _stableServiceMatch(
        uint256 demand,
        uint256 service,
        uint256 preferredClass,
        uint256 rejectedClass
    ) internal pure returns (uint256) {
        if (demand <= 4 * BPS || service <= 2 * BPS || preferredClass <= rejectedClass) {
            return 0;
        }
        uint256 matchedService = demand < service ? demand : service;
        uint256 preferenceMargin = preferredClass - rejectedClass;
        uint256 preferenceGate = clamp(
            wmul(preferenceMargin, 6500 * BPS) + wmul(matchedService, 900 * BPS),
            0,
            WAD
        );
        uint256 serviceKernel = wmul(matchedService, preferenceGate);
        uint256 kernelCap = 7 * BPS + wmul(preferenceMargin, 300 * BPS);
        if (kernelCap > 12 * BPS) {
            kernelCap = 12 * BPS;
        }
        return serviceKernel < kernelCap ? serviceKernel : kernelCap;
    }
}
