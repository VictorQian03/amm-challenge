...
/// @title Path Reversal Residue Transform
...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 pathReversalResidue = _pathReversalResidue(
            lastSpot,
            currentSpot,
            latentSpot,
            spotJump,
        ...
        if (pathReversalResidue > 0) {
            uint256 memoryInputScale = _oneMinus(pathReversalResidue);
            volMemoryObservation = wmul(volMemoryObservation, memoryInputScale);
            hazardMemoryObservation = wmul(hazardMemoryObservation, memoryInputScale);
            divergenceMemoryObservation = wmul(divergenceMemoryObservation, memoryInputScale);
        }
    ...
    function getName() external pure override returns (string memory) {
        return "PathReversalResidueTransform";
    }
    ...
    function _pathReversalResidue(
        ...
        if (lastSpot == 0 || latentSpot == 0 || gap > 2 || spotJump <= 3 * BPS || divergence <= 2 * BPS) {
            return 0;
        }

        bool lastAboveLatent = lastSpot >= latentSpot;
        if (lastAboveLatent != (currentSpot >= latentSpot)) {
            return 0;
        }

        uint256 priorDivergence = wdiv(absDiff(lastSpot, latentSpot), latentSpot);
        if (priorDivergence <= divergence + 2 * BPS) {
            return 0;
        }

        uint256 closingDistance = priorDivergence - divergence;
        uint256 residueShape = _min(spotJump, _min(closingDistance, divergence));
        if (residueShape <= 2 * BPS) {
            return 0;
        }

        uint256 gapGate = gap == 0 ? WAD : gap == 1 ? 8500 * BPS : 6000 * BPS;
        return clamp(wmul(_min(residueShape, 24 * BPS) * 7, gapGate), 0, 1400 * BPS);
    }
}
