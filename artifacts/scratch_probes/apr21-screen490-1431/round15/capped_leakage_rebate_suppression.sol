...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 rebateSuppression = clamp(
            wmul(flowPressure, 1800 * BPS) +
                wmul(hazardMemory, 1500 * BPS) +
                wmul(oneSidedFlow, 1200 * BPS),
            0,
            3500 * BPS
        );
        sharedRebate = wmul(sharedRebate, _oneMinus(rebateSuppression));
    ...
    function getName() external pure override returns (string memory) {
        return "CappedLeakageRebateSuppression";
    }
