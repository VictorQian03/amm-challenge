...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // discrete clearing-clock state
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 clearingClockState = 0;
        if (gap <= 1 && tradeSize > 14 * BPS && spotJump > 4 * BPS) {
            clearingClockState = 2;
        } else if (gap <= 2 && tradeSize > 9 * BPS && spotJump > 2 * BPS) {
            clearingClockState = 1;
        } else if (slots[11] == 2 && gap <= 2 && tradeSize > 7 * BPS) {
            clearingClockState = 1;
        }
        uint256 clearingClockHazard = clearingClockState * 220 * BPS;
        uint256 hazardObservation = _max(
            divergenceVol,
            volObservation + wmul(clusterObservation, 7000 * BPS) + clearingClockHazard
        );
        ...
        slots[11] = clearingClockState;

        return (bidFee, askFee);
    }

    function getName() external pure override returns (string memory) {
        return "DiscreteClearingClockState";
    }
