...
/// @title Quantile Firewall Ingress
/// @notice Tests tail buckets as monotone firewall evidence that can only lift protected-side risk.
contract Strategy is AMMStrategyBase {
    ...
    uint256 internal constant ALPHA_FIREWALL = 24 * BPS;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // one-way tail firewall evidence
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 firewallObservation = monotoneEvidenceFloor;
        if (tailBucket > 0) {
            uint256 bucketIngress = tailBucket == 1
                ? 6 * BPS
                : tailBucket == 2
                    ? 11 * BPS
                    : 18 * BPS;
            firewallObservation = clamp(
                firewallObservation +
                    bucketIngress +
                    wmul(clusterObservation, 4200 * BPS) +
                    wmul(divergenceVol, 2600 * BPS),
                0,
                30 * BPS
            );
        }
        ...
        firewallMemory = _blend(firewallMemory, firewallObservation, ALPHA_FIREWALL);
        ...
        slots[11] = firewallMemory;

        return (bidFee, askFee);
    }

    function getName() external pure override returns (string memory) {
        return "QuantileFirewallIngress";
    }
