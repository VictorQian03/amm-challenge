...
/// @title Firewall Event Path Conjunction
/// @notice Adds protection only when independent event-path signals agree.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        bool eventPathValid =
            volObservation > 5 * BPS &&
            hazardObservation > 7 * BPS &&
            divergenceMemory > 2 * BPS &&
            (spotJump > 2 * BPS || tradeSize > 4 * BPS);
        uint256 eventPathStress = 0;
        if (eventPathValid) {
            eventPathStress = _max(
                monotoneEvidenceFloor,
                _max(
                    wmul(hazardObservation, 7200 * BPS),
                    wmul(_max(spotJump, divergenceMemory), 8800 * BPS)
                )
            );
        }
        ...
        sideHazard += wmul(eventPathStress, 850 * BPS);
        if (sideHazard > WAD) {
            sideHazard = WAD;
        }
        ...
        eventCarry += wmul(eventPathStress, 360 * BPS);
        ...
        directionalBurstFee += wmul(eventPathStress, 1200 * BPS);
    ...
    function getName() external pure override returns (string memory) {
        return "FirewallEventPathConjunction";
    }
