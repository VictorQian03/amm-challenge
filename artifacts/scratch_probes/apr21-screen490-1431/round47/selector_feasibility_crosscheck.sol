...
/// @title Selector Feasibility Crosscheck
...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 selectorProof = wmul(
            _oneMinus(selectorDoubt),
            clamp(
                wmul(_max(divergenceMemory, spotJump), 6000 * BPS) +
                    wmul(calmMemory, 700 * BPS),
                0,
                WAD
            )
        );
        if (toxicBidSide) {
            uint256 bidFloorSlack = bidFee > BASE_FEE ? bidFee - BASE_FEE : 0;
            if (bidFloorSlack > 0) {
                uint256 bidFloorWeight = clamp(wdiv(bidFloorSlack, BASE_FEE), 0, WAD);
                bidFee += wmul(wmul(selectorProof, bidFloorWeight), 280 * BPS);
            }
        } else {
            uint256 askFloorSlack = askFee > BASE_FEE ? askFee - BASE_FEE : 0;
            if (askFloorSlack > 0) {
                uint256 askFloorWeight = clamp(wdiv(askFloorSlack, BASE_FEE), 0, WAD);
                askFee += wmul(wmul(selectorProof, askFloorWeight), 280 * BPS);
            }
        }
    ...
    function getName() external pure override returns (string memory) {
        return "SelectorFeasibilityCrosscheck";
    }
