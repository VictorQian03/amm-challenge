...
/// @title Split Bus Feasibility Union
/// @notice Keep information, liquidity, and event-path evidence separate until an upstream union raises risk.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 liquidityStress = liquidityDemand;
        uint256 pairedInformationLiquidity = informationStress;
        if (informationStress > 2 * BPS && liquidityStress > 2 * BPS) {
            pairedInformationLiquidity = clamp(
                pairedInformationLiquidity + wmul(_min(informationStress, liquidityStress), 2600 * BPS),
                0,
                WAD
            );
        }
        uint256 eventPathFeasibility = clamp(
            wmul(spotJumpVol, _oneMinus(gapShort)) +
                wmul(divergenceVol, 1800 * BPS) +
                wmul(_max(spotJumpVol, divergenceVol), gapShort),
            0,
            WAD
        );
        uint256 upstreamEvidenceUnion = _max(pairedInformationLiquidity, eventPathFeasibility);
        uint256 volObservation = _max(liquidityStress, upstreamEvidenceUnion);
        uint256 clusterObservation = wmul(eventPathFeasibility, _oneMinus(gapShort));
        uint256 hazardObservation = _max(
            divergenceVol,
            _max(informationStress, eventPathFeasibility) + wmul(clusterObservation, 8200 * BPS)
        );
        ...
        uint256 unionProtection = wmul(upstreamEvidenceUnion, 2600 * BPS);
        uint256 bidProtection = wmul(bidRiskSignal, 5400 * BPS) + unionProtection;
        uint256 askProtection = wmul(askRiskSignal, 5400 * BPS) + unionProtection;
    ...
    function getName() external pure override returns (string memory) {
        return "SplitBusFeasibilityUnion";
    }
