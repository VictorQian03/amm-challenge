...
contract Strategy is AMMStrategyBase {
    ...
    uint256 internal constant ALPHA_BOUNCE = 20 * BPS;
    uint256 internal constant ALPHA_DIRECTIONAL = 20 * BPS;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // bid/ask bounce observation memory
        slots[12] = 0; // directional information observation memory
        slots[13] = 0; // last observed trade side
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        bool hasPriorSide = slots[7] != 0;
        bool sideFlipped = hasPriorSide && slots[13] != (trade.isBuy ? WAD : 0);
        uint256 bounceGate = _oneMinus(
            clamp(
                wmul(divergenceVol, 9000 * BPS) +
                    wmul(gapShort, 2500 * BPS),
                0,
                WAD
            )
        );
        uint256 bounceObservation = sideFlipped
            ? wmul(_max(spotJumpVol, tradeSize), wmul(_oneMinus(gapShort), bounceGate))
            : 0;
        uint256 directionalObservation = _max(divergenceVol, spotJumpVol);
        if (sideFlipped) {
            directionalObservation = wmul(
                directionalObservation,
                _oneMinus(clamp(wmul(bounceObservation, 5000 * BPS), 0, WAD))
            );
        } else if (hasPriorSide) {
            directionalObservation = clamp(
                directionalObservation + wmul(tradeSize, 1200 * BPS),
                0,
                WAD
            );
        }
        uint256 bounceMemory = _blend(slots[11], bounceObservation, ALPHA_BOUNCE);
        uint256 directionalMemory = _blend(slots[12], directionalObservation, ALPHA_DIRECTIONAL);
        uint256 deconvolvedSize = tradeSize;
        uint256 deconvolvedJump = spotJumpVol;
        if (bounceMemory > directionalMemory) {
            uint256 bounceExcess = bounceMemory - directionalMemory;
            uint256 bounceAttenuation = clamp(wmul(bounceExcess, 3000 * BPS), 0, 4200 * BPS);
            deconvolvedSize = wmul(deconvolvedSize, _oneMinus(bounceAttenuation));
            deconvolvedJump = wmul(deconvolvedJump, _oneMinus(bounceAttenuation));
        } else if (directionalMemory > bounceMemory) {
            deconvolvedJump = clamp(
                deconvolvedJump + wmul(directionalMemory - bounceMemory, 1600 * BPS),
                0,
                WAD
            );
        }
        uint256 volObservation = _max(deconvolvedSize, _max(deconvolvedJump, divergenceVol));
        uint256 clusterObservation = wmul(
            _max(deconvolvedJump, divergenceVol),
            _oneMinus(gapShort)
        );
        uint256 hazardObservation = _max(
            divergenceVol,
            directionalMemory + wmul(clusterObservation, 7000 * BPS)
        );
        ...
        uint256 flowPulse = deconvolvedSize + wmul(volObservation, 4500 * BPS);
        ...
        uint256 sourceDirectionalSignal = _max(divergenceMemory, directionalMemory);
        uint256 sideHazard = hazardMemory + wmul(flowImbalance, _max(volMemory, sourceDirectionalSignal));
        if (bounceMemory > directionalMemory) {
            sideHazard = wmul(
                sideHazard,
                _oneMinus(clamp(wmul(bounceMemory - directionalMemory, 1600 * BPS), 0, 2500 * BPS))
            );
        }
        if (sideHazard > WAD) {
            sideHazard = WAD;
        ...
        if (flowPressure > 500 * BPS) {
            uint256 extensionSignal = _max(sourceDirectionalSignal, spotJump);
        ...
        slots[11] = bounceMemory;
        slots[12] = directionalMemory;
        slots[13] = trade.isBuy ? WAD : 0;

        return (bidFee, askFee);
    }

    function getName() external pure override returns (string memory) {
        return "BidAskBounceDeconvolutionBus";
    }
