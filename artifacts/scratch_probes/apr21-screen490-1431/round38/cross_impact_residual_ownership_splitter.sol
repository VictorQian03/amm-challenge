...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        bool residualBidOwner = sellFlow >= buyFlow;
        uint256 crossImpactResidual = 0;
        if (flowImbalance > 450 * BPS) {
            crossImpactResidual = wmul(
                flowImbalance - 450 * BPS,
                _max(divergenceMemory, wmul(spotJump, 7200 * BPS))
            );
            crossImpactResidual = clamp(
                crossImpactResidual + wmul(crossImpactResidual, _max(volMemory, hazardMemory)),
                0,
                1200 * BPS
            );
        }
        ...
        bool residualConflict = crossImpactResidual > 90 * BPS && residualBidOwner != toxicBidSide;
        uint256 residualConfirm = crossImpactResidual > 0 && !residualConflict
            ? wmul(crossImpactResidual, 6200 * BPS)
            : 0;
        ...
        if (flowPressure > 500 * BPS) {
            ...
            if (extensionSignal > 3 * BPS) {
                uint256 toxicFlowSignal = flowPressure +
                    wmul(extensionSignal, 2200 * BPS) +
                    residualConfirm;
        ...
        if (residualConflict) {
            uint256 residualSpreadHold = wmul(crossImpactResidual, 280 * BPS);
            sharedSpread += residualSpreadHold;
        }
    ...
    function getName() external pure override returns (string memory) {
        return "CrossImpactResidualOwnershipSplitter";
    }
