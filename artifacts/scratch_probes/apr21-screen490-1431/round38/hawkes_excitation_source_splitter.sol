...
contract Strategy is AMMStrategyBase {
    ...
    uint256 internal constant DECAY_SOURCE = 9050 * BPS;
    uint256 internal constant DECAY_RETAIL_SOURCE = 9400 * BPS;
    ...
    uint256 internal constant ALPHA_SOURCE = 24 * BPS;
    uint256 internal constant ALPHA_RETAIL_SOURCE = 16 * BPS;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // self-excited toxic burst source memory
        slots[12] = 0; // exogenous retail source memory
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 selfExcitationMemory = wmul(
            slots[11],
            _gapAdjustedDecay(DECAY_SOURCE, gapShort, 1500 * BPS)
        );
        uint256 exogenousRetailMemory = wmul(
            slots[12],
            _gapAdjustedDecay(DECAY_RETAIL_SOURCE, gapLong, 500 * BPS)
        );
        ...
        uint256 priorFlow = buyFlow + sellFlow;
        uint256 sameSidePriorFlow = trade.isBuy ? buyFlow : sellFlow;
        uint256 oppositeSidePriorFlow = trade.isBuy ? sellFlow : buyFlow;
        uint256 sameSidePriorShare = _share(sameSidePriorFlow, priorFlow);
        bool impactAlignedWithTrade = trade.isBuy ? currentSpot >= latentSpot : currentSpot < latentSpot;
        uint256 alignedShock = impactAlignedWithTrade ? _max(spotJumpVol, divergenceVol) : 0;
        uint256 selfExcitationObservation = 0;
        if (alignedShock > 3 * BPS) {
            selfExcitationObservation = clamp(
                wmul(alignedShock + wmul(volObservation, sameSidePriorShare), _oneMinus(gapShort)) +
                    wmul(clusterObservation, 4200 * BPS),
                0,
                WAD
            );
        }
        uint256 priorFlowBalance = _oneMinus(_share(absDiff(sameSidePriorFlow, oppositeSidePriorFlow), priorFlow));
        uint256 lowImpactGate = _oneMinus(
            clamp(
                wmul(spotJumpVol, 7800 * BPS) +
                    wmul(divergenceVol, 7200 * BPS) +
                    wmul(clusterObservation, 5000 * BPS),
                0,
                WAD
            )
        );
        uint256 exogenousRetailObservation =
            wmul(tradeSize, wmul(lowImpactGate, priorFlow == 0 ? 6500 * BPS : priorFlowBalance));
        ...
        selfExcitationMemory = _blend(selfExcitationMemory, selfExcitationObservation, ALPHA_SOURCE);
        exogenousRetailMemory = _blend(
            exogenousRetailMemory,
            exogenousRetailObservation,
            ALPHA_RETAIL_SOURCE
        );
        ...
        uint256 sourceEvidence = selfExcitationMemory + exogenousRetailMemory;
        uint256 selfExcitedLabel = sourceEvidence == 0 ? 0 : wdiv(selfExcitationMemory, sourceEvidence);
        uint256 exogenousRetailLabel = sourceEvidence == 0 ? 0 : wdiv(exogenousRetailMemory, sourceEvidence);
        if (flowDirectionalRisk > 0 && exogenousRetailLabel > selfExcitedLabel) {
            flowDirectionalRisk = wmul(
                flowDirectionalRisk,
                _oneMinus(wmul(exogenousRetailLabel - selfExcitedLabel, 900 * BPS))
            );
        }
        uint256 sourceDirectionalRisk = 0;
        if (selfExcitedLabel > 6100 * BPS && selfExcitationMemory > 2 * BPS) {
            sourceDirectionalRisk =
                wmul(selfExcitationMemory, 520 * BPS) +
                wmul(wmul(selfExcitedLabel - 6100 * BPS, _max(divergenceMemory, spotJump)), 1800 * BPS);
        }
        ...
        if (sourceDirectionalRisk > 0) {
            if (toxicBidSide) {
                askFlowRisk += sourceDirectionalRisk;
            } else {
                bidFlowRisk += sourceDirectionalRisk;
            }
        }
        ...
        slots[11] = selfExcitationMemory;
        slots[12] = exogenousRetailMemory;

        return (bidFee, askFee);
    }

    function getName() external pure override returns (string memory) {
        return "HawkesExcitationSourceSplitter";
    }
