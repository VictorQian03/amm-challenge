...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 eventPersistence = _oneMinus(gapShort);
        uint256 eventCarry =
            wmul(eventSignal, 150 * BPS) +
            wmul(wmul(eventSignal, eventPersistence), 170 * BPS);
        uint256 directionalBurstFee = 0;
        if (eventSignal > 8 * BPS) {
            uint256 burstSignal = eventSignal - 8 * BPS;
            eventCarry += wmul(wmul(burstSignal, eventPersistence), 430 * BPS);
    ...
    function getName() external pure override returns (string memory) {
        return "PersistentEventCarryPartition";
    }
