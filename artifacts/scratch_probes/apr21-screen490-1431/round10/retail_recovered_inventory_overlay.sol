...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        if (inventoryToxicStress < 1500 * BPS) {
            inventoryToxicOverlay = wmul(
                inventoryToxicOverlay,
                _oneMinus(wmul(quietRetailRecovery, 2200 * BPS))
            );
        }
        uint256 toxicOverlayCap = wmul(widthFlex, 540 * BPS);
    ...
    function getName() external pure override returns (string memory) {
        return "RetailRecoveredInventoryOverlay";
    }
