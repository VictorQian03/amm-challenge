...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 sameDirectionFlow = trade.isBuy ? buyFlow : sellFlow;
        uint256 oppositeDirectionFlow = trade.isBuy ? sellFlow : buyFlow;
        uint256 sameDirectionCluster = _share(sameDirectionFlow, sameDirectionFlow + oppositeDirectionFlow);
        uint256 latencyPressureClass = 0;
        if (gap <= 1 && tradeSize > 18 * BPS && spotJump > 5 * BPS && sameDirectionCluster > 6200 * BPS) {
            latencyPressureClass = 3;
        } else if (gap <= 2 && tradeSize > 12 * BPS && spotJump > 3 * BPS && sameDirectionCluster > 5800 * BPS) {
            latencyPressureClass = 2;
        } else if (gap <= 3 && tradeSize > 8 * BPS && spotJump > 2 * BPS && sameDirectionCluster > 5400 * BPS) {
            latencyPressureClass = 1;
        }
        uint256 latencyPressure = latencyPressureClass * 450 * BPS;
        uint256 hazardObservation = _max(
            divergenceVol,
            volObservation + wmul(clusterObservation, 7000 * BPS) + latencyPressure
        );
        ...
        if (latencyPressure > 0) {
            eventSignal += latencyPressure;
        }
        if (eventSignal > WAD) {
            eventSignal = WAD;
        }
    ...
    function getName() external pure override returns (string memory) {
        return "BatchClearingLatencyPressure";
    }
