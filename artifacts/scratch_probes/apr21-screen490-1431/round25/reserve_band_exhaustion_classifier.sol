...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 reservePressure = _max(tradeSize, absDiff(sizeX, sizeY));
        ...
        uint256 exhaustionClass = 0;
        if (reservePressure > 34 * BPS && spotJump > 7 * BPS) {
            exhaustionClass = 3;
        } else if (reservePressure > 24 * BPS && spotJump > 4 * BPS) {
            exhaustionClass = 2;
        } else if (reservePressure > 16 * BPS || (reservePressure > 10 * BPS && divergence > 4 * BPS)) {
            exhaustionClass = 1;
        }
        uint256 exhaustionSignal = exhaustionClass * 520 * BPS;
        ...
        if (exhaustionSignal > 0) {
            sharedSpread += wmul(exhaustionSignal, 260 * BPS);
        }
        ...
        uint256 exhaustionProtection = wmul(exhaustionSignal, 1350 * BPS);
        if (trade.isBuy) {
            askProtection += exhaustionProtection;
        } else {
            bidProtection += exhaustionProtection;
        }
    ...
    function getName() external pure override returns (string memory) {
        return "ReserveBandExhaustionClassifier";
    }
