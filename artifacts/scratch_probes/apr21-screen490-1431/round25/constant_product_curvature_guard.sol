...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 depthSkew = absDiff(sizeX, sizeY);
        uint256 curvatureStress =
            tradeSize + wmul(tradeSize, tradeSize) + wmul(depthSkew, 4200 * BPS);
        if (tradeSize > 30 * BPS) {
            curvatureStress += wmul(tradeSize - 30 * BPS, 5500 * BPS);
        }
        uint256 curveSpreadAdd = wmul(curvatureStress, 420 * BPS);
        if (curveSpreadAdd > 240 * BPS) {
            curveSpreadAdd = 240 * BPS;
        }
    ...
    function getName() external pure override returns (string memory) {
        return "ConstantProductCurvatureGuard";
    }
