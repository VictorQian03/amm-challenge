...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        if (lastSpot > 0) {
            uint256 previousDivergence = latentSpot == 0 ? 0 : wdiv(absDiff(lastSpot, latentSpot), latentSpot);
            if (previousDivergence > divergence && previousDivergence > 2 * BPS) {
                uint256 reversionProgress = previousDivergence - divergence;
                uint256 hazardVeto = clamp(reversionProgress * 5, 0, 1200 * BPS);
                hazardObservation = wmul(hazardObservation, _oneMinus(hazardVeto));
            }
        }
