...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 benignSize = tradeSize > impactImpulse ? tradeSize - impactImpulse : 0;
        uint256 volObservation = _max(tradeSize, impactImpulse);
        uint256 clusterObservation = wmul(volObservation, _oneMinus(gapShort));
        uint256 baseHazardObservation =
            _max(divergenceVol, volObservation + wmul(clusterObservation, 7000 * BPS));
        uint256 partitionedHazard = _max(
            divergenceVol,
            impactImpulse +
                wmul(benignSize, 4200 * BPS) +
                wmul(clusterObservation, 6200 * BPS)
        );
        uint256 hazardFloor = wmul(baseHazardObservation, 8200 * BPS);
        uint256 hazardObservation = _max(partitionedHazard, hazardFloor);
    ...
    function getName() external pure override returns (string memory) {
        return "ImpactBackstoppedObservationPartition";
    }
