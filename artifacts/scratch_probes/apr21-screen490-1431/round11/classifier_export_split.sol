        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        if (gap >= 3) {
            recenterQuietGate = clamp(
                recenterQuietGate + wmul(wmul(calmMemory, gapLong), 350 * BPS),
                0,
                WAD
            );
        }
        uint256 exportLeakBackstop = clamp(
            wmul(flowPressure, 1500 * BPS) + wmul(oneSidedFlow, 1700 * BPS),
            0,
            2600 * BPS
        );
        recaptureQuietGate = wmul(recaptureQuietGate, _oneMinus(exportLeakBackstop));
        if (gap >= 3) {
