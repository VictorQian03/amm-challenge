...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 oneShotEvent = wmul(eventSignal, gapShort);
        uint256 persistentEvent = eventSignal > oneShotEvent ? eventSignal - oneShotEvent : 0;
        uint256 eventCarry = wmul(eventSignal, 180 * BPS) + wmul(persistentEvent, 120 * BPS);
        uint256 directionalBurstFee = 0;
        if (eventSignal > 8 * BPS) {
            uint256 burstSignal = eventSignal - 8 * BPS;
            eventCarry += wmul(wmul(burstSignal, _oneMinus(gapShort)), 260 * BPS);
    ...
    function getName() external pure override returns (string memory) {
        return "OneShotEventCarryDecoupler";
    }
