...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 inventoryToxicStress = clamp(
            wmul(flowPressure, 2400 * BPS) +
                wmul(oneSidedFlow, 2000 * BPS) +
                wmul(divergenceMemory, 1800 * BPS),
            0,
            WAD
        );
        uint256 quietRetailGuard = wmul(
            wmul(calmMemory, quietGate),
            _oneMinus(
                clamp(
                    wmul(divergenceMemory, 6000 * BPS) + wmul(flowPressure, 1800 * BPS),
                    0,
                    WAD
                )
            )
        );
        uint256 inventoryToxicOverlay = wmul(
            wmul(inventorySkew, inventoryToxicStress),
            760 * BPS
        );
        inventoryToxicOverlay = wmul(
            inventoryToxicOverlay,
            _oneMinus(wmul(quietRetailGuard, 3500 * BPS))
        );
        if (inventoryToxicStress < 1200 * BPS) {
            inventoryToxicOverlay = wmul(inventoryToxicOverlay, 6500 * BPS);
        }
        uint256 toxicOverlayCap = wmul(widthFlex, 520 * BPS);
        if (inventoryToxicOverlay > toxicOverlayCap) {
            inventoryToxicOverlay = toxicOverlayCap;
        }
    ...
    function getName() external pure override returns (string memory) {
        return "RetailFloorGuardedInventoryOverlay";
    }
