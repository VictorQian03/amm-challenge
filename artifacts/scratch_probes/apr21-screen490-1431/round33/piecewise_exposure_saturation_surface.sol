...
/// @title Piecewise Exposure Saturation Surface
/// @notice Translates existing exposure into side protection with a dead zone and convex tail.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 exposureSurface = _max(sideHazard, _max(oneSidedFlow, divergenceMemory));
        uint256 exposureTail = exposureSurface > 7 * BPS ? exposureSurface - 7 * BPS : 0;
        uint256 saturationProtection = wmul(exposureTail, 1700 * BPS);
        if (exposureTail > 8 * BPS) {
            saturationProtection += wmul(exposureTail - 8 * BPS, 2100 * BPS);
        }
        saturationProtection = clamp(saturationProtection, 0, 1600 * BPS);
        if (currentSpot >= latentSpot) {
            bidProtection += saturationProtection;
        } else {
            askProtection += saturationProtection;
        }
