...
/// @title Protection Debt Repayment Gate
/// @notice Tests a debt-before-repair invariant that repays protection before benign state can reopen the book.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // protection debt memory
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 protectionDebt = wmul(
            slots[11],
            _gapAdjustedDecay(9300 * BPS, gapShort, 900 * BPS)
        );
        uint256 freshDebt = wmul(_max(hazardMemory, divergenceMemory), flowPressure);
        protectionDebt = _blend(protectionDebt, freshDebt, 14 * BPS);
        ...
        if (gap >= 3) {
            ...
            if (protectionDebt > 700 * BPS) {
                quietRecenter = wmul(quietRecenter, _oneMinus(clamp(protectionDebt * 3, 0, WAD)));
            }
        ...
        uint256 debtRepayment = wmul(protectionDebt, 520 * BPS);
        bidProtection += debtRepayment;
        askProtection += debtRepayment;
