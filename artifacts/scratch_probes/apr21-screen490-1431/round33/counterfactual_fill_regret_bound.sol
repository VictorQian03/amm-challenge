...
/// @title Counterfactual Fill Regret Bound
/// @notice Stores a bounded prior-fill regret signal consumed only as side-specific protection.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // last trade side: WAD for buy, zero for sell/none
        slots[12] = spot; // last fill spot
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 regretBound = 0;
        if (slots[12] > 0) {
            bool priorBuy = slots[11] > 0;
            bool adverseMove = priorBuy ? currentSpot > slots[12] : currentSpot < slots[12];
            if (adverseMove) {
                regretBound = wdiv(absDiff(currentSpot, slots[12]), slots[12]);
                regretBound = clamp(regretBound, 0, 1800 * BPS);
            }
        }
        ...
        uint256 regretProtection = wmul(regretBound, 3200 * BPS);
        if (slots[11] > 0) {
            askProtection += regretProtection;
        } else {
            bidProtection += regretProtection;
        }
