...
/// @title Adverse Benign Non Substitution
/// @notice Separates adverse and benign evidence so benign state repair cannot cancel adverse protection.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 adverseToken = _max(hazardMemory, wmul(flowPressure, _max(volMemory, divergenceMemory)));
        uint256 benignToken = wmul(calmMemory, _oneMinus(clamp(adverseToken * 5, 0, WAD)));
        ...
        sharedSpread += wmul(adverseToken, 180 * BPS);
        ...
        uint256 sharedRebate = wmul(wmul(_min(calmMemory, benignToken), quietGate), 70 * BPS);
        ...
        uint256 tokenProtection = wmul(adverseToken, 520 * BPS);
        bidProtection += tokenProtection;
        askProtection += tokenProtection;
