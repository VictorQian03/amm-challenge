...
/// @title Convex Adverse Envelope
/// @notice Uses a bounded convex envelope across risk signals before spread and side protection.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 convexAdverseEnvelope = _max(
            wmul(_max(volMemory, hazardMemory), flowPressure),
            wmul(divergenceMemory, _max(flowPressure, spotJump))
        );
        if (convexAdverseEnvelope > 5 * BPS) {
            uint256 tail = convexAdverseEnvelope - 5 * BPS;
            convexAdverseEnvelope = clamp(
                convexAdverseEnvelope + wmul(tail, 2800 * BPS),
                0,
                2200 * BPS
            );
        }
        ...
        sharedSpread += wmul(convexAdverseEnvelope, 260 * BPS);
        ...
        uint256 envelopeProtection = wmul(convexAdverseEnvelope, 1400 * BPS);
        bidProtection += envelopeProtection;
        askProtection += envelopeProtection;
