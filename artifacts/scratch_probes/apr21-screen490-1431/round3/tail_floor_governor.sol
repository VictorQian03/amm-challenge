...
/// @title Tail Floor Governor
/// @notice Clamp generosity after calm-state exposure repeatedly fails to protect the downside floor.
contract Strategy is AMMStrategyBase {
    ...
    uint256 internal constant DECAY_TAIL_DEBT = 9050 * BPS;
    ...
    uint256 internal constant ALPHA_TAIL_DEBT = 22 * BPS;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 tailDebtMemory = wmul(
            slots[11],
            _gapAdjustedDecay(DECAY_TAIL_DEBT, gapLong, 1100 * BPS)
        );
        uint256 tailFailureObservation = wmul(
            wmul(_max(passiveRecaptureMemory, calmMemory), quietGate),
            clamp(
                continuationVeto +
                    wmul(oneSidedFlow, 2600 * BPS) +
                    wmul(_max(divergenceMemory, spotJump), 2400 * BPS) +
                    wmul(hazardMemory, 2200 * BPS),
                0,
                WAD
            )
        );
        if (counterflowRelease > 0 && gap >= 4) {
            tailFailureObservation = clamp(
                tailFailureObservation + wmul(counterflowRelease, 1600 * BPS),
                0,
                WAD
            );
        }
        tailDebtMemory = _blend(tailDebtMemory, tailFailureObservation, ALPHA_TAIL_DEBT);
        uint256 floorGuard = _oneMinus(
            clamp(
                wmul(tailDebtMemory, gap >= 4 ? 7600 * BPS : 5800 * BPS),
                0,
                WAD
            )
        );
        ...
        opportunityGate = wmul(opportunityGate, floorGuard);
        ...
        sharedRebate = wmul(sharedRebate, floorGuard);
        ...
        inventoryGate = wmul(
            inventoryGate,
            _oneMinus(clamp(wmul(tailDebtMemory, 4200 * BPS), 0, WAD))
        );
        ...
        uint256 toxicTailProtection = wmul(tailDebtMemory, 260 * BPS);
        if (currentSpot >= latentSpot) {
            bidProtection +=
                directionalBurstFee + oneSidedProtection + inventoryCenteringOffset + toxicTailProtection;
        } else {
            askProtection +=
                directionalBurstFee + oneSidedProtection + inventoryCenteringOffset + toxicTailProtection;
        }
        ...
        uint256 passiveRecaptureCut =
            wmul(passiveRecaptureMemory, 1550 * BPS) +
            wmul(tailDebtMemory, 640 * BPS);
        ...
        if (hazardMemory < 1100 * BPS && flowPressure < 650 * BPS && gap >= 2) {
            ...
            calmDivergenceBonus = wmul(calmDivergenceBonus, floorGuard);
        }
        ...
        refillCalmConfirmation = wmul(refillCalmConfirmation, floorGuard);
        ...
        refillRiskGate = wmul(refillRiskGate, floorGuard);
        ...
        slots[11] = tailDebtMemory;

        return (bidFee, askFee);
    }

    function getName() external pure override returns (string memory) {
        return "TailFloorGovernor";
    }
