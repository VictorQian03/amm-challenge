...
/// @title Floor Clamped Benign Width Map
/// @notice Harvest calm-state width symmetrically only after reconvergence and explicit floor checks pass.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 reconvergenceDistance =
            latentSpot == 0 ? 0 : wdiv(absDiff(currentSpot, latentSpot), latentSpot);
        uint256 reconvergenceGate = _oneMinus(
            clamp(wmul(reconvergenceDistance, 10000 * BPS), 0, WAD)
        );
        uint256 floorClampGate = wmul(
            wmul(quietGate, reconvergenceGate),
            _oneMinus(
                clamp(
                    hazardMemory * 8 +
                        wmul(flowPressure, 2800 * BPS) +
                        wmul(oneSidedFlow, 2400 * BPS) +
                        wmul(continuationVeto, 9000 * BPS),
                    0,
                    WAD
                )
            )
        );
        uint256 calmBandSignal = _max(passiveRecaptureMemory, calmMemory);
        if (counterflowRelease > 0) {
            calmBandSignal = clamp(
                calmBandSignal + wmul(counterflowRelease, 4200 * BPS),
                0,
                WAD
            );
        }
        uint256 calmBandCut =
            wmul(
                wmul(calmBandSignal, floorClampGate),
                gap >= 4 ? 420 * BPS : 220 * BPS
            ) +
            wmul(
                wmul(divergenceMemory, floorClampGate),
                gap >= 4 ? 120 * BPS : 60 * BPS
            );
        uint256 calmBandCap = wmul(widthFlex, 1400 * BPS);
        if (calmBandCut > calmBandCap) {
            calmBandCut = calmBandCap;
        }
        uint256 reopenRiskGate = _oneMinus(
            clamp(
                hazardMemory * 8 +
        ...
        uint256 floorClampedRefillCut =
            wmul(
                wmul(wmul(passiveRecaptureMemory, quietGate), wmul(reopenRiskGate, floorClampGate)),
                gap >= 4 ? 180 * BPS : 90 * BPS
            );
        uint256 refillCap = wmul(widthFlex, 700 * BPS);
        if (floorClampedRefillCut > refillCap) {
            floorClampedRefillCut = refillCap;
        }
        bidOpportunityCut += passiveRecaptureCut + calmDivergenceBonus + healingRebate + calmBandCut;
        askOpportunityCut += passiveRecaptureCut + calmDivergenceBonus + healingRebate + calmBandCut;
        if (toxicBidSide) {
            askOpportunityCut += floorClampedRefillCut;
        } else {
            bidOpportunityCut += floorClampedRefillCut;
        }
        bidOpportunityCut = clamp(
            bidOpportunityCut,
            0,
            bidProtection + wmul(widthFlex, 1000 * BPS)
        );
        askOpportunityCut = clamp(
            askOpportunityCut,
            0,
            askProtection + wmul(widthFlex, 1000 * BPS)
        );
    ...
    function getName() external pure override returns (string memory) {
        return "FloorClampedBenignWidthMap";
    }
