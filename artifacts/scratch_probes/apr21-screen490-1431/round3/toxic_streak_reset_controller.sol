...
/// @title Toxic Streak Reset Controller
/// @notice Track repeated toxic extensions and only reopen after verified reset conditions.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // toxic streak memory
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 hazardObservation = _max(
            divergenceVol,
            volObservation + wmul(clusterObservation, 7000 * BPS)
        );
        ...
        uint256 passiveRecaptureMemory = _blend(
            slots[10],
            passiveRecaptureObservation,
            ALPHA_PASSIVE
        );
        ...
        uint256 toxicStreak = slots[11];
        toxicStreak = wmul(
            toxicStreak,
            continuationAligned ? 9300 * BPS : gap >= 4 ? 7000 * BPS : 8600 * BPS
        );
        uint256 toxicExtension = 0;
        if (continuationAligned) {
            toxicExtension = clamp(
                continuationVeto +
                    wmul(_max(divergenceMemory, spotJump), 2600 * BPS) +
                    wmul(oneSidedFlow, 2600 * BPS),
                0,
                WAD
            );
            toxicStreak = clamp(toxicStreak + toxicExtension, 0, WAD);
        }
        uint256 resetSignal = 0;
        if (gap >= 4) {
            uint256 resetGate = _oneMinus(
                clamp(
                    wmul(_max(divergenceMemory, spotJump), 10000 * BPS) +
            ...
            resetSignal = wmul(
                wmul(quietGate, resetGate),
                _max(passiveRecaptureMemory, counterflowRelease)
            );
            uint256 resetFactor = _oneMinus(clamp(wmul(resetSignal, 8000 * BPS), 0, WAD));
            toxicStreak = wmul(toxicStreak, resetFactor);
        }
        uint256 streakGuard = _oneMinus(
            clamp(
                wmul(toxicStreak, continuationAligned ? 7200 * BPS : 5600 * BPS),
                0,
                WAD
            )
        );
        counterflowRelease = wmul(counterflowRelease, streakGuard);
        ...
        uint256 streakSpreadPenalty = wmul(toxicStreak, 220 * BPS);
        sharedSpread += eventCarry + streakSpreadPenalty;

        uint256 sharedRebate = wmul(wmul(calmMemory, quietGate), 110 * BPS);
        sharedRebate = wmul(sharedRebate, streakGuard);
        ...
        uint256 healingRebate = wmul(wmul(directionalBurstFee, 2800 * BPS), streakGuard);
        ...
        inventoryGate = wmul(inventoryGate, streakGuard);
        ...
        uint256 streakProtection = wmul(toxicStreak, 240 * BPS);
        if (currentSpot >= latentSpot) {
            bidProtection +=
                directionalBurstFee + oneSidedProtection + inventoryCenteringOffset + streakProtection;
        } else {
            askProtection +=
                directionalBurstFee + oneSidedProtection + inventoryCenteringOffset + streakProtection;
        }
        ...
        if (hazardMemory < 1100 * BPS && flowPressure < 650 * BPS && gap >= 2) {
            ...
            calmDivergenceBonus = wmul(calmDivergenceBonus, streakGuard);
        }
        ...
        refillCalmConfirmation = wmul(refillCalmConfirmation, streakGuard);
        ...
        refillRiskGate = wmul(refillRiskGate, streakGuard);
        ...
        if (toxicBidSide) {
            askOpportunityCut +=
                passiveRecaptureCut + calmDivergenceBonus + healingRebate + inventoryCenteringOffset;
        ...
        slots[11] = toxicStreak;

        return (bidFee, askFee);
    }

    function getName() external pure override returns (string memory) {
        return "ToxicStreakResetController";
    }
