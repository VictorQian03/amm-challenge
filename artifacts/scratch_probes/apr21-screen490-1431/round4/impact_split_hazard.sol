...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 hazardObservation =
            _max(divergenceVol, impactImpulse + wmul(clusterObservation, 6200 * BPS));
        ...
        uint256 participationPulse = tradeSize + wmul(volObservation, 2500 * BPS);
        uint256 flowPulse = participationPulse + wmul(impactImpulse, 3200 * BPS);
        uint256 crossPulse = wmul(flowPulse, impactImpulse > 5 * BPS ? 1200 * BPS : 1500 * BPS);
        if (trade.isBuy) {
        ...
        if (flowPressure > 500 * BPS && extensionSignal > 4 * BPS) {
            uint256 toxicFlowSignal =
                flowPressure +
                wmul(extensionSignal, 2800 * BPS) +
                wmul(impactImpulse, 1200 * BPS);
        ...
        uint256 eventSignal = impactImpulse + hazardObservation;
        if (eventSignal > WAD) {
            eventSignal = WAD;
        }
