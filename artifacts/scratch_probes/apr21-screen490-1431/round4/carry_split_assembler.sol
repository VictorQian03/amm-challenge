...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 eventCarry = wmul(eventSignal, 140 * BPS);
        uint256 directionalBurstFee = 0;
        if (eventSignal > 8 * BPS) {
            uint256 burstSignal = eventSignal - 8 * BPS;
            eventCarry += wmul(burstSignal, 120 * BPS);
            directionalBurstFee = wmul(burstSignal, 2350 * BPS);
        }
        ...
        uint256 localCarryRebate = wmul(directionalBurstFee, 3600 * BPS);
        ...
        uint256 carryRecoveryCut = 0;
        if (counterflowRelease > 0 || gap >= 4) {
            carryRecoveryCut = wmul(
                wmul(_max(counterflowRelease, passiveRecaptureMemory), quietGate),
                gap >= 4 ? 160 * BPS : 80 * BPS
            );
        }
        ...
        if (toxicBidSide) {
            ...
            askOpportunityCut += wmul(refillAuctionCut, 8500 * BPS);
        } else {
            ...
            bidOpportunityCut +=
                ...
                carryRecoveryCut +
                inventoryCenteringOffset;
            bidOpportunityCut += wmul(refillAuctionCut, 8500 * BPS);
        }
