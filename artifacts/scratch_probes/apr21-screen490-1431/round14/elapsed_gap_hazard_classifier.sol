...
/// @title Elapsed Gap Hazard Classifier
/// @notice Keeps the starter quote map but feeds elapsed-gap evidence into hazard/calm classification.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 elapsedCompression = _oneMinus(gapShort);
        uint256 residualImpact = volObservation > divergenceVol
            ? volObservation - divergenceVol
            : 0;
        uint256 compressedEventEvidence = wmul(
            elapsedCompression,
            _max(residualImpact, divergenceVol)
        );
        uint256 elapsedGapHazard = wmul(compressedEventEvidence, 4200 * BPS);
        if (gap <= 1 && volObservation > 4 * BPS) {
            elapsedGapHazard += wmul(volObservation, 1200 * BPS);
        }
        uint256 elapsedGapCalm = wmul(
            gapLong,
            _oneMinus(
                clamp(
                    wmul(volObservation, 8500 * BPS) +
                        wmul(divergenceVol, 7000 * BPS) +
                        wmul(elapsedCompression, 2500 * BPS),
                    0,
                    WAD
                )
            )
        ...
        uint256 clusterObservation = wmul(volObservation, elapsedCompression);
        uint256 hazardObservation = _max(
            divergenceVol,
            volObservation + wmul(clusterObservation, 7000 * BPS) + elapsedGapHazard
        );
    ...
    function getName() external pure override returns (string memory) {
        return "ElapsedGapHazardClassifier";
    }
