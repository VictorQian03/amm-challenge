...
/// @title Passive Queue Fill Probability Witness
/// @notice Tests adverse passive-fill feasibility as a one-way uncertainty source.
contract Strategy is AMMStrategyBase {
    ...
    uint256 internal constant DECAY_QUEUE_FILL = 9000 * BPS;
    ...
    uint256 internal constant ALPHA_QUEUE_FILL = 24 * BPS;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // adverse passive queue-fill witness memory
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 queueFillMemory = wmul(
            slots[11],
            _gapAdjustedDecay(DECAY_QUEUE_FILL, gapShort, 1500 * BPS)
        );
        ...
        uint256 sameSideFillPressure = trade.isBuy ? buyFlow : sellFlow;
        uint256 oppositeFillPressure = trade.isBuy ? sellFlow : buyFlow;
        uint256 queueFillProbability = _share(
            sameSideFillPressure + tradeSize,
            sameSideFillPressure + oppositeFillPressure + tradeSize
        );
        uint256 adverseQueueFillEvidence = 0;
        bool adverseFillSide = trade.isBuy ? currentSpot >= latentSpot : currentSpot < latentSpot;
        if (adverseFillSide && divergence > 3 * BPS) {
            adverseQueueFillEvidence = wmul(
                queueFillProbability,
                _max(divergenceVol, wmul(tradeSize, 5200 * BPS))
            );
            adverseQueueFillEvidence = clamp(adverseQueueFillEvidence, 0, 12 * BPS);
            hazardObservation = clamp(
                hazardObservation + wmul(adverseQueueFillEvidence, 8200 * BPS),
                0,
                WAD
            );
        }
        ...
        queueFillMemory = _blend(queueFillMemory, adverseQueueFillEvidence, ALPHA_QUEUE_FILL);
        ...
        sideHazard += wmul(queueFillMemory, 1350 * BPS);
        if (sideHazard > WAD) {
            sideHazard = WAD;
        }
        ...
        slots[11] = queueFillMemory;

        return (bidFee, askFee);
    }

    function getName() external pure override returns (string memory) {
        return "PassiveQueueFillProbabilityWitness";
    }
