...
/// @title Depth Imbalance Crossing Barrier
/// @notice Tests reserve-depth sign crossings as a bounded hold-only adverse barrier.
contract Strategy is AMMStrategyBase {
    ...
    uint256 internal constant ALPHA_DEPTH = 24 * BPS;
    uint256 internal constant ALPHA_BARRIER = 20 * BPS;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // signed-depth crossing memory
        slots[12] = 0; // depth-crossing barrier state
        slots[13] = 0; // last reserve-depth side
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 reserveXValue = wmul(trade.reserveX, latentSpot);
        uint256 reserveYValue = trade.reserveY;
        uint256 depthTotal = reserveXValue + reserveYValue;
        uint256 depthImbalance =
            depthTotal == 0 ? 0 : wdiv(absDiff(reserveXValue, reserveYValue), depthTotal);
        bool yDepthHeavy = reserveYValue >= reserveXValue;
        uint256 depthSide = yDepthHeavy ? 1 : 2;
        bool depthCrossed = slots[13] != 0 && slots[13] != depthSide;
        uint256 signedDepthMemory = wmul(
            slots[11],
            _gapAdjustedDecay(9150 * BPS, gapShort, 1300 * BPS)
        );
        uint256 barrierState = wmul(
            slots[12],
            _gapAdjustedDecay(9050 * BPS, gapShort, 1500 * BPS)
        );
        uint256 depthCrossObservation = depthCrossed
            ? clamp(
                wmul(depthImbalance, 6800 * BPS) +
                    wmul(tradeSize, 1300 * BPS) +
                    wmul(_max(spotJump, divergence), 900 * BPS),
                0,
                16 * BPS
            )
            : 0;
        signedDepthMemory = _blend(signedDepthMemory, depthImbalance, ALPHA_DEPTH);
        barrierState = _blend(barrierState, depthCrossObservation, ALPHA_BARRIER);
        bool adverseDepthLabel =
            depthCrossed &&
            ((toxicBidSide && trade.isBuy && yDepthHeavy) ||
                (!toxicBidSide && !trade.isBuy && !yDepthHeavy));
        if (adverseDepthLabel) {
            barrierState = clamp(
                barrierState +
                    wmul(signedDepthMemory, 900 * BPS) +
                    wmul(flowPressure, 650 * BPS),
                0,
                22 * BPS
            );
        }
        ...
        sideHazard += wmul(barrierState, 2600 * BPS);
        if (sideHazard > WAD) {
            sideHazard = WAD;
        }
        ...
        uint256 depthBarrierProtection = wmul(barrierState, 1900 * BPS);
        ...
        if (currentSpot >= latentSpot) {
            bidProtection +=
                directionalBurstFee + oneSidedProtection + depthBarrierProtection + inventoryCenteringOffset;
        } else {
            askProtection +=
                directionalBurstFee + oneSidedProtection + depthBarrierProtection + inventoryCenteringOffset;
        }
        ...
        slots[11] = signedDepthMemory;
        slots[12] = barrierState;
        slots[13] = depthSide;
    ...
    function getName() external pure override returns (string memory) {
        return "DepthImbalanceCrossingBarrier";
    }
