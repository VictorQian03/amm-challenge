...
/// @title Resilient Depth Shape Recovery State
/// @notice Tests a one-way layer-2 depth-shape stress memory with bounded recovery.
contract Strategy is AMMStrategyBase {
    ...
    uint256 internal constant DECAY_DEPTH_SHAPE = 8850 * BPS;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // depth-shape stress / recovery memory
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 depthShapeStress = wmul(
            slots[11],
            _gapAdjustedDecay(DECAY_DEPTH_SHAPE, gapLong, 900 * BPS)
        );
        ...
        uint256 recoveryShape = _max(
            wmul(tradeSize, 5200 * BPS),
            _max(wmul(spotJumpVol, 4200 * BPS), wmul(divergenceVol, 3600 * BPS))
        );
        uint256 recoveryImbalance = totalShapeSkew(
            trade.isBuy ? tradeSize : 0,
            trade.isBuy ? 0 : tradeSize,
            buyFlow,
            sellFlow
        );
        uint256 fragileShapeLabel = clamp(
            recoveryShape +
                wmul(recoveryImbalance, 1800 * BPS) +
                (weaklyInconsistentEvent ? 160 * BPS : 0),
            0,
            18 * BPS
        );
        uint256 stableShapeLabel = 0;
        if (!weaklyInconsistentEvent && gap >= 3) {
            stableShapeLabel = wmul(
                gapLong,
                _oneMinus(
                    clamp(
                        wmul(volObservation, 5000 * BPS) +
                            wmul(divergence, 6200 * BPS) +
                            wmul(depthShapeStress, 2400 * BPS),
                        0,
                        WAD
                    )
                )
        ...
        uint256 recoveredDepthShapeStress = depthShapeStress;
        if (stableShapeLabel > 0) {
            recoveredDepthShapeStress = wmul(
                recoveredDepthShapeStress,
                _oneMinus(clamp(wmul(stableShapeLabel, 900 * BPS), 0, 1600 * BPS))
            );
        }
        if (fragileShapeLabel > recoveredDepthShapeStress) {
            recoveredDepthShapeStress = fragileShapeLabel;
        }
        depthShapeStress = recoveredDepthShapeStress;
        uint256 depthShapeFloor = wmul(depthShapeStress, 2300 * BPS);
        if (hazardObservation < depthShapeFloor) {
            hazardObservation = depthShapeFloor;
        }
        ...
        sideHazard += wmul(depthShapeStress, 750 * BPS);
        if (sideHazard > WAD) {
            sideHazard = WAD;
        }
        ...
        slots[11] = depthShapeStress;

        return (bidFee, askFee);
    }

    function getName() external pure override returns (string memory) {
        return "ResilientDepthShapeRecoveryState";
    }
    ...
    function totalShapeSkew(
        uint256 buyPulse,
        uint256 sellPulse,
        uint256 buyFlow,
        uint256 sellFlow
    ) internal pure returns (uint256) {
        uint256 buyShape = buyFlow + buyPulse;
        uint256 sellShape = sellFlow + sellPulse;
        uint256 totalShape = buyShape + sellShape;
        if (totalShape == 0) {
            return 0;
        }
        return wdiv(absDiff(buyShape, sellShape), totalShape);
    }
}
