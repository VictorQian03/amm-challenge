...
/// @title Latency Adverse Window Budget
/// @notice Tests a bounded stale-adverse evidence budget with hold-only downstream use.
contract Strategy is AMMStrategyBase {
    uint256 internal constant BASE_FEE = 16 * BPS;
    uint256 internal constant HARD_FEE_CAP = 51 * BPS;
    ...
    uint256 internal constant ALPHA_LATENCY = 24 * BPS;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[10] = 0; // latency-window adverse budget memory
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 latencyWindow = 0;
        if (gap >= 2 && gap <= 6) {
            latencyWindow = gap <= 3 ? WAD : _oneMinus(_gapRatio(gap - 3, 3));
        }
        uint256 staleDisplacement = _max(spotJump, divergence);
        uint256 latencyAdverseEvidence = 0;
        if (latencyWindow > 0 && staleDisplacement > 4 * BPS && tradeSize > 2 * BPS) {
            latencyAdverseEvidence = clamp(
                wmul(
                    wmul(staleDisplacement, 3600 * BPS) +
                        wmul(tradeSize, 1100 * BPS),
                    latencyWindow
                ),
                0,
                8 * BPS
            ...
            if (weaklyInconsistentEvent) {
                latencyAdverseEvidence = clamp(
                    latencyAdverseEvidence + wmul(feasibilityResidual, 3500 * BPS),
                    0,
                    10 * BPS
                );
            }
            hazardObservation = clamp(hazardObservation + latencyAdverseEvidence, 0, WAD);
        }
        ...
        uint256 latencyBudgetMemory = wmul(
            slots[10],
            _gapAdjustedDecay(9300 * BPS, gapShort, 1600 * BPS)
        );
        latencyBudgetMemory = _blend(latencyBudgetMemory, latencyAdverseEvidence, ALPHA_LATENCY);
        if (latencyBudgetMemory > 12 * BPS) {
            latencyBudgetMemory = 12 * BPS;
        }
        ...
        bool staleAdverseLabel = latencyBudgetMemory > BPS && gap >= 2 && gap <= 6;
        uint256 sharedRebate = wmul(wmul(calmMemory, quietGate), 110 * BPS);
        if (staleAdverseLabel) {
            uint256 latencyHold = wmul(latencyBudgetMemory, 2600 * BPS);
            sharedRebate = sharedRebate > latencyHold ? sharedRebate - latencyHold : 0;
        }
        ...
        if (bidFee > HARD_FEE_CAP) {
            bidFee = HARD_FEE_CAP;
        }
        if (askFee > HARD_FEE_CAP) {
            askFee = HARD_FEE_CAP;
        }
        ...
        slots[10] = latencyBudgetMemory;

        return (bidFee, askFee);
    }

    function getName() external pure override returns (string memory) {
        return "LatencyAdverseWindowBudget";
    }
