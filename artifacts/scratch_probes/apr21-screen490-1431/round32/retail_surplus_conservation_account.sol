...
contract Strategy is AMMStrategyBase {
    ...
    uint256 internal constant ALPHA_RETAIL_ACCOUNT = 16 * BPS;
    uint256 internal constant RETAIL_ACCOUNT_NEUTRAL = WAD / 2;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = RETAIL_ACCOUNT_NEUTRAL; // retail surplus / deficit conservation account
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 retailAccount = slots[11] == 0 ? RETAIL_ACCOUNT_NEUTRAL : slots[11];
        uint256 benignSurplus = wmul(
            wmul(calmMemory, quietGate),
            _oneMinus(clamp(wmul(flowPressure, 2600 * BPS), 0, WAD))
        );
        uint256 toxicDeficit = clamp(
            wmul(hazardMemory, 3400 * BPS) +
                wmul(divergenceMemory, 2600 * BPS) +
                wmul(oneSidedFlow, 2200 * BPS),
            0,
            WAD
        );
        if (benignSurplus >= toxicDeficit) {
            uint256 surplusStep = wmul(benignSurplus - toxicDeficit, ALPHA_RETAIL_ACCOUNT);
            retailAccount = clamp(retailAccount + surplusStep, 0, WAD);
        } else {
            uint256 deficitStep = wmul(toxicDeficit - benignSurplus, ALPHA_RETAIL_ACCOUNT + 600 * BPS);
            retailAccount = retailAccount > deficitStep ? retailAccount - deficitStep : 0;
        }
        uint256 accountReversion = gap >= 4 ? 500 * BPS : gap >= 2 ? 250 * BPS : 100 * BPS;
        retailAccount = _blend(retailAccount, RETAIL_ACCOUNT_NEUTRAL, accountReversion);
        uint256 retailDeficit = retailAccount < RETAIL_ACCOUNT_NEUTRAL
            ? RETAIL_ACCOUNT_NEUTRAL - retailAccount
            : 0;
        uint256 floorRiskLabel = 0;
        if (retailDeficit > 0) {
            floorRiskLabel = clamp(
                wmul(retailDeficit, 3200 * BPS) +
                    wmul(toxicDeficit, 900 * BPS),
                0,
                7200 * BPS
            );
        }
        ...
        sharedRebate = wmul(sharedRebate, _oneMinus(floorRiskLabel));
        ...
        slots[11] = retailAccount;

        return (bidFee, askFee);
    }

    function getName() external pure override returns (string memory) {
        return "RetailSurplusConservationAccount";
    }
