...
/// @title Two Account Spread Assembly
/// @notice Map existing state classes into non-fungible adverse-protection and benign-service width accounts.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[10] = 0; // benign service account memory
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 serviceObservation = wmul(
            gapLong,
            _oneMinus(
                clamp(
        ...
        if (gap >= 4) {
            uint256 serviceRecenterDivergence =
                latentSpot == 0 ? 0 : wdiv(absDiff(currentSpot, latentSpot), latentSpot);
            uint256 serviceRecenterGate = _oneMinus(
                clamp(wmul(serviceRecenterDivergence, 10000 * BPS), 0, WAD)
            );
            serviceObservation = wmul(
                serviceObservation,
                wmul(quietGate, serviceRecenterGate)
            );
        }
        uint256 serviceAccountMemory = _blend(slots[10], serviceObservation, ALPHA_PASSIVE);
        if (gap >= 4) {
            serviceAccountMemory = wmul(serviceAccountMemory, 9600 * BPS);
        } else if (gap >= 2) {
            serviceAccountMemory = wmul(serviceAccountMemory, 9000 * BPS);
        }
        ...
        if (eventSignal > 8 * BPS) {
            ...
            eventCarry += wmul(burstSignal, 250 * BPS);
            directionalBurstFee = wmul(burstSignal, 1680 * BPS);
        }
        sharedSpread += eventCarry;

        uint256 benignServiceGate = wmul(
            wmul(quietGate, serviceAccountMemory),
            _oneMinus(
                clamp(
                    hazardMemory * 7 +
                        wmul(flowPressure, 2600 * BPS) +
                        wmul(oneSidedFlow, 2200 * BPS) +
                        wmul(divergenceMemory, 4300 * BPS),
                    0,
                    WAD
                )
            )
        );
        uint256 sharedServiceCredit = wmul(benignServiceGate, 95 * BPS);
        sharedSpread = sharedSpread > sharedServiceCredit ? sharedSpread - sharedServiceCredit : MIN_FEE;

        uint256 bidProtection = wmul(bidRiskSignal, 5150 * BPS);
        uint256 askProtection = wmul(askRiskSignal, 5150 * BPS);
        uint256 adverseAccount = wmul(oneSidedFlow, 2450 * BPS);
        uint256 burstAccount = directionalBurstFee;
        if (currentSpot >= latentSpot) {
            bidProtection += burstAccount + adverseAccount;
        } else {
            askProtection += burstAccount + adverseAccount;
        }

        uint256 sideServiceGate = _oneMinus(
            clamp(
                hazardMemory * 9 +
                    wmul(flowPressure, 3600 * BPS) +
                    wmul(oneSidedFlow, 3000 * BPS) +
                    wmul(divergenceMemory, 5600 * BPS),
                0,
                WAD
            )
        );
        uint256 sideServiceAccount = wmul(
            wmul(serviceAccountMemory, sideServiceGate),
            _oneMinus(clamp(wmul(flowImbalance, 4500 * BPS), 0, WAD))
        );
        uint256 sideServiceCredit = wmul(sideServiceAccount, gap >= 4 ? 900 * BPS : 520 * BPS);
        uint256 calmServiceCredit = 0;
        if (hazardMemory < 1000 * BPS && flowPressure < 600 * BPS && gap >= 2) {
            calmServiceCredit = wmul(calmMemory, gap >= 4 ? 330 * BPS : 210 * BPS);
        }
        uint256 bidServiceCredit = 0;
        uint256 askServiceCredit = 0;
        if (toxicBidSide) {
            askServiceCredit = sideServiceCredit + calmServiceCredit;
        } else {
            bidServiceCredit = sideServiceCredit + calmServiceCredit;
        }
        uint256 baselineFlex = sharedSpread > BASE_FEE ? sharedSpread - BASE_FEE : 0;
        bidServiceCredit = clamp(
            bidServiceCredit,
            0,
            wmul(baselineFlex, 1800 * BPS)
        );
        askServiceCredit = clamp(
            askServiceCredit,
            0,
            wmul(baselineFlex, 1800 * BPS)
        );
        ...
        bidFee = bidFee > bidServiceCredit ? bidFee - bidServiceCredit : MIN_FEE;
        askFee = askFee > askServiceCredit ? askFee - askServiceCredit : MIN_FEE;
        ...
        slots[10] = serviceAccountMemory;

        return (bidFee, askFee);
    }

    function getName() external pure override returns (string memory) {
        return "TwoAccountSpreadAssembly";
    }
