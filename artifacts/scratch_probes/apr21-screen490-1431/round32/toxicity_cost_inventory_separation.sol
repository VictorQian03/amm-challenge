...
/// @title Toxicity Cost Inventory Separation
/// @notice Keep adverse-selection cost evidence separate from capital-cost evidence before preserving shared spread.
contract Strategy is AMMStrategyBase {
    ...
    uint256 internal constant DECAY_COST = 9050 * BPS;
    ...
    uint256 internal constant ALPHA_COST = 20 * BPS;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // public layer 2 adverse-selection cost evidence
        slots[12] = 0; // public layer 2 capital / inventory-cost evidence
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 adverseCostMemory = wmul(
            slots[11],
            _gapAdjustedDecay(DECAY_COST, gapShort, 1500 * BPS)
        );
        uint256 capitalCostMemory = wmul(
            slots[12],
            _gapAdjustedDecay(DECAY_COST, gapLong, 900 * BPS)
        );
        ...
        uint256 adverseDirectionalEvidence = wmul(
            flowPressure,
            _max(divergenceVol, spotJumpVol)
        );
        if (continuationAligned) {
            adverseDirectionalEvidence = clamp(
                adverseDirectionalEvidence + wmul(oneSidedFlow, 1800 * BPS),
                0,
                WAD
            );
        }
        uint256 adverseCostObservation = _max(divergenceVol, adverseDirectionalEvidence);
        uint256 capitalCostObservation = wmul(
            tradeSize + wmul(volObservation, 1800 * BPS),
            _oneMinus(
                clamp(
                    wmul(adverseCostObservation, 9000 * BPS) +
        ...
        if (gap >= 3) {
            capitalCostObservation = clamp(
                capitalCostObservation + wmul(wmul(gapLong, calmMemory), 420 * BPS),
                0,
                WAD
            );
        }
        adverseCostMemory = _blend(adverseCostMemory, adverseCostObservation, ALPHA_COST);
        capitalCostMemory = _blend(capitalCostMemory, capitalCostObservation, ALPHA_COST);
        uint256 costTotal = adverseCostMemory + capitalCostMemory;
        uint256 adverseCostDominance =
            adverseCostMemory > capitalCostMemory ? wdiv(adverseCostMemory - capitalCostMemory, costTotal) : 0;
        bool adverseSelectionOnly =
            adverseCostMemory > 3 * BPS &&
            adverseCostDominance > 1800 * BPS &&
            flowPressure > 450 * BPS;
        ...
        if (adverseSelectionOnly) {
            sharedRebate = wmul(
                sharedRebate,
                _oneMinus(clamp(wmul(adverseCostDominance, 7000 * BPS), 0, WAD))
            );
        }
        ...
        if (toxicBidSide) {
            askOpportunityCut += passiveRecaptureCut + calmDivergenceBonus + healingRebate;
            askOpportunityCut += refillAuctionCut;
        } else {
            bidOpportunityCut += passiveRecaptureCut + calmDivergenceBonus + healingRebate;
        ...
        slots[11] = adverseCostMemory;
        slots[12] = capitalCostMemory;

        return (bidFee, askFee);
    }

    function getName() external pure override returns (string memory) {
        return "ToxicityCostInventorySeparation";
    }
