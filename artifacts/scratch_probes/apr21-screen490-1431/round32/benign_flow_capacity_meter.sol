...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 balanceEvidence = _oneMinus(clamp(wmul(flowImbalance, 5600 * BPS), 0, WAD));
        uint256 informationClearance = _oneMinus(
            clamp(
                wmul(hazardMemory, 5200 * BPS) +
                    wmul(divergenceMemory, 4600 * BPS) +
                    wmul(spotJump, 3800 * BPS) +
                    wmul(flowPressure, 3000 * BPS),
                0,
                WAD
            )
        );
        uint256 benignCapacityEvidence = wmul(
            wmul(gapLong, _max(calmMemory, passiveRecaptureMemory)),
            wmul(quietGate, wmul(balanceEvidence, informationClearance))
        );
        uint256 capacityClass = 0;
        if (
            gap >= 4 &&
            benignCapacityEvidence > 1350 * BPS &&
            hazardMemory < 950 * BPS &&
            divergenceMemory < 950 * BPS &&
            flowPressure < 620 * BPS
        ) {
            capacityClass = 2;
        } else if (
            gap >= 2 &&
            benignCapacityEvidence > 550 * BPS &&
            hazardMemory < 1450 * BPS &&
            flowPressure < 850 * BPS
        ) {
            capacityClass = 1;
        }

        uint256 protectionBuyShare = buyShare;
        uint256 protectionSellShare = sellShare;
        if (capacityClass == 2) {
            protectionBuyShare = _pullTowardHalf(protectionBuyShare, 1300 * BPS);
            protectionSellShare = _pullTowardHalf(protectionSellShare, 1300 * BPS);
        } else if (capacityClass == 1) {
            protectionBuyShare = _pullTowardHalf(protectionBuyShare, 550 * BPS);
            protectionSellShare = _pullTowardHalf(protectionSellShare, 550 * BPS);
        }

        uint256 bidRiskSignal =
            wmul(protectionSellShare, sideHazard) +
            wmul(richSignal, 8500 * BPS) +
            bidFlowRisk;
        uint256 askRiskSignal =
            wmul(protectionBuyShare, sideHazard) +
        ...
        if (capacityClass == 2) {
            uint256 overprotectionShift = wmul(
                directionalBurstFee + oneSidedProtection + inventoryCenteringOffset,
                850 * BPS
            ) + wmul(absDiff(bidProtection, askProtection), 300 * BPS);
            uint256 shiftCap = wmul(sharedSpread > BASE_FEE ? sharedSpread - BASE_FEE : 0, 900 * BPS);
            if (overprotectionShift > shiftCap) {
                overprotectionShift = shiftCap;
            }
            if (currentSpot >= latentSpot) {
                if (overprotectionShift > bidProtection) {
                    overprotectionShift = bidProtection;
                }
                bidProtection -= overprotectionShift;
                askProtection += overprotectionShift;
            } else {
                if (overprotectionShift > askProtection) {
                    overprotectionShift = askProtection;
                }
                askProtection -= overprotectionShift;
                bidProtection += overprotectionShift;
            }
    ...
    function getName() external pure override returns (string memory) {
        return "BenignFlowCapacityMeter";
    }
    ...
    function _pullTowardHalf(uint256 share, uint256 weight) internal pure returns (uint256) {
        if (share > WAD / 2) {
            return share - wmul(share - WAD / 2, weight);
        }
        return share + wmul(WAD / 2 - share, weight);
    }
}
