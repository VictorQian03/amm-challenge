...
contract Strategy is AMMStrategyBase {
    ...
    uint256 internal constant DECAY_SYMMETRY_DEBT = 9250 * BPS;
    ...
    uint256 internal constant ALPHA_SYMMETRY_DEBT = 16 * BPS;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // cross-side symmetry debt memory
        slots[12] = 0; // symmetry debt attribution class
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 symmetryDebtMemory = wmul(
            slots[11],
            _gapAdjustedDecay(DECAY_SYMMETRY_DEBT, gapLong, 900 * BPS)
        );
        ...
        uint256 askRiskSignal =
            ...
            askFlowRisk;
        uint256 riskSymmetryGap = absDiff(bidRiskSignal, askRiskSignal);
        uint256 flowSymmetryDebt = wmul(flowImbalance, _max(volMemory, hazardMemory));
        uint256 debtObservation = _max(riskSymmetryGap, flowSymmetryDebt);
        uint256 debtClass = 0;
        if (debtObservation > 10 * BPS) {
            debtClass = riskSymmetryGap >= flowSymmetryDebt ? 2 : 1;
        } else if (symmetryDebtMemory > 6 * BPS) {
            debtClass = 3;
        }
        uint256 classWeight = debtClass == 2 ? 1250 * BPS : debtClass == 1 ? 900 * BPS : 550 * BPS;
        symmetryDebtMemory = _blend(symmetryDebtMemory, debtObservation, ALPHA_SYMMETRY_DEBT);
        ...
        if (symmetryDebtMemory > 4 * BPS) {
            uint256 debtWidth = wmul(symmetryDebtMemory - 4 * BPS, classWeight);
            uint256 debtWidthCap = BASE_FEE + wmul(_max(volMemory, hazardMemory), 650 * BPS);
            if (debtWidth > debtWidthCap) {
                debtWidth = debtWidthCap;
            }
            sharedSpread += debtWidth;
        }
