...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        if (eventSignal > 8 * BPS) {
            ...
            uint256 burstLabelGate = WAD;
            if (
                gap >= 2 &&
                !continuationAligned &&
                oneSidedFlow < 450 * BPS &&
                divergenceMemory < 450 * BPS &&
                spotJump < 450 * BPS
            ) {
                uint256 benignOneShotLabel = wmul(
                    gapShort,
                    _oneMinus(
                        clamp(
                            wmul(flowPressure, 2200 * BPS) +
                                wmul(divergenceMemory, 5000 * BPS) +
                                wmul(spotJump, 5000 * BPS),
                            0,
                            WAD
                        )
                    )
                );
                burstLabelGate = _oneMinus(clamp(wmul(benignOneShotLabel, 2200 * BPS), 0, 3000 * BPS));
            }
            directionalBurstFee = wmul(wmul(burstSignal, 1850 * BPS), burstLabelGate);
        }
    ...
    function getName() external pure override returns (string memory) {
        return "BenignOneShotBurstLabelFirewall";
    }
