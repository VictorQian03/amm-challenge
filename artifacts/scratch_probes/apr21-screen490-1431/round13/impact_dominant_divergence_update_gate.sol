...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 impactBasisTotal = impactObservation + boundedParticipation;
        uint256 impactDominance = _share(impactObservation, impactBasisTotal);
        uint256 divergenceSample = divergence;
        if (impactBasisTotal > 0 && impactDominance > 6200 * BPS) {
            divergenceSample = clamp(divergence + wmul(divergenceVol, 1800 * BPS), 0, WAD);
        } else if (impactBasisTotal > 0) {
            divergenceSample = wmul(divergence, 7200 * BPS);
        }
        divergenceMemory = _blend(divergenceMemory, divergenceSample, ALPHA_DIVERGENCE);
    ...
    function getName() external pure override returns (string memory) {
        return "ImpactDominantDivergenceUpdateGate";
    }
