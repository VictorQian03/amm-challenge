...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 basisToxicityFirewall = clamp(
            wmul(impactObservation, 2600 * BPS) +
                wmul(wmul(boundedParticipation, _oneMinus(gapShort)), 900 * BPS) +
                wmul(oneSidedFlow, 1500 * BPS) +
                wmul(flowPressure, 900 * BPS),
            0,
            2800 * BPS
        );
        uint256 recenterQuietExport = quietGate;
        if (gap >= 3) {
            recenterQuietExport = clamp(
                recenterQuietExport + wmul(wmul(calmMemory, gapLong), 180 * BPS),
                0,
                WAD
            );
            recenterQuietExport = wmul(recenterQuietExport, _oneMinus(wmul(basisToxicityFirewall, 4500 * BPS)));
        }
        uint256 spreadQuietExport = wmul(
            quietGate,
            _oneMinus(clamp(wmul(oneSidedFlow, 900 * BPS), 0, 1200 * BPS))
        );
        uint256 recaptureQuietExport = wmul(quietGate, _oneMinus(basisToxicityFirewall));
        if (gap >= 3) {
            uint256 quietRecenter =
                wmul(recenterQuietExport, gap >= 5 ? 1050 * BPS : gap >= 4 ? 750 * BPS : 550 * BPS);
        ...
        if (gap >= 4) {
            ...
            uint256 quietRelease = wmul(wmul(recenterQuietExport, reconvergenceGate), benignGapGate);
        ...
        if (gap >= 4) {
            ...
            passiveRecaptureObservation = wmul(
                ...
                wmul(recaptureQuietExport, passiveRecenterGate)
            );
        }
        ...
        uint256 sharedRebate = wmul(wmul(calmMemory, spreadQuietExport), 110 * BPS);
    ...
    function getName() external pure override returns (string memory) {
        return "BasisOwnedClassifierExports";
    }
