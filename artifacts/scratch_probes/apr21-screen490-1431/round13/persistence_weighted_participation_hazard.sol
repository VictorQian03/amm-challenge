...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 participationHazardWeight = clamp(
            2200 * BPS + wmul(_oneMinus(gapShort), 1800 * BPS),
            0,
            4200 * BPS
        );
        uint256 boundedParticipation = wmul(volumeParticipation, participationHazardWeight);
        ...
        uint256 persistentImpactFloor =
            impactObservation + wmul(wmul(impactObservation, _oneMinus(gapShort)), 7600 * BPS);
        uint256 persistentParticipationFloor =
            wmul(wmul(boundedParticipation, _oneMinus(gapShort)), 2600 * BPS);
        uint256 baselineHazardFloor = wmul(
            persistentImpactFloor + persistentParticipationFloor,
            9200 * BPS
        );
    ...
    function getName() external pure override returns (string memory) {
        return "PersistenceWeightedParticipationHazard";
    }
