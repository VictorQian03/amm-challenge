...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 serviceLeakageBand = _oneMinus(
            clamp(
                hazardMemory * 9 +
                    wmul(flowPressure, 4200 * BPS) +
                    wmul(divergenceMemory, 6800 * BPS) +
                    wmul(oneSidedFlow, 3400 * BPS),
                0,
                WAD
            )
        );
        uint256 serviceCapacity = 0;
        if (gap >= 4 && serviceLeakageBand > 0) {
            serviceCapacity = wmul(
                wmul(wmul(passiveRecaptureMemory, quietGate), serviceLeakageBand),
                760 * BPS
            );
            uint256 serviceCapacityCap = 7 * BPS +
                wmul(sharedSpread > BASE_FEE ? sharedSpread - BASE_FEE : 0, 500 * BPS);
            if (serviceCapacity > serviceCapacityCap) {
                serviceCapacity = serviceCapacityCap;
            }
        }
        uint256 serviceWindowCut =
            passiveRecaptureCut + calmDivergenceBonus + healingRebate + inventoryCenteringOffset + refillAuctionCut;
        if (serviceWindowCut > serviceCapacity) {
            serviceWindowCut = serviceCapacity;
        }
        if (toxicBidSide) {
            askOpportunityCut += serviceWindowCut;
        } else {
            bidOpportunityCut += serviceWindowCut;
        }
