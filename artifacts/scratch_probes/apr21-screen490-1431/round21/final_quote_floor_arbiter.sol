...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 floorRisk = clamp(
            wmul(sideHazard, 520 * BPS) +
                wmul(flowPressure, 260 * BPS) +
                wmul(divergenceMemory, 300 * BPS),
            0,
            8 * BPS
        );
        uint256 floorQuote = BASE_FEE + floorRisk;
        uint256 floorLiftLimit = 5 * BPS + wmul(baselineFlex, 1800 * BPS);
        if (toxicBidSide) {
            if (bidFee < floorQuote) {
                uint256 liftedBid = bidFee + floorLiftLimit;
                bidFee = floorQuote < liftedBid ? floorQuote : liftedBid;
            }
        } else if (askFee < floorQuote) {
            uint256 liftedAsk = askFee + floorLiftLimit;
            askFee = floorQuote < liftedAsk ? floorQuote : liftedAsk;
        }
