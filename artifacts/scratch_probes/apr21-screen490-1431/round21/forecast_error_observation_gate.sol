...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 forecastError = priorLatentSpot == 0
            ? 0
            : wdiv(absDiff(currentSpot, priorLatentSpot), priorLatentSpot);
        uint256 forecastErrorClass = 0;
        if (forecastError > 14 * BPS) {
            forecastErrorClass = 2;
        } else if (forecastError > 7 * BPS) {
            forecastErrorClass = 1;
        }
        uint256 latentTrustCut = forecastErrorClass == 2
            ? 2200 * BPS
            : forecastErrorClass == 1
                ? 1000 * BPS
                : 0;
        uint256 latentSpot = _blend(
            priorLatentSpot,
            currentSpot,
            wmul(ALPHA_SPOT, _oneMinus(latentTrustCut))
        );
        ...
        if (forecastErrorClass > 0) {
            uint256 observationTrustCut = clamp(
                wmul(forecastError, 4200 * BPS) +
                    (forecastErrorClass == 2 ? 900 * BPS : 400 * BPS),
                0,
                2600 * BPS
            );
            calmObservation = wmul(calmObservation, _oneMinus(observationTrustCut));
            volObservation = wmul(
                volObservation,
                _oneMinus(clamp(wmul(observationTrustCut, 1800 * BPS), 0, 450 * BPS))
            );
            hazardObservation = wmul(
                hazardObservation,
                _oneMinus(clamp(wmul(observationTrustCut, 1200 * BPS), 0, 300 * BPS))
            );
        }
