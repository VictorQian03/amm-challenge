...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 volumeBucket = 0;
        if (tradeSize > 18 * BPS) {
            volumeBucket = 5200 * BPS + wmul(tradeSize - 18 * BPS, tradeSize);
        } else if (tradeSize > 7 * BPS) {
            volumeBucket = 2400 * BPS + wmul(tradeSize - 7 * BPS, tradeSize);
        } else if (tradeSize > 2 * BPS) {
            volumeBucket = 850 * BPS;
        }
        volumeBucket = clamp(volumeBucket, 0, WAD);

        uint256 imbalanceBucket = 0;
        if (flowImbalance > 7600 * BPS) {
            imbalanceBucket = 6800 * BPS + wmul(flowImbalance - 7600 * BPS, flowImbalance);
        } else if (flowImbalance > 5200 * BPS) {
            imbalanceBucket = 2800 * BPS + wmul(flowImbalance - 5200 * BPS, flowImbalance);
        } else if (flowImbalance > 3000 * BPS) {
            imbalanceBucket = 900 * BPS;
        }
        imbalanceBucket = clamp(imbalanceBucket, 0, WAD);

        uint256 timeBucket = 0;
        if (gap == 0) {
            timeBucket = WAD;
        } else if (gap <= 2) {
            timeBucket = 6400 * BPS;
        ...
        uint256 immediateShape = wmul(wmul(volumeBucket, imbalanceBucket), timeBucket);
        uint256 delayedBulkShape = 0;
        if (gap >= 2 && gap <= 6 && tradeSize > 10 * BPS) {
            delayedBulkShape = wmul(
                wmul(volumeBucket, _max(flowPressure, imbalanceBucket)),
                4200 * BPS
            );
        }
        uint256 pinprickShape = 0;
        if (tradeSize <= 6 * BPS && flowImbalance > 6600 * BPS && gap <= 1) {
            pinprickShape = wmul(wmul(flowImbalance, flowImbalance), 900 * BPS);
        }
        uint256 volumeImbalanceLattice = _max(immediateShape, _max(delayedBulkShape, pinprickShape));
        uint256 benignCaptureCap = WAD;
        if (calmMemory > 3500 * BPS && divergenceMemory < 900 * BPS && spotJump < 7 * BPS) {
            benignCaptureCap = gap >= 3 ? 2200 * BPS : 3600 * BPS;
        } else if (calmMemory > 1800 * BPS && divergenceMemory < 1300 * BPS && gap >= 2) {
            benignCaptureCap = 5200 * BPS;
        }
        volumeImbalanceLattice = clamp(volumeImbalanceLattice, 0, benignCaptureCap);
        uint256 latticeHazardClassifier = wmul(volumeImbalanceLattice, 1550 * BPS);
        if (latticeHazardClassifier > 360 * BPS) {
            latticeHazardClassifier = 360 * BPS;
        }
        ...
        sideHazard += latticeHazardClassifier;
        if (sideHazard > WAD) {
            sideHazard = WAD;
        }
        ...
        uint256 latticeProtectionFloor = wmul(latticeHazardClassifier, 6400 * BPS);
        if (latticeProtectionFloor > 0) {
            if (toxicBidSide) {
                if (askProtection < latticeProtectionFloor) {
                    askProtection = latticeProtectionFloor;
                }
            } else if (bidProtection < latticeProtectionFloor) {
                bidProtection = latticeProtectionFloor;
            }
        }
