...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 retailFloorClass = 0;
        if (hazardMemory > 0 && flowPressure > 0 && oneSidedFlow > 0) {
            retailFloorClass = 1;
        }
        if (hazardMemory > 35 * BPS && flowPressure > 80 * BPS && oneSidedFlow > 2 * BPS) {
            retailFloorClass = 2;
        }
        if (hazardMemory > 70 * BPS && flowPressure > 140 * BPS && oneSidedFlow > 4 * BPS && gap <= 2) {
            retailFloorClass = 3;
        }
        uint256 retailFloorHazard = 0;
        if (retailFloorClass == 1) {
            retailFloorHazard = 16 * BPS;
        } else if (retailFloorClass == 2) {
            retailFloorHazard = 30 * BPS;
        } else if (retailFloorClass == 3) {
            retailFloorHazard = 45 * BPS;
        }
        if (retailFloorHazard > 0) {
            uint256 floorProtectionSignal = retailFloorHazard;
            if (toxicBidSide) {
                bidRiskSignal = clamp(bidRiskSignal + floorProtectionSignal, 0, WAD);
            } else {
                askRiskSignal = clamp(askRiskSignal + floorProtectionSignal, 0, WAD);
            }
        }
