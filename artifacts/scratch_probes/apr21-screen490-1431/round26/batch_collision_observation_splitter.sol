...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 priceMovementObservation = _max(spotJumpVol, divergenceVol);
        uint256 volObservation = _max(tradeSize, priceMovementObservation);
        uint256 shortGapWeight = _oneMinus(gapShort);
        uint256 priceClusterObservation = wmul(priceMovementObservation, shortGapWeight);
        uint256 sizeClusterObservation = wmul(tradeSize, shortGapWeight);
        uint256 sizeOnlyCollision = 0;
        if (
            gap <= 2 &&
            tradeSize > 9 * BPS &&
            priceMovementObservation < wmul(tradeSize, 3800 * BPS)
        ) {
            uint256 collisionCap = tradeSize > 24 * BPS ? 260 * BPS : 170 * BPS;
            sizeOnlyCollision = wmul(sizeClusterObservation, 1700 * BPS);
            if (sizeOnlyCollision > collisionCap) {
                sizeOnlyCollision = collisionCap;
            }
        }
        uint256 priceInformedMovement = wmul(priceClusterObservation, 7200 * BPS);
        uint256 collisionClassification = priceInformedMovement + sizeOnlyCollision;
        if (collisionClassification > 520 * BPS) {
            collisionClassification = 520 * BPS;
        }
        uint256 hazardObservation = _max(divergenceVol, volObservation + collisionClassification);
    ...
    function getName() external pure override returns (string memory) {
        return "BatchCollisionObservationSplitter";
    }
