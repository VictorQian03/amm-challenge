...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 depthCollision = absDiff(sizeX, sizeY);
        ...
        uint256 explainedMove = _max(spotJumpVol, divergenceVol);
        uint256 residualCollision = tradeSize > explainedMove ? tradeSize - explainedMove : 0;
        uint256 collisionEvidence = clamp(
            residualCollision +
                wmul(depthCollision, 2600 * BPS) +
                wmul(clusterObservation, 1800 * BPS),
            0,
            WAD
        );
        uint256 residualEvidenceGate = clamp(
            wmul(hazardObservation, 1500 * BPS) +
                wmul(flowImbalance, 1800 * BPS) +
                wmul(_max(divergenceMemory, spotJump), 1100 * BPS),
            0,
            WAD
        );
        uint256 residualSpreadAdjunct = wmul(
            wmul(collisionEvidence, residualEvidenceGate),
            680 * BPS
        );
        if (residualSpreadAdjunct > 16 * BPS) {
            residualSpreadAdjunct = 16 * BPS;
        }
        ...
        sharedSpread += residualSpreadAdjunct;
    ...
    function getName() external pure override returns (string memory) {
        return "CurvatureResidualSpreadAdjunct";
    }
