...
/// @title Prior Trade Markout Ledger
...
contract Strategy is AMMStrategyBase {
    ...
    uint256 internal constant ALPHA_MARKOUT = 34 * BPS;
    uint256 internal constant DECAY_MARKOUT = 8600 * BPS;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // prior trade side: 1 buy, 2 sell
        slots[12] = spot; // prior trade spot
        slots[13] = 0; // bounded prior-trade adverse markout score
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 priorMarkoutScore = _priorTradeMarkoutScore(
            slots[11],
            slots[12],
            slots[13],
            currentSpot,
        ...
        if (priorMarkoutScore > hazardObservation) {
            hazardObservation = priorMarkoutScore;
        }
        ...
        slots[11] = trade.isBuy ? 1 : 2;
        slots[12] = currentSpot;
        slots[13] = priorMarkoutScore;

        return (bidFee, askFee);
    }

    function getName() external pure override returns (string memory) {
        return "PriorTradeMarkoutLedger";
    }
    ...
    function _priorTradeMarkoutScore(
        uint256 priorSide,
        uint256 priorSpot,
        uint256 priorScore,
        uint256 currentSpot,
        ...
        uint256 decayedScore = wmul(
            priorScore,
            _gapAdjustedDecay(DECAY_MARKOUT, gapShort, 1800 * BPS)
        );
        if (priorSide == 0 || priorSpot == 0) {
            return decayedScore;
        }

        uint256 markout = 0;
        if (priorSide == 1 && currentSpot > priorSpot) {
            markout = wdiv(currentSpot - priorSpot, priorSpot);
        } else if (priorSide == 2 && currentSpot < priorSpot) {
            markout = wdiv(priorSpot - currentSpot, priorSpot);
        }
        if (markout == 0) {
            return decayedScore;
        }

        uint256 boundedObservation = clamp(
            markout + wmul(divergenceMemory, 1800 * BPS),
            0,
            2600 * BPS
        );
        return _blend(decayedScore, boundedObservation, ALPHA_MARKOUT);
    }
}
