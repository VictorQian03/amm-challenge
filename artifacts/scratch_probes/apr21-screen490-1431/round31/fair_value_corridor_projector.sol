...
/// @title Fair Value Corridor Projector
...
contract Strategy is AMMStrategyBase {
    ...
    uint256 internal constant LATENT_DEADBAND = 3 * BPS;
    uint256 internal constant LATENT_CORRIDOR = 20 * BPS;
    uint256 internal constant LATENT_NOISE_ALPHA = 4 * BPS;
    uint256 internal constant LATENT_CORRIDOR_ALPHA = 10 * BPS;
    uint256 internal constant LATENT_EXCESS_ALPHA = 24 * BPS;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 latentSpot = _projectLatentSpot(slots[0], currentSpot);
    ...
    function getName() external pure override returns (string memory) {
        return "FairValueCorridorProjector";
    }
    ...
    function _projectLatentSpot(uint256 latentSpot, uint256 observedSpot)
        internal
        pure
        returns (uint256)
    {
        ...
        uint256 distance = absDiff(observedSpot, latentSpot);
        uint256 distanceRatio = wdiv(distance, latentSpot);
        uint256 alpha = LATENT_CORRIDOR_ALPHA;

        if (distanceRatio <= LATENT_DEADBAND) {
            alpha = LATENT_NOISE_ALPHA;
        } else if (distanceRatio > LATENT_CORRIDOR) {
            uint256 excess = distanceRatio - LATENT_CORRIDOR;
            alpha = clamp(
                LATENT_CORRIDOR_ALPHA + wmul(excess, LATENT_EXCESS_ALPHA),
                LATENT_CORRIDOR_ALPHA,
                36 * BPS
            );
        }

        return _blend(latentSpot, observedSpot, alpha);
    }
