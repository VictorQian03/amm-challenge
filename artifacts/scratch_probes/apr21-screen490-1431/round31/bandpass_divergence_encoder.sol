...
/// @title Bandpass Divergence Encoder
...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 bandpassDivergence = _bandpassDivergence(divergence, divergenceMemory);
        if (bandpassDivergence > 0) {
            divergenceVol = clamp(
                divergenceVol + wmul(bandpassDivergence, 6200 * BPS),
                0,
                WAD
            );
        }
        ...
        if (bandpassDivergence > 0) {
            hazardObservation = clamp(
                hazardObservation + wmul(bandpassDivergence, 1800 * BPS),
                0,
                WAD
            );
        }
        ...
        divergenceMemory = _blend(
            divergenceMemory,
            _max(divergence, bandpassDivergence),
            ALPHA_DIVERGENCE
        );
    ...
    function getName() external pure override returns (string memory) {
        return "BandpassDivergenceEncoder";
    }
    ...
    function _bandpassDivergence(uint256 divergence, uint256 priorMemory) internal pure returns (uint256) {
        uint256 low = 2 * BPS;
        uint256 peak = 11 * BPS;
        uint256 high = 34 * BPS;
        if (divergence <= low || divergence >= high) {
            return 0;
        }

        uint256 triangle = divergence <= peak
            ? wdiv(divergence - low, peak - low)
            : wdiv(high - divergence, high - peak);
        uint256 memoryLimiter = _oneMinus(clamp(wmul(priorMemory, 5200 * BPS), 0, 5200 * BPS));
        uint256 encodedShape = wmul(triangle, memoryLimiter);
        return wmul(divergence + 4 * BPS, encodedShape);
    }
}
