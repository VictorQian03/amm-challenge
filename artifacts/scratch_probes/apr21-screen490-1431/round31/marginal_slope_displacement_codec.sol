...
/// @title Marginal Slope Displacement Codec
...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 volMemoryObservation = volObservation;
        uint256 hazardMemoryObservation = hazardObservation;
        uint256 divergenceMemoryObservation = divergence;
        (
            uint256 slopeVolObservation,
            uint256 slopeHazardObservation,
            uint256 slopeDivergenceObservation
        ) = _marginalSlopeDisplacementCodec(
            volObservation,
            hazardObservation,
            divergenceMemory
        );
        volMemoryObservation = slopeVolObservation;
        hazardMemoryObservation = slopeHazardObservation;
        if (slopeDivergenceObservation > divergenceMemoryObservation) {
            divergenceMemoryObservation = slopeDivergenceObservation;
        }

        volMemory = _blend(volMemory, volMemoryObservation, ALPHA_VOL);
        hazardMemory = _blend(hazardMemory, hazardMemoryObservation, ALPHA_HAZARD);
        calmMemory = _blend(calmMemory, calmObservation, ALPHA_CALM);
        divergenceMemory = _blend(divergenceMemory, divergenceMemoryObservation, ALPHA_DIVERGENCE);
    ...
    function getName() external pure override returns (string memory) {
        return "MarginalSlopeDisplacementCodec";
    }
    ...
    function _marginalSlopeDisplacementCodec(
        uint256 volObservation,
        ...
        pure
        returns (
            uint256 volMemoryObservation,
            uint256 hazardMemoryObservation,
            uint256 divergenceMemoryObservation
        )
    {
        volMemoryObservation = volObservation;
        hazardMemoryObservation = hazardObservation;
        divergenceMemoryObservation = divergenceMemory;

        uint256 slopeResidual = absDiff(hazardObservation, volObservation);
        if (slopeResidual <= 2 * BPS && divergenceMemory <= 2 * BPS) {
            return (
                volMemoryObservation,
                hazardMemoryObservation,
                divergenceMemoryObservation
            );
        }

        uint256 magnitudeBand = slopeResidual;
        if (magnitudeBand > 4 * BPS) {
            magnitudeBand += wmul(magnitudeBand - 4 * BPS, 5200 * BPS);
        }
        if (magnitudeBand > 12 * BPS) {
            magnitudeBand += wmul(magnitudeBand - 12 * BPS, 7200 * BPS);
        }

        uint256 persistentDisplacement = _max(slopeResidual, divergenceMemory);
        uint256 boundedDisplacement = clamp(
            wmul(magnitudeBand, 3600 * BPS) +
                wmul(persistentDisplacement, 1800 * BPS),
            0,
            22 * BPS
        );

        if (hazardObservation >= volObservation) {
            hazardMemoryObservation = clamp(
                hazardObservation + boundedDisplacement,
                0,
                WAD
            );
            uint256 displacementEcho = wmul(boundedDisplacement, 5200 * BPS);
            divergenceMemoryObservation = clamp(
                divergenceMemory + displacementEcho,
                0,
                WAD
            );
        } else {
            uint256 reliefScale = _oneMinus(wmul(boundedDisplacement, 6800 * BPS));
            hazardMemoryObservation = wmul(hazardObservation, reliefScale);
            divergenceMemoryObservation = wmul(divergenceMemory, reliefScale);
            volMemoryObservation = clamp(
                volObservation + wmul(boundedDisplacement, 3600 * BPS),
                0,
                WAD
            );
        }
