...
/// @title Boundary Normalized State Loop
/// @notice Normalize raw gap, size, and jump signals at the boundary so the latent loop sees curved observations first.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 gapShort = _curve(_gapRatio(gap, 3), 6400 * BPS);
        uint256 gapLong = _curve(_gapRatio(gap, 8), 5900 * BPS);
        uint256 gapBoundary = _curve(_gapRatio(gap, 6), 6800 * BPS);
        uint256 sizeBoundary = _curve(_normalizeWad(tradeSize, 2400 * BPS), 7000 * BPS);
        uint256 jumpBoundary = _curve(_normalizeWad(_max(spotJump, divergence), 1600 * BPS), 6600 * BPS);
        uint256 boundaryPressure = _max(sizeBoundary, jumpBoundary);
        uint256 boundaryCoupling = wmul(sizeBoundary, jumpBoundary);

        uint256 volMemory = wmul(
            slots[1],
            _gapAdjustedDecay(DECAY_VOL, gapShort, 1500 * BPS)
        );
        uint256 buyFlow = wmul(
            slots[2],
            _gapAdjustedDecay(DECAY_FLOW, gapShort, 2100 * BPS)
        );
        ...
        uint256 hazardMemory = wmul(
            slots[4],
            _gapAdjustedDecay(DECAY_HAZARD, gapShort, 1600 * BPS)
        );
        uint256 calmMemory = wmul(
            slots[5],
            _gapAdjustedDecay(DECAY_CALM, gapLong, 800 * BPS)
        );
        uint256 divergenceMemory = wmul(
            slots[8],
            _gapAdjustedDecay(DECAY_DIVERGENCE, gapShort, 1300 * BPS)
        );

        uint256 normalizedTradeSize = tradeSize + wmul(tradeSize, sizeBoundary);
        uint256 normalizedSpotJump = spotJump + wmul(spotJump, jumpBoundary);
        uint256 normalizedDivergence = divergence + wmul(divergence, jumpBoundary);
        uint256 volObservation = _max(
            normalizedTradeSize,
            _max(normalizedSpotJump, normalizedDivergence)
        );
        uint256 clusterObservation = wmul(volObservation, _oneMinus(gapBoundary));
        uint256 hazardObservation = _max(
            normalizedDivergence,
            volObservation +
                wmul(clusterObservation, 5200 * BPS + wmul(boundaryCoupling, 1400 * BPS))
        );
        uint256 calmObservation = wmul(
            _oneMinus(gapBoundary),
            _oneMinus(clamp(hazardObservation * 5, 0, WAD))
        );
        ...
        uint256 flowPulse =
            normalizedTradeSize +
            wmul(volObservation, 3200 * BPS) +
            wmul(boundaryPressure, 900 * BPS);
        uint256 crossPulse = wmul(flowPulse, boundaryCoupling > 1500 * BPS ? 1400 * BPS : 1800 * BPS);
        if (trade.isBuy) {
        ...
        if (continuationAligned) {
            continuationVeto = clamp(
                wmul(flowPressure, 2600 * BPS) +
                    wmul(oneSidedFlow, 2100 * BPS) +
                    wmul(boundaryPressure, 1700 * BPS) +
                    wmul(normalizedSpotJump, 1200 * BPS),
                0,
                4500 * BPS
            );
        }
        ...
        if (!continuationAligned && gap >= 4) {
            counterflowRelease = clamp(
                wmul(calmMemory, 1100 * BPS) +
                    wmul(gapLong, 700 * BPS),
                0,
                1800 * BPS
            );
        }
        ...
        uint256 quietGate = _oneMinus(
            clamp(
                wmul(volMemory, 2200 * BPS) +
                    ...
                    wmul(normalizedSpotJump, 1600 * BPS),
                0,
                WAD
            )
        );
        if (gap >= 3) {
            uint256 quietRecenter =
                wmul(quietGate, gap >= 5 ? 1000 * BPS : gap >= 4 ? 720 * BPS : 520 * BPS);
        ...
        if (gap >= 4) {
            ...
            divergenceMemory = wmul(
                divergenceMemory,
                _oneMinus(clamp(wmul(quietRelease, 1100 * BPS), 0, WAD))
            );
        }
        ...
        uint256 sideHazard =
            hazardMemory +
            wmul(flowImbalance, _max(volMemory, _max(divergenceMemory, boundaryPressure)));
        if (sideHazard > WAD) {
            sideHazard = WAD;
        }

        uint256 flowDirectionalRisk = 0;
        if (flowPressure > 500 * BPS && boundaryPressure > 3000 * BPS) {
            uint256 toxicFlowSignal =
                flowPressure +
                wmul(boundaryPressure, 2400 * BPS) +
                wmul(normalizedSpotJump, 1400 * BPS);
            flowDirectionalRisk = wmul(toxicFlowSignal, 300 * BPS);
        }
        ...
        uint256 passiveRecaptureObservation = wmul(
            ...
            _oneMinus(
                clamp(
                    wmul(flowPressure, 2600 * BPS) +
                        wmul(divergence, 4200 * BPS) +
                        wmul(boundaryPressure, 1000 * BPS),
                    0,
                    WAD
                )
            )
            ...
            _oneMinus(
                clamp(
                    hazardMemory * 8 +
                        ...
                        wmul(boundaryPressure, 1800 * BPS),
                    0,
                    WAD
                )
            )
        ...
        uint256 sharedSpread =
            BASE_FEE +
            wmul(volMemory, 1700 * BPS) +
            wmul(hazardMemory, 1800 * BPS) +
            wmul(boundaryPressure, 900 * BPS);

        uint256 boundaryEvent = _max(volObservation, hazardObservation) + wmul(boundaryPressure, 1600 * BPS);
        if (boundaryEvent > WAD) {
            boundaryEvent = WAD;
        }
        uint256 eventCarry = wmul(boundaryEvent, 180 * BPS);
        uint256 boundaryBurstFee = 0;
        if (boundaryEvent > 8 * BPS) {
            uint256 burstSignal = boundaryEvent - 8 * BPS;
            eventCarry += wmul(burstSignal, 240 * BPS);
            boundaryBurstFee = wmul(burstSignal, 1700 * BPS);
        }
        ...
        uint256 healingRebate = wmul(boundaryBurstFee, 2600 * BPS);
            ...
            _oneMinus(
                clamp(
                    hazardMemory * 8 +
                        ...
                        wmul(boundaryPressure, 1500 * BPS),
                    0,
                    WAD
                )
            )
        ...
        uint256 inventoryCenteringOffset = wmul(
            wmul(_max(passiveRecaptureMemory, calmMemory), inventoryGate),
            650 * BPS
        );
        ...
        if (currentSpot >= latentSpot) {
            bidProtection += boundaryBurstFee + oneSidedProtection + inventoryCenteringOffset;
        } else {
            askProtection += boundaryBurstFee + oneSidedProtection + inventoryCenteringOffset;
        }

        uint256 bidOpportunityCut = wmul(bidOpportunitySignal, 8200 * BPS);
        uint256 askOpportunityCut = wmul(askOpportunitySignal, 8200 * BPS);
        uint256 passiveRecaptureCut = wmul(passiveRecaptureMemory, 1450 * BPS);
        ...
        uint256 refillRiskGate = _oneMinus(
            clamp(
                hazardMemory * 8 +
                    ...
                    wmul(divergenceMemory, 5000 * BPS) +
                    wmul(boundaryPressure, 1200 * BPS),
                0,
                WAD
            )
        );
        ...
        if (toxicBidSide) {
            ...
            bidOpportunityCut += passiveRecaptureCut + calmDivergenceBonus + healingRebate + inventoryCenteringOffset;
            bidOpportunityCut += refillAuctionCut;
        }
    ...
    function getName() external pure override returns (string memory) {
        return "BoundaryNormalizedStateLoop";
    }
    ...
    function _normalizeWad(uint256 raw, uint256 pivot) internal pure returns (uint256) {
        if (raw == 0) {
            return 0;
        }
        return wdiv(raw, raw + pivot);
    }

    function _curve(uint256 ratio, uint256 curveBias) internal pure returns (uint256) {
        uint256 curved = wmul(ratio, ratio);
        return wmul(ratio, curveBias) + wmul(curved, WAD - curveBias);
    }
