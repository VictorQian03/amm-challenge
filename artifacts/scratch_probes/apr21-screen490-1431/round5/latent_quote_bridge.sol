...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 recenterConfidence = 0;
        if (gap >= 3) {
            uint256 quietRecenter =
                wmul(quietGate, gap >= 5 ? 700 * BPS : gap >= 4 ? 520 * BPS : 380 * BPS);
            if (continuationVeto > 0) {
                quietRecenter = wmul(quietRecenter, _oneMinus(continuationVeto));
            } else if (counterflowRelease > 0) {
                quietRecenter = clamp(
                    quietRecenter + wmul(quietRecenter, wmul(counterflowRelease, 7000 * BPS)),
                    0,
                    WAD
                );
            }
            uint256 ratchetSpot = _blend(latentSpot, currentSpot, quietRecenter);
            uint256 ratchetDivergence =
                ratchetSpot == 0 ? 0 : wdiv(absDiff(currentSpot, ratchetSpot), ratchetSpot);
            recenterConfidence = _oneMinus(
                clamp(
                    wmul(ratchetDivergence, 10000 * BPS) +
                        wmul(flowPressure, 1400 * BPS) +
                        wmul(oneSidedFlow, 1200 * BPS),
                    0,
                    WAD
                )
            );
            uint256 quietFollowThrough = wmul(
                wmul(quietGate, recenterConfidence),
                gap >= 5 ? 320 * BPS : gap >= 4 ? 220 * BPS : 120 * BPS
            );
            latentSpot = _blend(ratchetSpot, currentSpot, quietFollowThrough);
        }
        if (gap >= 4) {
            ...
            uint256 quietRelease = wmul(
                wmul(wmul(quietGate, reconvergenceGate), benignGapGate),
                recenterConfidence
            );
        ...
        uint256 widthFlex = sharedSpread > BASE_FEE ? sharedSpread - BASE_FEE : 0;
        ...
        if (recenterConfidence > 0) {
            inventoryGate = wmul(
                inventoryGate,
                clamp(recenterConfidence + 2200 * BPS, 0, WAD)
            );
        }
        ...
        // Narrow bridge: only reopen the safe side when recentering is already
        // trusted and the calm/counterflow confirmation is strong enough.
        uint256 benignQuoteGate = wmul(quietGate, recenterConfidence);
        uint256 reopenCalmConfirmation = passiveRecaptureMemory;
        if (gap >= 4) {
            reopenCalmConfirmation = clamp(
                reopenCalmConfirmation +
                    wmul(calmMemory, 1900 * BPS) +
                    wmul(counterflowRelease, 3600 * BPS),
                0,
                WAD
            );
        } else if (gap >= 2) {
            reopenCalmConfirmation = wmul(
                clamp(
                    reopenCalmConfirmation + wmul(calmMemory, 1100 * BPS),
                    0,
                    WAD
                ),
                7200 * BPS
            ...
            reopenCalmConfirmation = 0;
        }
        reopenCalmConfirmation = wmul(
            reopenCalmConfirmation,
            _oneMinus(clamp(wmul(flowPressure, 1800 * BPS), 0, WAD))
        );
        uint256 reopenQuoteGate = wmul(benignQuoteGate, reopenCalmConfirmation);
        if (counterflowRelease > 0) {
            reopenQuoteGate = clamp(
                reopenQuoteGate + wmul(reopenQuoteGate, wmul(counterflowRelease, 4200 * BPS)),
                0,
                WAD
            );
        } else {
            reopenQuoteGate = wmul(reopenQuoteGate, 6200 * BPS);
        }
        reopenQuoteGate = wmul(
            reopenQuoteGate,
            _oneMinus(clamp(wmul(hazardMemory, 1200 * BPS), 0, WAD))
        );
        uint256 safeSideReopenCut =
            wmul(
                wmul(reopenQuoteGate, counterflowRelease > 0 ? counterflowRelease : calmMemory),
                gap >= 4 ? 340 * BPS : 180 * BPS
            ) +
            wmul(
                wmul(reopenQuoteGate, passiveRecaptureMemory),
                gap >= 4 ? 120 * BPS : 60 * BPS
            );
        uint256 reopenCap = wmul(widthFlex, 1200 * BPS);
        if (safeSideReopenCut > reopenCap) {
            safeSideReopenCut = reopenCap;
        }
        ...
        if (hazardMemory < 1100 * BPS && flowPressure < 650 * BPS && gap >= 2) {
            ...
            calmDivergenceBonus = wmul(
                calmDivergenceBonus,
                recenterConfidence > 0 ? recenterConfidence : quietGate
            );
        }
        ...
        if (toxicBidSide) {
            ...
            askOpportunityCut += safeSideReopenCut;
            ...
            bidOpportunityCut += safeSideReopenCut;
