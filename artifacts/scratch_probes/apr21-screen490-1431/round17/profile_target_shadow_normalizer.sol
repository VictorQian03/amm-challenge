...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 rawStress = _max(spotJumpVol, divergenceVol);
        uint256 stressDiscount = clamp(
            wmul(_oneMinus(gapShort), 700 * BPS) +
                wmul(tradeSize, 1000 * BPS),
            0,
            1200 * BPS
        );
        uint256 normalizedStress = wmul(rawStress, _oneMinus(stressDiscount));
        uint256 demandWeight = WAD + wmul(gapLong, 500 * BPS);
        uint256 normalizedDemand = wmul(tradeSize, demandWeight);
        uint256 normalizedDivergence = wmul(divergence, _oneMinus(wmul(stressDiscount, 5000 * BPS)));
        uint256 normalizedDivergenceVol = wmul(divergenceVol, _oneMinus(wmul(stressDiscount, 5000 * BPS)));
        uint256 volObservation = _max(normalizedDemand, normalizedStress);
        uint256 clusterObservation = wmul(normalizedStress, _oneMinus(gapShort));
        uint256 hazardObservation = _max(
            normalizedDivergenceVol,
            normalizedStress + wmul(clusterObservation, 7000 * BPS)
        );
        ...
        divergenceMemory = _blend(divergenceMemory, normalizedDivergence, ALPHA_DIVERGENCE);
