        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        if (tradeSize > 4 * BPS) {
            liquidityDemand = clamp(
                liquidityDemand + wmul(tradeSize - 4 * BPS, 3500 * BPS),
                0,
                WAD
            );
        }
        ...
        if (liquidityDemand > informationStress) {
            informationStress = clamp(
                informationStress + wmul(liquidityDemand - informationStress, 1800 * BPS),
                0,
                WAD
            );
        }
        ...
        uint256 hazardObservation = _max(
            divergenceVol,
            informationStress + wmul(clusterObservation, 7000 * BPS)
        );
