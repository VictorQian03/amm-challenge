...
/// @title Benign Fill Exclusion Proof
/// @notice Tests a classifier-only exclusion: benign fills may suppress excess protection only without adverse evidence.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        bool adverseEvidencePresent =
            hazardObservation > 360 * BPS ||
            divergenceMemory > 420 * BPS ||
            spotJump > 240 * BPS ||
            flowPressure > 850 * BPS ||
            flowImbalance > 1500 * BPS;
        bool benignFillEvidence =
            !adverseEvidencePresent &&
            gap >= 2 &&
            tradeSize > 180 * BPS &&
            calmMemory > 1600 * BPS;
        ...
        if (benignFillEvidence) {
            uint256 exclusionLimit = wmul(sideHazard, 2600 * BPS);
            sideHazard -= exclusionLimit;
        }
        if (sideHazard > WAD) {
            sideHazard = WAD;
        }
    ...
    function getName() external pure override returns (string memory) {
        return "BenignFillExclusionProof";
    }
