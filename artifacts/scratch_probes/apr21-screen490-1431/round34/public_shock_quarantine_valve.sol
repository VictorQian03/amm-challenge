...
/// @title Public Shock Quarantine Valve
/// @notice Tests whether public-signal synchrony should only quarantine risk, never release fees.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 publicShockCertificate = 0;
        uint256 publicShockBase = _max(spotJumpVol, divergenceVol);
        if (gap <= 2 && publicShockBase > 7 * BPS && tradeSize > 2 * BPS) {
            uint256 synchronyFloor = publicShockBase > tradeSize ? tradeSize : publicShockBase;
            publicShockCertificate = _max(
                wmul(synchronyFloor, 7200 * BPS),
                wmul(clusterObservation, 6100 * BPS)
            );
            if (spotJumpVol > 12 * BPS && divergenceVol > 5 * BPS) {
                publicShockCertificate = clamp(
                    publicShockCertificate + wmul(publicShockBase, 1700 * BPS),
                    0,
                    WAD
                );
            }
        }
        if (monotoneEvidenceFloor < publicShockCertificate) {
            monotoneEvidenceFloor = publicShockCertificate;
        }
        ...
        sideHazard += wmul(publicShockCertificate, 290 * BPS);
        if (sideHazard > WAD) {
            sideHazard = WAD;
        }
    ...
    function getName() external pure override returns (string memory) {
        return "PublicShockQuarantineValve";
    }
