...
/// @title Hysteretic Floor Latch
/// @notice Tests a layer-3 risk latch: strong floor evidence enters, two benign confirmations exit.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // floor-risk latch
        slots[12] = 0; // benign confirmation mask for latch release
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 rawFloorEvidence = _max(
            divergenceVol,
            wmul(clusterObservation, 6200 * BPS)
        );
        uint256 monotoneEvidenceFloor = rawFloorEvidence;
        if (slots[11] > 0 && monotoneEvidenceFloor < 260 * BPS) {
            monotoneEvidenceFloor = 260 * BPS;
        }
        ...
        uint256 floorLatch = slots[11];
        uint256 benignConfirmationMask = slots[12];
        bool strongFloorEvidence =
            rawFloorEvidence > 700 * BPS ||
            (rawFloorEvidence > 420 * BPS && flowPressure > 1200 * BPS);
        if (strongFloorEvidence) {
            floorLatch = WAD;
            benignConfirmationMask = 0;
        } else if (floorLatch > 0) {
            bool calmConfirmation =
                gap >= 4 &&
                calmMemory > 2100 * BPS &&
                hazardObservation < 360 * BPS;
            bool flowConfirmation =
                flowPressure < 700 * BPS &&
                flowImbalance < 1200 * BPS &&
                spotJump < 240 * BPS;
            if (calmConfirmation) {
                benignConfirmationMask |= 1;
            }
            if (flowConfirmation) {
                benignConfirmationMask |= 2;
            }
            if (benignConfirmationMask == 3) {
                floorLatch = 0;
                benignConfirmationMask = 0;
            }
        }
        if (floorLatch > 0 && monotoneEvidenceFloor < 360 * BPS) {
            monotoneEvidenceFloor = 360 * BPS;
        }
        ...
        if (floorLatch > 0) {
            sideHazard += wmul(_max(hazardMemory, monotoneEvidenceFloor), 420 * BPS);
        }
        if (sideHazard > WAD) {
            sideHazard = WAD;
        }
        ...
        slots[11] = floorLatch;
        slots[12] = benignConfirmationMask;

        return (bidFee, askFee);
    }

    function getName() external pure override returns (string memory) {
        return "HystereticFloorLatch";
    }
