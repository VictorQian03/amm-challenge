...
/// @title Protected Path Consumer Isolation
/// @notice Tests a private weak-feasibility anchor that can only raise protected width.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 weakFeasibilityAnchor = 0;
        if (weaklyInconsistentEvent) {
            weakFeasibilityAnchor = clamp(
                feasibilityResidual +
                    wmul(monotoneEvidenceFloor, 650 * BPS) +
                    wmul(tradeSize, 280 * BPS),
                0,
                13 * BPS
            );
        }
        {
            uint256 protectedWidthLift = wmul(weakFeasibilityAnchor, 1250 * BPS);
            if (protectedWidthLift > 0) {
                if (toxicBidSide) {
                    bidProtection = clamp(bidProtection + protectedWidthLift, 0, WAD);
                } else {
                    askProtection = clamp(askProtection + protectedWidthLift, 0, WAD);
                }
            }
        }
    ...
    function getName() external pure override returns (string memory) {
        return "ProtectedPathConsumerIsolation";
    }
