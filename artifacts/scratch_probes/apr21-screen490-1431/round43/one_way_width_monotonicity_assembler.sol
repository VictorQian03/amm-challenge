...
/// @title One Way Width Monotonicity Assembler
/// @notice Floors the protected path to its pre-feasibility layer-4 width only.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 preFeasibilityHazardObservation = hazardObservation;
        ...
        uint256 preFeasibilityHazardMemory = _blend(
            hazardMemory,
            preFeasibilityHazardObservation,
            ALPHA_HAZARD
        );
        ...
        uint256 preFeasibilitySideHazard =
            preFeasibilityHazardMemory + wmul(flowImbalance, _max(volMemory, divergenceMemory));
        preFeasibilitySideHazard += wmul(monotoneEvidenceFloor, 950 * BPS);
        if (preFeasibilitySideHazard > WAD) {
            preFeasibilitySideHazard = WAD;
        }
        ...
        uint256 askRiskSignal =
            ...
            askFlowRisk;
        uint256 preFeasibilityBidRiskSignal =
            wmul(sellShare, preFeasibilitySideHazard) +
            wmul(richSignal, 8500 * BPS) +
            bidFlowRisk;
        uint256 preFeasibilityAskRiskSignal =
            wmul(buyShare, preFeasibilitySideHazard) +
            wmul(cheapSignal, 8500 * BPS) +
            askFlowRisk;
        ...
        uint256 preFeasibilityBidProtection = wmul(preFeasibilityBidRiskSignal, 5400 * BPS);
        uint256 preFeasibilityAskProtection = wmul(preFeasibilityAskRiskSignal, 5400 * BPS);
        ...
        if (preFeasibilityBidProtection > tailProtectionCap) {
            preFeasibilityBidProtection = tailProtectionCap;
        }
        if (preFeasibilityAskProtection > tailProtectionCap) {
            preFeasibilityAskProtection = tailProtectionCap;
        }
        ...
        uint256 protectedWidthIncrement =
            directionalBurstFee + oneSidedProtection + inventoryCenteringOffset;
        if (currentSpot >= latentSpot) {
            uint256 preFeasibilityBidWidth =
                preFeasibilityBidProtection + protectedWidthIncrement;
            bidProtection += protectedWidthIncrement;
            bidProtection = _max(preFeasibilityBidWidth, bidProtection);
        } else {
            uint256 preFeasibilityAskWidth =
                preFeasibilityAskProtection + protectedWidthIncrement;
            askProtection += protectedWidthIncrement;
            askProtection = _max(preFeasibilityAskWidth, askProtection);
        }
    ...
    function getName() external pure override returns (string memory) {
        return "OneWayWidthMonotonicityAssembler";
    }
