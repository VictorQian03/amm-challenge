...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 floorPreserveGate = _oneMinus(
            clamp(
                wmul(tradeSize, 3600 * BPS) +
                    wmul(divergenceVol, 7600 * BPS),
                0,
                WAD
            )
        );
        uint256 toxicityClock = wmul(
            _oneMinus(gapShort),
            wmul(_max(tradeSize, spotJumpVol), _oneMinus(wmul(gapLong, floorPreserveGate)))
        );
        uint256 flowToxicity = wmul(flowPressure, toxicityClock);
        if (tradeSize > 4 * BPS && flowImbalance > 6000 * BPS) {
            flowToxicity = clamp(
                flowToxicity + wmul(tradeSize - 4 * BPS, 1700 * BPS),
                0,
                WAD
            );
        }
        ...
        sideHazard += wmul(flowToxicity, 3000 * BPS);
        if (sideHazard > WAD) {
            sideHazard = WAD;
        }
        ...
        if (flowPressure > 500 * BPS) {
            ...
            if (extensionSignal > 3 * BPS) {
                uint256 toxicFlowSignal =
                    flowPressure + wmul(extensionSignal, 2200 * BPS) + wmul(flowToxicity, 4200 * BPS);
        ...
        sharedSpread += wmul(flowToxicity, 480 * BPS);
