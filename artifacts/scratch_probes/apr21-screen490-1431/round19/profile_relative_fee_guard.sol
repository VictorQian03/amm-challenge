        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        if (liquidityDemand > informationStress) {
            informationStress = clamp(
                informationStress + wmul(liquidityDemand - informationStress, 3200 * BPS),
                0,
                WAD
            );
        }
        uint256 volObservation = _max(liquidityDemand, informationStress);
        uint256 clusterObservation = wmul(informationStress, _oneMinus(gapShort));
        ...
        uint256 profileFloorGuard = wmul(
            quietGate,
            _oneMinus(
                clamp(
                    hazardMemory * 7 +
        ...
        if (gap >= 3 && sharedSpread > BASE_FEE) {
            sharedRebate = clamp(
                sharedRebate +
                    wmul(
                        sharedSpread - BASE_FEE,
                        wmul(profileFloorGuard, gap >= 5 ? 1450 * BPS : 850 * BPS)
                    ),
                0,
