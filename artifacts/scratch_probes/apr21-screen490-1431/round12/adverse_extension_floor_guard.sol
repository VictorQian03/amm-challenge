...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 adverseExtensionFloorGuard = 0;
        uint256 adverseExtension = _max(divergenceMemory, spotJump);
        if (continuationAligned && adverseExtension > 3 * BPS) {
            adverseExtensionFloorGuard = clamp(
                wmul(_max(flowPressure, oneSidedFlow), 360 * BPS) +
                    wmul(adverseExtension - 3 * BPS, 1250 * BPS),
                0,
                650 * BPS
            );
        }
    ...
    function getName() external pure override returns (string memory) {
        return "AdverseExtensionFloorGuard";
    }
