...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 recenterQuietGate = quietGate;
        uint256 spreadQuietGate = quietGate;
        uint256 recaptureQuietGate = quietGate;
        if (gap >= 3) {
            recenterQuietGate = clamp(
                recenterQuietGate + wmul(wmul(calmMemory, gapLong), 240 * BPS),
                0,
                WAD
            );
        }
        uint256 exportToxicityFirewall = clamp(
            wmul(flowPressure, 1200 * BPS) +
                wmul(oneSidedFlow, 1500 * BPS) +
                wmul(divergenceMemory, 700 * BPS),
            0,
            2200 * BPS
        );
        recaptureQuietGate = wmul(recaptureQuietGate, _oneMinus(exportToxicityFirewall));
        spreadQuietGate = wmul(
            spreadQuietGate,
            _oneMinus(clamp(wmul(oneSidedFlow, 900 * BPS), 0, 1200 * BPS))
        );
        if (gap >= 3) {
            uint256 quietRecenter =
                wmul(recenterQuietGate, gap >= 5 ? 1050 * BPS : gap >= 4 ? 750 * BPS : 550 * BPS);
        ...
        if (gap >= 4) {
            ...
            uint256 quietRelease = wmul(wmul(recenterQuietGate, reconvergenceGate), benignGapGate);
        ...
        if (gap >= 4) {
            ...
            passiveRecaptureObservation = wmul(
                ...
                wmul(recaptureQuietGate, passiveRecenterGate)
            );
        }
        ...
        uint256 sharedRebate = wmul(wmul(calmMemory, spreadQuietGate), 110 * BPS);
    ...
    function getName() external pure override returns (string memory) {
        return "TypedClassifierExportFirewall";
    }
