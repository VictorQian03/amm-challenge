...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 toxicPersistence = wmul(eventSignal, _oneMinus(gapShort));
        toxicPersistence = clamp(
            toxicPersistence +
                wmul(oneSidedFlow, 1800 * BPS) +
                wmul(_max(divergenceMemory, spotJump), 900 * BPS),
            0,
            WAD
        );
        ...
        if (toxicPersistence > 5 * BPS) {
            uint256 persistenceExcess = toxicPersistence - 5 * BPS;
            eventCarry += wmul(persistenceExcess, 90 * BPS);
            directionalBurstFee += wmul(persistenceExcess, 420 * BPS);
        }
    ...
    function getName() external pure override returns (string memory) {
        return "ToxicPersistenceEventClassifier";
    }
