...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 baselineFlexForCuts = sharedSpread > BASE_FEE ? sharedSpread - BASE_FEE : 0;
        uint256 lvrProxy = _max(spotJumpVol, _max(divergenceVol, divergenceMemory));
        uint256 lvrFloorAdd = 0;
        if (lvrProxy > 4 * BPS) {
            lvrFloorAdd = wmul(lvrProxy - 4 * BPS, 520 * BPS);
        }
        lvrFloorAdd = clamp(
            lvrFloorAdd + wmul(wmul(spotJumpVol, divergenceMemory), 1600 * BPS),
            0,
            18 * BPS
        );
        uint256 lvrSharedFloor = BASE_FEE + lvrFloorAdd;
        if (sharedSpread < lvrSharedFloor) {
            sharedSpread = lvrSharedFloor;
        }
        ...
        bidOpportunityCut = clamp(
            bidOpportunityCut,
            0,
            bidProtection + wmul(baselineFlexForCuts, 1200 * BPS)
        );
        askOpportunityCut = clamp(
            askOpportunityCut,
            0,
            askProtection + wmul(baselineFlexForCuts, 1200 * BPS)
        );
