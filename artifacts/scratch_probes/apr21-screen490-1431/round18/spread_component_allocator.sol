...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 orderProcessingComponent = wmul(
            volMemory,
            _oneMinus(
                clamp(
                    hazardMemory * 5 +
        ...
        uint256 adverseSelectionComponent = wmul(
            sideHazard,
            clamp(wmul(flowPressure, 2600 * BPS) + wmul(divergenceMemory, 5200 * BPS), 0, WAD)
        );
        ...
        uint256 sharedSpread =
            BASE_FEE +
            wmul(volMemory, 1500 * BPS) +
            wmul(hazardMemory, 1650 * BPS) +
            wmul(orderProcessingComponent, 550 * BPS);
        ...
        if (currentSpot >= latentSpot) {
            bidProtection +=
                ...
                wmul(adverseSelectionComponent, 1250 * BPS);
        } else {
            askProtection +=
                directionalBurstFee +
                oneSidedProtection +
