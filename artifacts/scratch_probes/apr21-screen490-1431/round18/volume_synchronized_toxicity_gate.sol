...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 toxicityClock = wmul(_oneMinus(gapShort), _max(tradeSize, spotJumpVol));
        uint256 flowToxicity = wmul(flowPressure, toxicityClock);
        if (tradeSize > 4 * BPS && flowImbalance > 6000 * BPS) {
            flowToxicity = clamp(
                flowToxicity + wmul(tradeSize - 4 * BPS, 2400 * BPS),
                0,
                WAD
            );
        }
        ...
        sideHazard += wmul(flowToxicity, 4200 * BPS);
        if (sideHazard > WAD) {
            sideHazard = WAD;
        }
        ...
        if (flowPressure > 500 * BPS) {
            ...
            if (extensionSignal > 3 * BPS) {
                uint256 toxicFlowSignal =
                    flowPressure + wmul(extensionSignal, 2200 * BPS) + wmul(flowToxicity, 6000 * BPS);
        ...
        sharedSpread += wmul(flowToxicity, 800 * BPS);
