...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 quietRetailRecovery = wmul(calmMemory, quietGate);
        uint256 inventoryToxicOverlay = wmul(
            wmul(inventorySkew, inventoryToxicStress),
            720 * BPS
        );
        if (quietRetailRecovery > 2500 * BPS && inventoryToxicStress < 1200 * BPS) {
            inventoryToxicOverlay = wmul(inventoryToxicOverlay, 8500 * BPS);
        }
        uint256 toxicOverlayCap = wmul(widthFlex, 580 * BPS);
    ...
    function getName() external pure override returns (string memory) {
        return "RetailGuardedInventoryToxicOverlay";
    }
