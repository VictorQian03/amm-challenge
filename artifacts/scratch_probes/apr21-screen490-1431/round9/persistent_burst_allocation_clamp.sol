...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        if (eventSignal > 8 * BPS) {
            ...
            uint256 persistentBurstGate = clamp(
                wmul(_oneMinus(gapShort), 6500 * BPS) +
                    wmul(flowPressure, 2500 * BPS) +
                    wmul(oneSidedFlow, 1800 * BPS),
                0,
                WAD
            );
            uint256 persistentBurstSignal = wmul(burstSignal, persistentBurstGate);
            uint256 transientBurstSignal =
                burstSignal > persistentBurstSignal ? burstSignal - persistentBurstSignal : 0;
            eventCarry +=
                wmul(persistentBurstSignal, 220 * BPS) +
                wmul(transientBurstSignal, 120 * BPS);
            directionalBurstFee =
                wmul(persistentBurstSignal, 2100 * BPS) +
                wmul(transientBurstSignal, 900 * BPS);
        }
        ...
        uint256 healingRebate = wmul(directionalBurstFee, 2200 * BPS);
    ...
    function getName() external pure override returns (string memory) {
        return "PersistentBurstAllocationClamp";
    }
