...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 refillAmplificationVeto = _oneMinus(
            clamp(
                wmul(inventoryCenteringShare, 1500 * BPS) +
                    wmul(oneSidedFlow, 900 * BPS) +
                    wmul(flowPressure, 700 * BPS),
                0,
                3000 * BPS
            )
        );
        ...
        refillAuctionCut = wmul(refillAuctionCut, refillAmplificationVeto);
        if (toxicBidSide) {
    ...
    function getName() external pure override returns (string memory) {
        return "ConsumedWidthRefillAmplificationVeto";
    }
