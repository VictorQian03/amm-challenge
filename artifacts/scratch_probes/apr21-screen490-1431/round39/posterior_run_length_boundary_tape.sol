...
/// @title Posterior Run Length Boundary Tape
/// @notice Tests quantized boundary age uncertainty as a no-release order-flow state.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 tailBucketFloor = tailBucket == 0
            ...
            : tailBucket == 1
                ...
                : tailBucket == 2
                    ? 8 * BPS
                    : 13 * BPS;
        bool signedFlowFlip = slots[11] != 0 && ((trade.isBuy && slots[11] == 2) || (!trade.isBuy && slots[11] == 1));
        uint256 boundaryAge = signedFlowFlip ? 0 : clamp(slots[12] + 1, 0, 8);
        uint256 boundaryUncertainty = signedFlowFlip
            ? _max(tradeSize, wmul(volObservation, 3600 * BPS))
            : wmul(_gapRatio(8 - boundaryAge, 8), _max(spotJump, divergence));
        ...
        monotoneEvidenceFloor = _max(monotoneEvidenceFloor, wmul(boundaryUncertainty, 380 * BPS));
        ...
        sideHazard += wmul(monotoneEvidenceFloor, 950 * BPS) + wmul(boundaryUncertainty, 260 * BPS);
        if (sideHazard > WAD) {
            sideHazard = WAD;
        }
        ...
        slots[12] = boundaryAge;

        return (bidFee, askFee);
    }

    function getName() external pure override returns (string memory) {
        return "PosteriorRunLengthBoundaryTape";
    }
