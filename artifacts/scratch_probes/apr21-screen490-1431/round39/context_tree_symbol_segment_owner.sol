...
/// @title Context Tree Symbol Segment Owner
/// @notice Tests variable-memory flow-symbol segmentation as bounded upstream uncertainty.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 tailBucketFloor = tailBucket == 0
            ...
            : tailBucket == 1
                ...
                : tailBucket == 2
                    ? 8 * BPS
                    : 13 * BPS;
        uint256 symbolCode = (trade.isBuy ? 1 : 2) + (tradeSize > 10 * BPS ? 2 : 0) + (spotJump > 7 * BPS ? 3 : 0);
        uint256 previousSymbol = slots[11];
        uint256 segmentFreshness = previousSymbol == 0 || previousSymbol == symbolCode ? 0 : _gapRatio(gap + 1, 5);
        uint256 contextDepthUncertainty = wmul(segmentFreshness, _max(tradeSize, wmul(volObservation, 2800 * BPS)));
        ...
        monotoneEvidenceFloor = _max(monotoneEvidenceFloor, wmul(contextDepthUncertainty, 420 * BPS));
        ...
        sideHazard += wmul(monotoneEvidenceFloor, 950 * BPS) + wmul(contextDepthUncertainty, 300 * BPS);
        if (sideHazard > WAD) {
            sideHazard = WAD;
        }
    ...
    function getName() external pure override returns (string memory) {
        return "ContextTreeSymbolSegmentOwner";
    }
