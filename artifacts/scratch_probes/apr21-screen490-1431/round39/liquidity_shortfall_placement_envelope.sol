...
/// @title Liquidity Shortfall Placement Envelope
/// @notice Tests a one-way execution-liquidity shortfall envelope on retained spread width.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 liquidityShortfall = 0;
        if (tradeSize > 9 * BPS && gapShort < 6500 * BPS) {
            liquidityShortfall = clamp(
                wmul(tradeSize - 9 * BPS, 2400 * BPS) +
                    wmul(flowImbalance, 520 * BPS) +
                    wmul(_max(volMemory, hazardMemory), 640 * BPS),
                0,
                11 * BPS
            );
        }
        ...
        sharedSpread += liquidityShortfall;
    ...
    function getName() external pure override returns (string memory) {
        return "LiquidityShortfallPlacementEnvelope";
    }
