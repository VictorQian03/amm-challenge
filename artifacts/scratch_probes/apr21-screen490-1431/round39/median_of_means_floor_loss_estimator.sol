...
/// @title Median Of Means Floor Loss Estimator
/// @notice Tests robust block aggregation over loss evidence as a one-way protection hold.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 blockA = _max(divergenceMemory, spotJump);
        uint256 blockB = _max(volMemory, hazardMemory);
        uint256 blockC = wmul(flowImbalance, _max(volObservation, divergenceVol));
        uint256 robustFloorLoss = _median3(blockA, blockB, blockC);
        uint256 robustFloorHold = 0;
        if (robustFloorLoss > 7 * BPS) {
            robustFloorHold = clamp(wmul(robustFloorLoss - 7 * BPS, 1900 * BPS), 0, 12 * BPS);
        }
        ...
        bidProtection += robustFloorHold;
        askProtection += robustFloorHold;
    ...
    function getName() external pure override returns (string memory) {
        return "MedianOfMeansFloorLossEstimator";
    }
