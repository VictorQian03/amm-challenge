...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 benignThroughput = wmul(
            wmul(quietGate, calmMemory),
            _oneMinus(clamp(wmul(flowPressure, 3000 * BPS), 0, WAD))
        );
        if (gap >= 3 && benignThroughput > 1200 * BPS && hazardMemory < 1300 * BPS) {
            uint256 capWindow =
                BASE_FEE +
                wmul(sharedSpread > BASE_FEE ? sharedSpread - BASE_FEE : 0, 6800 * BPS) +
                wmul(sideHazard, 1800 * BPS);
            uint256 bidIncrementCap = capWindow + wmul(bidRiskSignal, 1900 * BPS);
            uint256 askIncrementCap = capWindow + wmul(askRiskSignal, 1900 * BPS);
            if (bidProtection > bidIncrementCap) {
                bidProtection = bidIncrementCap;
            }
            if (askProtection > askIncrementCap) {
                askProtection = askIncrementCap;
            }
        }
    ...
    function getName() external pure override returns (string memory) {
        return "RetailThroughputPreservationCap";
    }
