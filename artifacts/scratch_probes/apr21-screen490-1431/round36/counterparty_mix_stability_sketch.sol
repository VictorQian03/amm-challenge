...
contract Strategy is AMMStrategyBase {
    ...
    uint256 internal constant ALPHA_MIX_STABILITY = 16 * BPS;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = WAD / 2; // bounded counterparty mix-stability sketch
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 mixBalance = _oneMinus(flowImbalance);
        uint256 mixQuiet = _oneMinus(
            clamp(
                wmul(hazardMemory, 3000 * BPS) +
                    wmul(volMemory, 2200 * BPS) +
                    wmul(divergenceMemory, 2500 * BPS),
                0,
                WAD
            )
        );
        uint256 mixStabilityObservation = wmul(wmul(mixBalance, mixQuiet), gap >= 2 ? gapShort : 0);
        uint256 mixStabilityMemory =
            _blend(slots[11], mixStabilityObservation, ALPHA_MIX_STABILITY);
        bool unstableCounterpartyMix = mixStabilityMemory < 3300 * BPS && flowPressure > 700 * BPS;
        ...
        if (unstableCounterpartyMix) {
            passiveRecaptureMemory = wmul(passiveRecaptureMemory, 8800 * BPS);
        }
        ...
        if (unstableCounterpartyMix) {
            refillCalmConfirmation = wmul(refillCalmConfirmation, 6500 * BPS);
            calmDivergenceBonus = 0;
        }
        ...
        slots[11] = mixStabilityMemory;

        return (bidFee, askFee);
    }

    function getName() external pure override returns (string memory) {
        return "CounterpartyMixStabilitySketch";
    }
