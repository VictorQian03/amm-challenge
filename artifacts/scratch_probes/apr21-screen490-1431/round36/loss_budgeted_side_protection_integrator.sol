...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 lossBudget = clamp(
            wmul(hazardMemory, 1450 * BPS) +
                wmul(flowPressure, 520 * BPS) +
                wmul(oneSidedFlow, 1900 * BPS) +
                wmul(_max(divergenceMemory, spotJump), 650 * BPS),
            0,
            86 * BPS
        );
        uint256 lossBudgetFloor = BASE_FEE + lossBudget;
        uint256 sideAsymmetryCap = wmul(lossBudget, 4200 * BPS);
        ...
        if (toxicBidSide) {
            if (bidProtection < lossBudgetFloor) {
                uint256 bidFloorLift = lossBudgetFloor - bidProtection;
                if (bidFloorLift > sideAsymmetryCap) {
                    bidFloorLift = sideAsymmetryCap;
                }
                bidProtection += bidFloorLift;
            }
        } else if (askProtection < lossBudgetFloor) {
            uint256 askFloorLift = lossBudgetFloor - askProtection;
            if (askFloorLift > sideAsymmetryCap) {
                askFloorLift = sideAsymmetryCap;
            }
            askProtection += askFloorLift;
        }
    ...
    function getName() external pure override returns (string memory) {
        return "LossBudgetedSideProtectionIntegrator";
    }
