...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 independentEvidenceCount = 0;
        if (_max(divergenceMemory, divergenceVol) > 9 * BPS) {
            independentEvidenceCount += 1;
        }
        if (_max(volMemory, spotJumpVol) > 10 * BPS) {
            independentEvidenceCount += 1;
        }
        if (flowPressure > 900 * BPS && oneSidedFlow > 3 * BPS) {
            independentEvidenceCount += 1;
        }
        bool independentEvidenceQuorum = independentEvidenceCount >= 2;
        uint256 bidFlowRisk = 0;
        uint256 askFlowRisk = 0;
        if (flowDirectionalRisk > 0 && independentEvidenceQuorum) {
            if (toxicBidSide) {
                if (buyFlow >= sellFlow) {
        ...
        uint256 askRiskSignal =
            ...
            askFlowRisk;
        if (independentEvidenceQuorum) {
            uint256 quorumFloor = wmul(_max(divergenceMemory, oneSidedFlow), 900 * BPS);
            if (toxicBidSide) {
                bidRiskSignal = clamp(bidRiskSignal + quorumFloor, 0, WAD);
            } else {
                askRiskSignal = clamp(askRiskSignal + quorumFloor, 0, WAD);
            }
        }
    ...
    function getName() external pure override returns (string memory) {
        return "IndependentEvidenceQuorumRouter";
    }
