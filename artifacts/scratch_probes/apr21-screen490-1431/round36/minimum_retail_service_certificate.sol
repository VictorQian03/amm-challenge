...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 adverseVotes = 0;
        if (hazardMemory > 900 * BPS || volObservation > 12 * BPS) {
            adverseVotes += 1;
        }
        if (flowPressure > 900 * BPS || oneSidedFlow > 12 * BPS) {
            adverseVotes += 1;
        }
        if (_max(divergenceMemory, spotJump) > 11 * BPS || continuationVeto > 1400 * BPS) {
            adverseVotes += 1;
        }

        uint256 bidBeforeCut = sharedSpread + bidProtection;
        uint256 askBeforeCut = sharedSpread + askProtection;
        uint256 feeBandFloor = 48 * BPS;
        bool bandIntact =
            bidBeforeCut > bidOpportunityCut + feeBandFloor &&
            askBeforeCut > askOpportunityCut + feeBandFloor;
        bool retailServiceCertificate =
            adverseVotes < 2 &&
            bandIntact &&
            quietGate > 6200 * BPS &&
            calmMemory > 450 * BPS &&
            passiveRecaptureMemory > 300 * BPS;
        if (!retailServiceCertificate) {
            bidOpportunityCut = wmul(bidOpportunityCut, 6500 * BPS);
            askOpportunityCut = wmul(askOpportunityCut, 6500 * BPS);
        }
    ...
    function getName() external pure override returns (string memory) {
        return "MinimumRetailServiceCertificate";
    }
