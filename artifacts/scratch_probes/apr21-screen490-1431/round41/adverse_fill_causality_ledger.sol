...
/// @title Adverse Fill Causality Ledger
/// @notice Tests a weak-feasibility causal adverse-fill label that only preserves existing width.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // causal adverse-fill memory
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 causalAdverseMemory = wmul(
            slots[11],
            _gapAdjustedDecay(DECAY_HAZARD, gapShort, 1500 * BPS)
        );
        uint256 causalAdverseObservation = 0;
        bool causalAdverseFill =
            weaklyInconsistentEvent &&
            feasibilityResidual > 0 &&
            _max(spotJump, divergence) >= 2 * BPS &&
            tradeSize >= BPS / 2;
        if (causalAdverseFill) {
            causalAdverseObservation = clamp(
                _max(feasibilityResidual, wmul(_max(spotJump, divergence), 3600 * BPS)),
                0,
                12 * BPS
            );
        }
        causalAdverseMemory = _blend(causalAdverseMemory, causalAdverseObservation, ALPHA_HAZARD);
        ...
        uint256 causalAdverseWidthHold = wmul(causalAdverseMemory, 4200 * BPS);
        if (causalAdverseWidthHold > 0) {
            if (toxicBidSide) {
                askOpportunityCut = askOpportunityCut > causalAdverseWidthHold
                    ? askOpportunityCut - causalAdverseWidthHold
                    : 0;
            } else {
                bidOpportunityCut = bidOpportunityCut > causalAdverseWidthHold
                    ? bidOpportunityCut - causalAdverseWidthHold
                    : 0;
            }
        }
        ...
        slots[11] = causalAdverseMemory;

        return (bidFee, askFee);
    }

    function getName() external pure override returns (string memory) {
        return "AdverseFillCausalityLedger";
    }
