...
/// @title Benign Capture Conservation Switch
/// @notice Blocks layer-4 incremental width when weak adverse evidence collides with benign capture.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 conservedBidProtection = bidProtection;
        uint256 conservedAskProtection = askProtection;
        ...
        uint256 adverseWeakness = clamp(
            wmul(hazardMemory, 7600 * BPS) +
                wmul(flowPressure, 2100 * BPS) +
                wmul(oneSidedFlow, 3200 * BPS) +
                wmul(_max(monotoneEvidenceFloor, feasibilityResidual), 4800 * BPS),
            0,
            WAD
        );
        bool benignCaptureConserved =
            passiveRecaptureMemory > 1700 * BPS &&
            calmMemory > 1150 * BPS &&
            quietGate > 6500 * BPS &&
            adverseWeakness < 1050 * BPS &&
            directionalBurstFee + oneSidedProtection + inventoryCenteringOffset > 0;
        if (currentSpot >= latentSpot) {
            bidProtection += directionalBurstFee + oneSidedProtection + inventoryCenteringOffset;
            if (benignCaptureConserved && bidProtection > conservedBidProtection) {
                bidProtection = conservedBidProtection;
            }
        } else {
            askProtection += directionalBurstFee + oneSidedProtection + inventoryCenteringOffset;
            if (benignCaptureConserved && askProtection > conservedAskProtection) {
                askProtection = conservedAskProtection;
            }
        }
    ...
    function getName() external pure override returns (string memory) {
        return "BenignCaptureConservationSwitch";
    }
