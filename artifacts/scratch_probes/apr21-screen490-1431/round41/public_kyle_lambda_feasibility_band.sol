...
/// @title Public Kyle Lambda Feasibility Band
/// @notice Tests a stateless public impact-feasibility label that can only hold width.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 impactFeasibilityLabel = 1; // 0 consistent, 1 ambiguous, 2 impact_too_fast
        if (tradeSize > 0 && spotJump > 0) {
            uint256 publicKyleLambda = wdiv(spotJump, tradeSize);
            if (
                tradeSize > 2 * BPS &&
                spotJump > 7 * BPS &&
                publicKyleLambda > 24000 * BPS
            ) {
                impactFeasibilityLabel = 2;
            } else if (publicKyleLambda >= 4500 * BPS && publicKyleLambda <= 21000 * BPS) {
                impactFeasibilityLabel = 0;
            }
        }
        ...
        uint256 publicKyleWidthHold = 0;
        if (impactFeasibilityLabel == 2) {
            uint256 kyleExcess = spotJump > tradeSize ? spotJump - tradeSize : spotJump;
            uint256 publicKyleResidual = clamp(
                wmul(kyleExcess, 2800 * BPS) + wmul(spotJump, 1400 * BPS),
                0,
                9 * BPS
            );
            hazardObservation = clamp(hazardObservation + publicKyleResidual, 0, WAD);
            publicKyleWidthHold = clamp(
                wmul(publicKyleResidual, 1700 * BPS) + wmul(kyleExcess, 350 * BPS),
                0,
                7 * BPS
            );
        }
        ...
        sharedSpread += publicKyleWidthHold;
    ...
    function getName() external pure override returns (string memory) {
        return "PublicKyleLambdaFeasibilityBand";
    }
