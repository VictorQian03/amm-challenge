...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 calmSmallTradeGate = wmul(
            ...
            _oneMinus(
                clamp(
                    wmul(liquidityDemand, 5200 * BPS) +
                        wmul(informationStress, 9000 * BPS),
                    0,
                    WAD
                )
            )
        ...
        if (liquidityDemand > informationStress) {
            ...
            if (calmSmallTradeGate > 0) {
                demandStressWeight = wmul(
                    demandStressWeight,
                    _oneMinus(clamp(wmul(calmSmallTradeGate, 5200 * BPS), 0, 2200 * BPS))
                );
            }
        ...
        uint256 clusterObservation = wmul(
            informationStress,
            _oneMinus(wmul(gapShort, _oneMinus(wmul(calmSmallTradeGate, 3000 * BPS))))
        );
        ...
        uint256 estimatorReleaseGate = wmul(
            calmSmallTradeGate,
            _oneMinus(
                clamp(
                    wmul(flowPressure, 2600 * BPS) +
                        wmul(adverseSelectionComponent, 7000 * BPS),
                    0,
                    WAD
                )
            )
        );
        if (sharedSpread > BASE_FEE) {
            sharedRebate = clamp(
                sharedRebate +
                    wmul(sharedSpread - BASE_FEE, wmul(estimatorReleaseGate, 900 * BPS)),
                0,
                sharedSpread - MIN_FEE
            );
        }
