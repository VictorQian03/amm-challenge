...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // mechanical invariant-loss memory
        slots[12] = 0; // adverse-selection loss memory
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 mechanicalLossMemory = wmul(
            slots[11],
            _gapAdjustedDecay(DECAY_VOL, gapShort, 1700 * BPS)
        );
        uint256 adverseLossMemory = wmul(
            slots[12],
            _gapAdjustedDecay(DECAY_HAZARD, gapShort, 1900 * BPS)
        );
        uint256 mechanicalLossObservation = _max(tradeSize, spotJumpVol);
        uint256 adverseLossObservation = divergenceVol;
        if (continuationAligned) {
            adverseLossObservation = clamp(
                adverseLossObservation +
                    wmul(oneSidedFlow, 3200 * BPS) +
                    wmul(flowPressure, divergenceVol),
                0,
                WAD
            );
            mechanicalLossObservation = wmul(mechanicalLossObservation, 8300 * BPS);
        } else if (gap >= 3) {
            mechanicalLossObservation = clamp(
                mechanicalLossObservation + wmul(calmMemory, 600 * BPS),
                0,
                WAD
            );
            adverseLossObservation = wmul(adverseLossObservation, 7600 * BPS);
        }
        mechanicalLossMemory = _blend(mechanicalLossMemory, mechanicalLossObservation, ALPHA_VOL);
        adverseLossMemory = _blend(adverseLossMemory, adverseLossObservation, ALPHA_HAZARD);
        uint256 attributedAdverseShare = _share(adverseLossMemory, adverseLossMemory + mechanicalLossMemory);
        bool adverseDominates = attributedAdverseShare > 5600 * BPS && adverseLossMemory > 4 * BPS;
        ...
        uint256 attributedAdverseHazard = adverseDominates
            ? clamp(hazardMemory + wmul(adverseLossMemory, 3600 * BPS), 0, WAD)
            : wmul(hazardMemory, 9200 * BPS);
        uint256 sideHazard = attributedAdverseHazard +
            wmul(flowImbalance, _max(adverseLossMemory, divergenceMemory));
        if (sideHazard > WAD) {
            sideHazard = WAD;
        }

        uint256 flowDirectionalRisk = 0;
        if (flowPressure > 500 * BPS && adverseLossMemory >= wmul(mechanicalLossMemory, 8800 * BPS)) {
        ...
        if (adverseDominates) {
            uint256 adverseSpreadPreservation = wmul(adverseLossMemory, 360 * BPS);
            uint256 preservationCap = wmul(sharedSpread > BASE_FEE ? sharedSpread - BASE_FEE : 0, 2200 * BPS);
            if (adverseSpreadPreservation > preservationCap) {
                adverseSpreadPreservation = preservationCap;
            }
            sharedSpread += adverseSpreadPreservation;
            sharedRebate = wmul(sharedRebate, 7200 * BPS);
        } else if (mechanicalLossMemory > adverseLossMemory) {
            sharedRebate = clamp(sharedRebate + wmul(mechanicalLossMemory, 35 * BPS), 0, sharedSpread);
        }
        ...
        if (adverseDominates) {
            bidOpportunityCut = wmul(bidOpportunityCut, 9000 * BPS);
            askOpportunityCut = wmul(askOpportunityCut, 9000 * BPS);
        }
        ...
        slots[11] = mechanicalLossMemory;
        slots[12] = adverseLossMemory;

        return (bidFee, askFee);
    }

    function getName() external pure override returns (string memory) {
        return "InvariantLossAttributionSplitter";
    }
