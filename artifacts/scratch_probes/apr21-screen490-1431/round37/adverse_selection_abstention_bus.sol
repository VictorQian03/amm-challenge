...
/// @title Adverse Selection Abstention Bus
/// @notice Abstains from benign-side fee relaxation when independent adverse signals agree.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 adverseSelectionAbstention = 0;
        if (continuationAligned && flowPressure > 700 * BPS) {
            uint256 adverseResidual = _max(divergenceMemory, spotJumpVol);
            uint256 abstentionEvidence =
                adverseResidual +
                wmul(oneSidedFlow, 4200 * BPS) +
                wmul(hazardMemory, 1400 * BPS);
            if (adverseResidual > 8 * BPS && abstentionEvidence > 11 * BPS) {
                adverseSelectionAbstention = clamp(
                    wmul(abstentionEvidence - 11 * BPS, 1120000 * BPS),
                    0,
                    7200 * BPS
                );
            }
        ...
        if (toxicBidSide) {
            ...
            askOpportunityCut = wmul(askOpportunityCut, _oneMinus(adverseSelectionAbstention));
        } else {
            ...
            bidOpportunityCut = wmul(bidOpportunityCut, _oneMinus(adverseSelectionAbstention));
        }
    ...
    function getName() external pure override returns (string memory) {
        return "AdverseSelectionAbstentionBus";
    }
