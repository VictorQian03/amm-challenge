...
/// @title Matched Impact Continuation State
/// @notice Tracks matched two-sided continuation state, then consumes only abstention labels.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // buy-side continuation abstention state
        slots[12] = 0; // sell-side continuation abstention state
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 matchedImpact = _max(
            wmul(flowPressure, _max(tradeSize, volObservation)),
            wmul(flowImbalance, _max(divergenceVol, spotJumpVol))
        );
        uint256 buyContinuationState = wmul(
            slots[11],
            _gapAdjustedDecay(DECAY_FLOW, gapShort, 1800 * BPS)
        );
        uint256 sellContinuationState = wmul(
            slots[12],
            _gapAdjustedDecay(DECAY_FLOW, gapShort, 1800 * BPS)
        );
        uint256 buyContinuationObservation = 0;
        uint256 sellContinuationObservation = 0;
        if (buyFlow >= sellFlow) {
            buyContinuationObservation = matchedImpact;
        }
        if (sellFlow >= buyFlow) {
            sellContinuationObservation = matchedImpact;
        }
        buyContinuationState = _blend(buyContinuationState, buyContinuationObservation, 22 * BPS);
        sellContinuationState = _blend(sellContinuationState, sellContinuationObservation, 22 * BPS);
        uint256 continuationBalance = _max(buyContinuationState, sellContinuationState) == 0
            ? 0
            : wdiv(
                absDiff(buyContinuationState, sellContinuationState),
                _max(buyContinuationState, sellContinuationState)
            );
        bool adverseAbstentionLabel = toxicBidSide
            ? buyContinuationState > sellContinuationState && continuationBalance > 1400 * BPS
            : sellContinuationState > buyContinuationState && continuationBalance > 1400 * BPS;
        bool benignAbstentionLabel = !adverseAbstentionLabel
            && _max(buyContinuationState, sellContinuationState) > 4 * BPS
            && continuationBalance <= 9000 * BPS
            && gap >= 2;
        ...
        if (adverseAbstentionLabel) {
            continuationVeto = clamp(continuationVeto + 520 * BPS, 0, 4500 * BPS);
        } else if (benignAbstentionLabel && continuationVeto > 0) {
            continuationVeto = clamp(continuationVeto + 180 * BPS, 0, 4500 * BPS);
        }
        uint256 counterflowRelease = 0;
        if (!continuationAligned && gap >= 4 && !adverseAbstentionLabel && !benignAbstentionLabel) {
        ...
        } else if (benignAbstentionLabel) {
            passiveRecaptureObservation = wmul(passiveRecaptureObservation, 9000 * BPS);
        }
        ...
        if (adverseAbstentionLabel || benignAbstentionLabel) {
            opportunityGate = wmul(opportunityGate, adverseAbstentionLabel ? 8200 * BPS : 9200 * BPS);
        }
        ...
        slots[11] = buyContinuationState;
        slots[12] = sellContinuationState;

        return (bidFee, askFee);
    }

    function getName() external pure override returns (string memory) {
        return "MatchedImpactContinuationState";
    }
