...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 convexResidual = _max(
            wmul(_max(volMemory, hazardMemory), flowPressure),
            wmul(_max(divergenceMemory, spotJump), _max(flowPressure, flowImbalance))
        );
        uint256 concaveResidual = wmul(
            _max(calmMemory, gapLong),
            _oneMinus(
                clamp(
                    wmul(_max(volMemory, hazardMemory), 3600 * BPS) +
                        wmul(flowPressure, 2800 * BPS) +
                        wmul(divergenceMemory, 4200 * BPS),
                    0,
                    WAD
                )
            )
        );
        uint256 residualRetailConvexity = 0;
        if (convexResidual > concaveResidual) {
            residualRetailConvexity = convexResidual - concaveResidual;
            if (residualRetailConvexity > 5 * BPS) {
                uint256 convexTail = residualRetailConvexity - 5 * BPS;
                residualRetailConvexity = clamp(
                    residualRetailConvexity + wmul(convexTail, 2400 * BPS),
                    0,
                    2600 * BPS
                );
            }
        ...
        uint256 residualFloorClass = 0;
        if (residualRetailConvexity > 2 * BPS && flowPressure > 50 * BPS) {
            residualFloorClass = 1;
        }
        if (residualRetailConvexity > 6 * BPS && oneSidedFlow > 2 * BPS && gap <= 3) {
            residualFloorClass = 2;
        }
        if (residualRetailConvexity > 10 * BPS && hazardMemory > 55 * BPS && divergenceMemory > 2 * BPS) {
            residualFloorClass = 3;
        }
        ...
        if (residualFloorClass > 0) {
            uint256 residualSpreadHold = residualFloorClass == 1
                ? wmul(residualRetailConvexity, 260 * BPS)
                : residualFloorClass == 2
                    ? wmul(residualRetailConvexity, 430 * BPS)
                    : wmul(residualRetailConvexity, 620 * BPS);
            sharedSpread += clamp(residualSpreadHold, 0, 12 * BPS);
        }
    ...
    function getName() external pure override returns (string memory) {
        return "ResidualRetailConvexityPartition";
    }
