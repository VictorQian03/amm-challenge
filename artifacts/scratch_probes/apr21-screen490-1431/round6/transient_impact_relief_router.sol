...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 impactImpulse = _max(spotJumpVol, divergenceVol);
        uint256 volObservation = _max(tradeSize, impactImpulse);
        uint256 clusterObservation = wmul(volObservation, _oneMinus(gapShort));
        uint256 hazardObservation = _max(
            divergenceVol,
            impactImpulse + wmul(clusterObservation, 6800 * BPS)
        );
        ...
        uint256 sideHazard = hazardMemory + wmul(flowImbalance, _max(impactImpulse, divergenceMemory));
        if (sideHazard > WAD) {
            sideHazard = WAD;
        }
        ...
        if (flowPressure > 500 * BPS) {
            if (extensionSignal > 3 * BPS || impactImpulse > 5 * BPS) {
                uint256 toxicFlowSignal =
                    flowPressure +
                    wmul(extensionSignal, 2000 * BPS) +
                    wmul(impactImpulse, 1200 * BPS);
        ...
        uint256 persistentImpact = wmul(_max(flowPressure, hazardMemory), extensionSignal);
        uint256 transientImpact = impactImpulse > extensionSignal ? impactImpulse - extensionSignal : 0;
        uint256 eventCarry = wmul(eventSignal, 170 * BPS);
        uint256 directionalBurstFee = 0;
        if (eventSignal > 8 * BPS) {
            uint256 burstSignal = eventSignal - 8 * BPS;
            eventCarry += wmul(
                burstSignal,
                persistentImpact > 3 * BPS ? 180 * BPS : 90 * BPS
            );
            directionalBurstFee = wmul(
                burstSignal,
                persistentImpact > 3 * BPS ? 1850 * BPS : 1050 * BPS
            );
        }
        ...
        uint256 transientImpactRelief = 0;
        if (transientImpact > 0) {
            uint256 transientImpactGate = wmul(
                quietGate,
                _oneMinus(
                    clamp(
                        wmul(flowPressure, 2200 * BPS) +
                            wmul(extensionSignal, 5200 * BPS) +
                            wmul(hazardMemory, 1600 * BPS),
                        0,
                        WAD
                    )
                )
            );
            transientImpactRelief = wmul(
                wmul(transientImpact, transientImpactGate),
                gap >= 4 ? 420 * BPS : 220 * BPS
            );
            uint256 reliefCap = wmul(widthFlex, 900 * BPS);
            if (transientImpactRelief > reliefCap) {
                transientImpactRelief = reliefCap;
            }
        }
        if (toxicBidSide) {
            askOpportunityCut +=
    ...
    function getName() external pure override returns (string memory) {
        return "TransientImpactReliefRouter";
    }
