...
/// @title Tail Bucket Consumer Transducer
/// @notice Consumes existing tail buckets with a small finite classifier state.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // tail bucket consumer state
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 tailState = _tailTransducer(
            slots[11],
            tailBucket,
            continuationAligned,
            gap,
        ...
        uint256 tailStateBias = tailState == 0
            ? 0
            : tailState == 1
                ? 60 * BPS
                : tailState == 2
        ...
        sideHazard += wmul(tailStateBias, 1200 * BPS);
        if (sideHazard > WAD) {
            sideHazard = WAD;
        }
        ...
        if (tailState >= 2) {
            uint256 transducerBrake = tailState == 2 ? 9400 * BPS : 8900 * BPS;
            if (toxicBidSide) {
                askOpportunityCut = wmul(askOpportunityCut, transducerBrake);
            } else {
                bidOpportunityCut = wmul(bidOpportunityCut, transducerBrake);
            }
        }
    ...
    function getName() external pure override returns (string memory) {
        return "TailBucketConsumerTransducer";
    }
    ...
    function _tailTransducer(
        ...
        uint256 state = previousState > 3 ? 3 : previousState;
        if (tailBucket == 3) {
            return continuationAligned || flowPressure > 850 * BPS ? 3 : 2;
        }
        if (tailBucket == 2) {
            if (continuationAligned || state >= 2) {
                return 2;
            }
            return 1;
        }
        if (tailBucket == 1) {
            if (state >= 2 && (continuationAligned || gap <= 1)) {
                return 2;
            }
            return state > 0 ? 1 : 0;
        }
        if (gap >= 4 && calmMemory > 2200 * BPS) {
            return 0;
        }
        if (gap >= 2 && state > 0) {
            return state - 1;
