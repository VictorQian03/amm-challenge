...
/// @title Pickoff Exposure Stop Loss
/// @notice Track stale-price pickoff exposure by side and raise only the exposed side's protection floor.
contract Strategy is AMMStrategyBase {
    ...
    uint256 internal constant ALPHA_PICKOFF = 32 * BPS;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // bid-side stale pickoff exposure
        slots[12] = 0; // ask-side stale pickoff exposure
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 pickoffBidExposure = wmul(
            slots[11],
            _gapAdjustedDecay(8600 * BPS, gapShort, 2400 * BPS)
        );
        uint256 pickoffAskExposure = wmul(
            slots[12],
            _gapAdjustedDecay(8600 * BPS, gapShort, 2400 * BPS)
        );
        ...
        uint256 stalePickoffEvidence = 0;
        if (divergence > 5 * BPS || spotJump > 8 * BPS) {
            stalePickoffEvidence =
                _max(divergence, spotJump) +
                wmul(tradeSize, 3600 * BPS) +
                wmul(flowImbalance, _max(divergenceMemory, spotJump));
            stalePickoffEvidence = clamp(stalePickoffEvidence, 0, WAD);
        }
        uint256 pickoffContinuationGate = wmul(
            _oneMinus(gapShort),
            _oneMinus(
                clamp(
                    wmul(calmMemory, 5000 * BPS) +
                        wmul(passiveRecaptureMemory, 3600 * BPS),
                    0,
                    WAD
                )
            )
        );
        uint256 pickoffObservation = wmul(stalePickoffEvidence, pickoffContinuationGate);
        if (pickoffObservation > 0) {
            if (toxicBidSide) {
                pickoffAskExposure = _blend(pickoffAskExposure, pickoffObservation, ALPHA_PICKOFF);
                pickoffBidExposure = wmul(pickoffBidExposure, 9200 * BPS);
            } else {
                pickoffBidExposure = _blend(pickoffBidExposure, pickoffObservation, ALPHA_PICKOFF);
                pickoffAskExposure = wmul(pickoffAskExposure, 9200 * BPS);
            }
        } else if (gap >= 3) {
            uint256 quietRelease = _oneMinus(
                clamp(
                    wmul(divergenceMemory, 7000 * BPS) +
                        wmul(flowPressure, 2800 * BPS),
                    0,
                    WAD
                )
            );
            pickoffBidExposure = wmul(pickoffBidExposure, _oneMinus(wmul(quietRelease, 2600 * BPS)));
            pickoffAskExposure = wmul(pickoffAskExposure, _oneMinus(wmul(quietRelease, 2600 * BPS)));
        }
        ...
        uint256 bidPickoffFloor = wmul(pickoffBidExposure, 4300 * BPS);
        uint256 askPickoffFloor = wmul(pickoffAskExposure, 4300 * BPS);
        uint256 floorCap = 21 * BPS + wmul(_max(volMemory, hazardMemory), 2600 * BPS);
        if (bidPickoffFloor > floorCap) {
            bidPickoffFloor = floorCap;
        }
        if (askPickoffFloor > floorCap) {
            askPickoffFloor = floorCap;
        }
        if (bidProtection < bidPickoffFloor) {
            bidProtection = bidPickoffFloor;
        }
        if (askProtection < askPickoffFloor) {
            askProtection = askPickoffFloor;
        }
        ...
        slots[11] = pickoffBidExposure;
        slots[12] = pickoffAskExposure;

        return (bidFee, askFee);
    }

    function getName() external pure override returns (string memory) {
        return "PickoffExposureStopLoss";
    }
