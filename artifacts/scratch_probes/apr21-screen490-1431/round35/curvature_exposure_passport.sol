...
/// @title Curvature Exposure Passport
/// @notice Tests a discrete AMM-curvature exposure passport that can only raise firewall evidence and side floors.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 curvaturePassport = _curvaturePassport(
            tradeSize,
            spotJumpVol,
            divergenceVol,
            gapShort
        );
        uint256 passportFloor = _passportFloor(curvaturePassport);
        uint256 monotoneEvidenceFloor = _max(
            divergenceVol,
            wmul(clusterObservation, 5400 * BPS)
        );
        if (monotoneEvidenceFloor < passportFloor) {
            monotoneEvidenceFloor = passportFloor;
        }
        ...
        uint256 passportSideFloor = wmul(passportFloor, 2700 * BPS);
        if (currentSpot >= latentSpot) {
            if (bidProtection < passportSideFloor) {
                bidProtection = passportSideFloor;
            }
        } else if (askProtection < passportSideFloor) {
            askProtection = passportSideFloor;
        }
    ...
    function getName() external pure override returns (string memory) {
        return "CurvatureExposurePassport";
    }
    ...
    function _curvaturePassport(
        uint256 tradeSize,
        uint256 spotJumpVol,
        uint256 divergenceVol,
        uint256 gapShort
    ) internal pure returns (uint256) {
        uint256 bend = _max(spotJumpVol, divergenceVol);
        uint256 curvatureExposure = tradeSize + wmul(tradeSize, bend);
        if (gapShort < WAD) {
            curvatureExposure += wmul(bend, _oneMinus(gapShort));
        }
        if (curvatureExposure > 24 * BPS) {
            return 4;
        }
        if (curvatureExposure > 15 * BPS) {
            return 3;
        }
        if (curvatureExposure > 9 * BPS) {
            return 2;
        }
        if (curvatureExposure > 5 * BPS) {
            return 1;
        }
        return 0;
    }

    function _passportFloor(uint256 passport) internal pure returns (uint256) {
        if (passport == 0) {
            return 0;
        }
        if (passport == 1) {
