...
/// @title Sequential Adverse Stop Test
/// @notice Tests a bounded sequential adverse-stop state that only raises firewall and side-floor evidence.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // bounded sequential adverse-stop score
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 stopExtensionSignal = _max(divergenceMemory, spotJump);
        uint256 adverseStopPulse = 0;
        if (continuationAligned) {
            adverseStopPulse = clamp(
                wmul(flowPressure, 2600 * BPS) +
                    wmul(oneSidedFlow, 3200 * BPS) +
                    wmul(stopExtensionSignal, 1900 * BPS) +
                    wmul(tailSignal, 900 * BPS) +
                    tailBucket * 60 * BPS,
                0,
                2400 * BPS
            );
            if (tradeSize > 9 * BPS) {
                adverseStopPulse = clamp(
                    adverseStopPulse + wmul(tradeSize - 9 * BPS, 1800 * BPS),
                    0,
                    2600 * BPS
                );
            }
        }
        uint256 benignStopPulse = 0;
        if (!continuationAligned) {
            benignStopPulse = clamp(
                wmul(flowPressure, 1200 * BPS) +
                    wmul(calmMemory, 1600 * BPS) +
                    (stopExtensionSignal < 4 * BPS ? 250 * BPS : 0),
                0,
                2200 * BPS
            );
        } else if (tailSignal < 5 * BPS && flowImbalance < 5500 * BPS) {
            benignStopPulse = wmul(calmMemory, 800 * BPS);
        }
        uint256 adverseStopScore = _boundedStopUpdate(
            slots[11],
            adverseStopPulse,
            benignStopPulse
        );
        uint256 adverseStopHold = 0;
        if (adverseStopScore > 1200 * BPS) {
            adverseStopHold = clamp(
                wmul(adverseStopScore - 1200 * BPS, 2400 * BPS) +
                    wmul(adverseStopPulse, 1200 * BPS),
                0,
                3600 * BPS
            );
        }
        ...
        sideHazard += wmul(adverseStopScore, 1250 * BPS);
        if (sideHazard > WAD) {
            sideHazard = WAD;
        }
        ...
        uint256 stopSideFloor = 0;
        if (adverseStopScore > 1800 * BPS) {
            stopSideFloor = clamp(
                wmul(adverseStopScore - 1800 * BPS, 3100 * BPS) +
                    wmul(adverseStopPulse, 700 * BPS),
                0,
                14 * BPS
            );
        }
        if (toxicBidSide && bidProtection < stopSideFloor) {
            bidProtection = stopSideFloor;
        } else if (!toxicBidSide && askProtection < stopSideFloor) {
            askProtection = stopSideFloor;
        }
        ...
        slots[11] = adverseStopScore;

        return (bidFee, askFee);
    }

    function getName() external pure override returns (string memory) {
        return "SequentialAdverseStopTest";
    }
    ...
    function _boundedStopUpdate(
        uint256 previous,
        uint256 adversePulse,
        uint256 benignPulse
    ) internal pure returns (uint256) {
        uint256 carried = wmul(previous, 8800 * BPS);
        uint256 reinforcement = previous > 1800 * BPS
            ? wmul(adversePulse, 1700 * BPS)
            : wmul(adversePulse, 800 * BPS);
        uint256 nextScore = carried + adversePulse + reinforcement;
        uint256 relief = benignPulse + (benignPulse > 0 ? wmul(previous, 1800 * BPS) : 0);
        nextScore = nextScore > relief ? nextScore - relief : 0;
        return clamp(nextScore, 0, 6500 * BPS);
    }
}
