...
/// @title Conformal Abstention Certificate
/// @notice Uses normalized residual agreement to abstain from weakening toxic-side evidence.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 residualScale =
            6 * BPS +
            wmul(volMemory, 4200 * BPS) +
            wmul(hazardMemory, 3600 * BPS) +
            wmul(flowPressureFromSlots(buyFlow, sellFlow), 1800 * BPS);
        uint256 normalizedPriceResidual = _ratioToWad(divergenceVol, residualScale);
        uint256 normalizedJumpResidual = _ratioToWad(spotJumpVol, residualScale);
        uint256 normalizedSizeResidual = _ratioToWad(tradeSize, residualScale);
        uint256 residualConsensus = _min(
            _max(normalizedPriceResidual, normalizedJumpResidual),
            _max(normalizedJumpResidual, normalizedSizeResidual)
        );
        uint256 abstentionCertificate = 0;
        if (residualConsensus > 7600 * BPS) {
            abstentionCertificate =
                _max(divergenceVol, _max(spotJumpVol, wmul(tradeSize, 7000 * BPS)));
            abstentionCertificate += wmul(residualConsensus - 7600 * BPS, 10 * BPS);
        } else if (
            normalizedPriceResidual > 9000 * BPS &&
            normalizedJumpResidual > 5200 * BPS &&
            normalizedSizeResidual > 3600 * BPS
        ) {
            abstentionCertificate = _max(divergenceVol, wmul(spotJumpVol, 8200 * BPS));
        }
        uint256 abstentionFloor = clamp(abstentionCertificate, 0, 18 * BPS);
        uint256 monotoneEvidenceFloor = _max(
            divergenceVol,
            wmul(clusterObservation, 5400 * BPS)
        );
        if (monotoneEvidenceFloor < abstentionFloor) {
            monotoneEvidenceFloor = abstentionFloor;
        }
        ...
        sideHazard += wmul(abstentionFloor, 2600 * BPS);
        if (sideHazard > WAD) {
            sideHazard = WAD;
        }
        ...
        uint256 abstentionProtectionFloor = wmul(abstentionFloor, 7200 * BPS);
        if (toxicBidSide) {
            if (bidProtection < abstentionProtectionFloor) {
                bidProtection = abstentionProtectionFloor;
            }
        } else if (askProtection < abstentionProtectionFloor) {
            askProtection = abstentionProtectionFloor;
        }
    ...
    function getName() external pure override returns (string memory) {
        return "ConformalAbstentionCertificate";
    }
    ...
    function _ratioToWad(uint256 value, uint256 scale) internal pure returns (uint256) {
        if (scale == 0) {
            return 0;
        }
        return clamp(wdiv(value, scale), 0, WAD);
    }

    function flowPressureFromSlots(uint256 buyFlow, uint256 sellFlow) internal pure returns (uint256) {
        uint256 totalFlow = buyFlow + sellFlow;
        if (totalFlow == 0) {
            return 0;
        }
        return wdiv(absDiff(buyFlow, sellFlow), totalFlow);
    }
