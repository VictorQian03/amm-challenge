...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 quoteAgeRisk = gap >= 6 ? WAD : gapLong;
        uint256 recentDirectionalFill = wmul(flowPressure, _max(flowImbalance, oneSidedFlow));
        uint256 markMovement = _max(spotJump, divergenceMemory);
        uint256 optionalitySignal = clamp(
            wmul(quoteAgeRisk, 2400 * BPS) +
                wmul(recentDirectionalFill, 4200 * BPS) +
                wmul(markMovement, 6200 * BPS),
            0,
            WAD
        );
        uint256 optionalityClass = 0;
        if (optionalitySignal > 20 * BPS) {
            optionalityClass = 3;
        } else if (optionalitySignal > 12 * BPS) {
            optionalityClass = 2;
        } else if (optionalitySignal > 7 * BPS) {
            optionalityClass = 1;
        }
        uint256 optionalityWidthBudget = 0;
        if (optionalityClass > 0) {
            optionalityWidthBudget = clamp(
                optionalityClass * 3 * BPS + wmul(optionalitySignal, 35 * BPS),
                0,
                11 * BPS
            );
        }
        ...
        sharedSpread += optionalityWidthBudget;
    ...
    function getName() external pure override returns (string memory) {
        return "AdverseOptionalityClassifier";
    }
