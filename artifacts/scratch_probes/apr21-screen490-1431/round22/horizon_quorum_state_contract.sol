...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = spot; // long-horizon latent estimator
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 priorLatentSpot = slots[0];
        uint256 mediumEstimator = _blend(priorLatentSpot, currentSpot, ALPHA_SPOT);
        uint256 storedLongEstimator = slots[11] == 0 ? priorLatentSpot : slots[11];
        uint256 longEstimator = _blend(storedLongEstimator, currentSpot, 5 * BPS);
        uint256 latentSpot = mediumEstimator;
        uint256 lastSpot = slots[6];
        uint256 gap = trade.timestamp > slots[7] ? trade.timestamp - slots[7] : 0;
        uint256 shortMediumDisagreement = _relativeDiff(currentSpot, mediumEstimator);
        uint256 mediumLongDisagreement = _relativeDiff(mediumEstimator, longEstimator);
        uint256 shortLongDisagreement = _relativeDiff(currentSpot, longEstimator);
        uint256 quorumDisagreement = _max(
            shortMediumDisagreement,
            _max(mediumLongDisagreement, shortLongDisagreement)
        );
        bool quorumAligned = quorumDisagreement <= 8 * BPS;
        bool quorumContested = quorumDisagreement > 18 * BPS;
        bool quorumStale = gap >= 8 && shortLongDisagreement > 12 * BPS;
        uint256 classifierConfidence = WAD;
        if (quorumContested) {
            classifierConfidence = 8400 * BPS;
            latentSpot = _blend(latentSpot, priorLatentSpot, 850 * BPS);
        } else if (quorumStale) {
            classifierConfidence = 9000 * BPS;
            latentSpot = _blend(latentSpot, longEstimator, 550 * BPS);
        } else if (quorumAligned) {
            latentSpot = _blend(latentSpot, currentSpot, 180 * BPS);
        }
        ...
        if (classifierConfidence < WAD) {
            hazardObservation = wmul(hazardObservation, classifierConfidence);
            divergenceMemory = wmul(divergenceMemory, classifierConfidence);
        } else if (quorumAligned && quorumDisagreement < 4 * BPS) {
            uint256 confidenceLift = 250 * BPS;
            hazardObservation = clamp(
                hazardObservation + wmul(hazardObservation, confidenceLift),
                0,
                WAD
            );
        }
        ...
        slots[11] = longEstimator;

        return (bidFee, askFee);
    }

    function getName() external pure override returns (string memory) {
        return "HorizonQuorumStateContract";
    }
    ...
    function _relativeDiff(uint256 a, uint256 b) internal pure returns (uint256) {
        uint256 denominator = a > b ? a : b;
        if (denominator == 0) {
            return 0;
        }
        return wdiv(absDiff(a, b), denominator);
    }
}
