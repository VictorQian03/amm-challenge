...
contract Strategy is AMMStrategyBase {
    ...
    uint256 internal constant ALPHA_RECONSTRUCTABILITY = 16 * BPS;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // impact reconstructability confidence
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 latentSpot = _blend(priorLatentSpot, currentSpot, ALPHA_SPOT);
        ...
        uint256 impactEnvelope = clamp(
            tradeSize + wmul(tradeSize, 3500 * BPS) + 2 * BPS,
            0,
            WAD
        );
        uint256 reconstructableMove = spotJump == 0
            ? WAD
            : clamp(wdiv(_min(impactEnvelope, spotJump), spotJump), 0, WAD);
        uint256 participationEvidence = clamp(wmul(tradeSize, 12000 * BPS), 0, WAD);
        uint256 reconstructabilityObservation = wmul(reconstructableMove, participationEvidence);
        uint256 reconstructabilityMemory = _blend(
            wmul(slots[11], _gapAdjustedDecay(9300 * BPS, _gapRatio(gap, 5), 800 * BPS)),
            reconstructabilityObservation,
            ALPHA_RECONSTRUCTABILITY
        );
        uint256 impactConfidence = wmul(reconstructabilityMemory, _oneMinus(wmul(spotJump, 14000 * BPS)));
        if (impactConfidence > 0 && priorLatentSpot > 0) {
            latentSpot = _blend(
                latentSpot,
                priorLatentSpot,
                clamp(wmul(impactConfidence, 900 * BPS), 0, 900 * BPS)
            );
        }
        ...
        if (impactConfidence > 0) {
            uint256 confidenceDecay = clamp(wmul(impactConfidence, 500 * BPS), 0, 500 * BPS);
            hazardMemory = wmul(hazardMemory, _oneMinus(confidenceDecay));
            divergenceMemory = wmul(divergenceMemory, _oneMinus(confidenceDecay));
        }
        ...
        if (impactConfidence > 0) {
            uint256 observationCut = clamp(wmul(impactConfidence, 1200 * BPS), 0, 1200 * BPS);
            hazardObservation = wmul(hazardObservation, _oneMinus(observationCut));
            divergenceVol = wmul(divergenceVol, _oneMinus(wmul(observationCut, 5000 * BPS)));
        }
        ...
        slots[11] = reconstructabilityMemory;

        return (bidFee, askFee);
    }

    function getName() external pure override returns (string memory) {
        return "ImpactReconstructabilityEncoder";
    }
