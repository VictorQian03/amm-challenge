...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        if (spotJump > 4 * BPS) {
            adverseEvidence = clamp(adverseEvidence + wmul(spotJump, 1800 * BPS), 0, WAD);
        }
        uint256 adverseContinuity = wmul(
            slots[11],
            _gapAdjustedDecay(7600 * BPS, gapShort, 2600 * BPS)
        );
        adverseContinuity = _blend(adverseContinuity, adverseEvidence, 3400 * BPS);
        ...
        if (hazardMemory > 30 * BPS && flowPressure > 70 * BPS && oneSidedFlow > 2 * BPS) {
            floorPremiumRequest = 18 * BPS;
        }
        if (hazardMemory > 65 * BPS && flowPressure > 130 * BPS && oneSidedFlow > 4 * BPS && gap <= 2) {
            floorPremiumRequest = 30 * BPS;
        }
        uint256 continuityBudget = 0;
        if (adverseContinuity > 3 * BPS) {
            continuityBudget = 12 * BPS + wmul(adverseContinuity, 2200 * BPS);
        }
        if (adverseEvidence > adverseContinuity) {
            continuityBudget += wmul(adverseEvidence - adverseContinuity, 1300 * BPS);
        }
        continuityBudget = clamp(continuityBudget, 0, 32 * BPS);
        uint256 agedFloorPremium = floorPremiumRequest < continuityBudget
            ? floorPremiumRequest
            : continuityBudget;
        if (agedFloorPremium > 0) {
            if (toxicBidSide) {
                bidRiskSignal = clamp(bidRiskSignal + agedFloorPremium, 0, WAD);
            } else {
                askRiskSignal = clamp(askRiskSignal + agedFloorPremium, 0, WAD);
            }
        }
