...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 inventoryDirectionSurcharge = 0;
        if (spotJump > 4 * BPS) {
            uint256 directionStress = clamp(
                (spotJump - 4 * BPS) +
                    wmul(tradeSize, 1200 * BPS) +
                    wmul(flowPressure, 450 * BPS),
                0,
                WAD
            );
            inventoryDirectionSurcharge = clamp(wmul(directionStress, 950 * BPS), 0, 20 * BPS);
        }
        if (inventoryDirectionSurcharge > 0) {
            if (currentSpot > lastSpot && trade.isBuy) {
                askFee += inventoryDirectionSurcharge;
            } else if (lastSpot > currentSpot && !trade.isBuy) {
                bidFee += inventoryDirectionSurcharge;
            }
        }
    ...
    function getName() external pure override returns (string memory) {
        return "InventoryDirectionFeeSlope";
    }
