...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 fastMarkRisk = _max(spotJumpVol, volObservation);
        uint256 volatilityIndexedRent = 0;
        if (fastMarkRisk > 5 * BPS) {
            volatilityIndexedRent = wmul(fastMarkRisk - 5 * BPS, 650 * BPS);
        }
        volatilityIndexedRent = clamp(
            volatilityIndexedRent + wmul(volMemory, 75 * BPS),
            0,
            22 * BPS
        );
        bidFee += volatilityIndexedRent;
        askFee += volatilityIndexedRent;
    ...
    function getName() external pure override returns (string memory) {
        return "VolatilityIndexedBaseSpread";
    }
