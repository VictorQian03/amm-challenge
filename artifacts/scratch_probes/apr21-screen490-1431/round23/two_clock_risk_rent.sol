...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // fast price-move protective clock
        slots[12] = 0; // slow exposure protective clock
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 fastRiskClock = _blend(
            wmul(slots[11], _gapAdjustedDecay(7600 * BPS, gapShort, 2600 * BPS)),
            _max(spotJumpVol, informationStress),
            4200 * BPS
        );
        ...
        uint256 slowExposureClock = _blend(
            wmul(slots[12], _gapAdjustedDecay(9550 * BPS, gapLong, 600 * BPS)),
            _max(oneSidedFlow, wmul(flowPressure, _max(volMemory, hazardMemory))),
            1050 * BPS
        );
        ...
        uint256 twoClockRent = clamp(wmul(_max(fastRiskClock, slowExposureClock), 950 * BPS), 0, 24 * BPS);
        bidFee += twoClockRent;
        askFee += twoClockRent;
        ...
        slots[11] = fastRiskClock;
        slots[12] = slowExposureClock;

        return (bidFee, askFee);
    }

    function getName() external pure override returns (string memory) {
        return "TwoClockRiskRent";
    }
