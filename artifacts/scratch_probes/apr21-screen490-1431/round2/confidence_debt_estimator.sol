...
contract Strategy is AMMStrategyBase {
    ...
    uint256 internal constant ALPHA_ESTIMATION_DEBT = 20 * BPS;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // estimation-debt memory
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 estimationDebtMemory = wmul(
            slots[11],
            _gapAdjustedDecay(DECAY_DIVERGENCE, gapLong, 900 * BPS)
        );
        uint256 estimationDebtObservation = wmul(
            _max(divergence, spotJump),
            clamp(
                wmul(flowPressure, 3600 * BPS) +
                    wmul(hazardMemory, 2600 * BPS) +
                    wmul(oneSidedFlow, 1800 * BPS) +
                    (continuationAligned ? 1800 * BPS : 0),
                0,
                WAD
            )
        );
        estimationDebtMemory = _blend(
            estimationDebtMemory,
            estimationDebtObservation,
            ALPHA_ESTIMATION_DEBT
        );
        ...
        uint256 estimationConfidence = _oneMinus(
            clamp(
                wmul(estimationDebtMemory, gap >= 4 ? 7200 * BPS : 5800 * BPS) +
        ...
        if (gap >= 3) {
            ...
            quietRecenter = wmul(quietRecenter, estimationConfidence);
        ...
        uint256 quotedDivergence = wmul(divergenceMemory, estimationConfidence);
        uint256 richSignal = 0;
        uint256 cheapSignal = 0;
        if (toxicBidSide) {
            richSignal = quotedDivergence;
        } else {
            cheapSignal = quotedDivergence;
        }

        uint256 sideHazard =
            hazardMemory +
            wmul(flowImbalance, _max(volMemory, divergenceMemory)) +
            wmul(estimationDebtMemory, 4200 * BPS);
        if (sideHazard > WAD) {
            sideHazard = WAD;
        }
        ...
        if (flowPressure > 500 * BPS) {
            uint256 extensionSignal = _max(
                _max(divergenceMemory, spotJump),
                wmul(estimationDebtMemory, 3000 * BPS)
            );
            ...
            _oneMinus(
                clamp(
                    hazardMemory * 8 +
                        ...
                        wmul(estimationDebtMemory, 1800 * BPS),
                    0,
                    WAD
                )
            )
        ...
        if (hazardMemory < 1100 * BPS && flowPressure < 650 * BPS && gap >= 2) {
            calmDivergenceBonus = wmul(quotedDivergence, gap >= 4 ? 650 * BPS : 400 * BPS);
        }
        ...
        uint256 refillRiskGate = _oneMinus(
            clamp(
                hazardMemory * 8 +
                    ...
                    wmul(estimationDebtMemory, 2400 * BPS),
                0,
                WAD
            )
        );
        ...
        slots[11] = estimationDebtMemory;

        return (bidFee, askFee);
    }

    function getName() external pure override returns (string memory) {
        return "ConfidenceDebtEstimator";
    }
