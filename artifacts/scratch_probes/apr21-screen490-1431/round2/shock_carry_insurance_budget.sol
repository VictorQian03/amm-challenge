...
contract Strategy is AMMStrategyBase {
    ...
    uint256 internal constant ALPHA_SHOCK = 24 * BPS;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // persistent shock-carry memory
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 shockPulse = volObservation + hazardObservation;
        if (shockPulse > WAD) {
            shockPulse = WAD;
        }
        uint256 shockCarryMemory = wmul(
            slots[11],
            _gapAdjustedDecay(DECAY_HAZARD, gapShort, 1200 * BPS)
        );
        shockCarryMemory = _blend(
            shockCarryMemory,
            _max(shockPulse, wmul(oneSidedFlow, 8000 * BPS)),
            ALPHA_SHOCK
        );
        if (counterflowRelease > 0 && gap >= 4) {
            shockCarryMemory = wmul(
                shockCarryMemory,
                _oneMinus(clamp(wmul(counterflowRelease, 5000 * BPS), 0, WAD))
            );
        }
        ...
        uint256 askRiskSignal =
            ...
            askFlowRisk;
        uint256 toxicShockProtection = wmul(shockCarryMemory, 2600 * BPS);
        uint256 safeShockRelief = wmul(
            wmul(_oneMinus(shockCarryMemory), passiveRecaptureMemory),
            800 * BPS
        );
        if (toxicBidSide) {
            askRiskSignal += toxicShockProtection;
            bidRiskSignal = bidRiskSignal > safeShockRelief ? bidRiskSignal - safeShockRelief : 0;
        } else {
            bidRiskSignal += toxicShockProtection;
            askRiskSignal = askRiskSignal > safeShockRelief ? askRiskSignal - safeShockRelief : 0;
        }
            ...
            _oneMinus(
                clamp(
                    hazardMemory * 8 +
                        ...
                        wmul(shockCarryMemory, 2400 * BPS),
                    0,
                    WAD
                )
            )
        ...
        sharedSpread += eventCarry + wmul(shockCarryMemory, 260 * BPS);
            ...
            _oneMinus(
                clamp(
                    hazardMemory * 8 +
                        ...
                        wmul(oneSidedFlow, 2200 * BPS) +
                        wmul(shockCarryMemory, 2600 * BPS),
                    0,
                    WAD
                )
            )
        ...
        uint256 refillRiskGate = _oneMinus(
            clamp(
                hazardMemory * 8 +
                    ...
                    wmul(shockCarryMemory, 2200 * BPS),
                0,
                WAD
            )
        );
        ...
        slots[11] = shockCarryMemory;

        return (bidFee, askFee);
    }

    function getName() external pure override returns (string memory) {
        return "ShockCarryInsuranceBudget";
    }
