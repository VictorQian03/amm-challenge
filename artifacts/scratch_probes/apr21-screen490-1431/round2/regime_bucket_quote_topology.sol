...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 regimeStress = _max(eventSignal, _max(hazardMemory, oneSidedFlow));
        ...
        uint256 regimeBucketPremium = regimeStress > 12 * BPS
            ? 9 * BPS
            : regimeStress > 7 * BPS
            ? 5 * BPS
            : regimeStress > 3 * BPS
            ? 2 * BPS
            : 0;
        uint256 regimeDirectionalProtection = regimeStress > 12 * BPS
            ? 7 * BPS
            : regimeStress > 7 * BPS
            ? 4 * BPS
            : regimeStress > 3 * BPS
            ? 2 * BPS
            : 0;
        sharedSpread += eventCarry + regimeBucketPremium;
        ...
        uint256 regimeReopenCut = 0;
        if (gap >= 4 && quietGate > 6200 * BPS && hazardMemory < 1200 * BPS) {
            regimeReopenCut =
                5 * BPS +
                wmul(passiveRecaptureMemory, 260 * BPS) +
                wmul(counterflowRelease, 180 * BPS);
        } else if (gap >= 2 && quietGate > 5000 * BPS && hazardMemory < 1600 * BPS) {
            regimeReopenCut =
                2 * BPS +
                wmul(passiveRecaptureMemory, 180 * BPS) +
                wmul(calmMemory, 120 * BPS);
        }
        uint256 regimeReopenCap = wmul(widthFlex, 1800 * BPS);
        if (regimeReopenCut > regimeReopenCap) {
            regimeReopenCut = regimeReopenCap;
        }
        if (continuationAligned) {
            regimeReopenCut = wmul(regimeReopenCut, 7000 * BPS);
        }
        if (toxicBidSide) {
            askOpportunityCut += passiveRecaptureCut + calmDivergenceBonus + healingRebate + inventoryCenteringOffset;
            askOpportunityCut += regimeReopenCut;
        } else {
            bidOpportunityCut += passiveRecaptureCut + calmDivergenceBonus + healingRebate + inventoryCenteringOffset;
            bidOpportunityCut += regimeReopenCut;
        }
    ...
    function getName() external pure override returns (string memory) {
        return "RegimeBucketQuoteTopology";
    }
