...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 centerCap = wmul(widthFlex, 1400 * BPS);
        if (inventoryCenteringOffset > centerCap) {
            inventoryCenteringOffset = centerCap;
        }
        // width-skew-decoupled-quote-map: keep width stress-driven, then skew/reopen
        // benign quotes with a separate quiet-state map instead of another burst tweak.
        uint256 benignQuoteGate = wmul(
            quietGate,
            _oneMinus(
                clamp(
                    hazardMemory * 8 +
                        wmul(flowPressure, 2800 * BPS) +
                        wmul(oneSidedFlow, 2400 * BPS) +
                        wmul(volMemory, 1800 * BPS),
                    0,
                    WAD
                )
        ...
        benignQuoteGate = wmul(
            benignQuoteGate,
            _oneMinus(clamp(wmul(eventSignal, 6500 * BPS), 0, WAD))
        );
        uint256 counterflowBias = counterflowRelease > 0
            ? clamp(
                counterflowRelease + wmul(passiveRecaptureMemory, 2600 * BPS),
                0,
                WAD
            )
            : wmul(passiveRecaptureMemory, 6200 * BPS);
        uint256 benignSkewOffset = wmul(
            wmul(benignQuoteGate, counterflowBias),
            gap >= 4 ? 220 * BPS : 120 * BPS
        );
        uint256 skewCap = wmul(widthFlex, 1000 * BPS);
        if (benignSkewOffset > skewCap) {
            benignSkewOffset = skewCap;
        }
        if (currentSpot >= latentSpot) {
            bidProtection +=
                directionalBurstFee + oneSidedProtection + inventoryCenteringOffset + benignSkewOffset;
        } else {
            askProtection +=
                directionalBurstFee + oneSidedProtection + inventoryCenteringOffset + benignSkewOffset;
        }
        ...
        if (gap >= 4) {
            reopenCalmConfirmation = clamp(
                reopenCalmConfirmation +
                    ...
                    wmul(counterflowRelease, 5000 * BPS),
                0,
                WAD
            );
        } else if (gap >= 2) {
            reopenCalmConfirmation = wmul(
                clamp(
                    reopenCalmConfirmation + wmul(calmMemory, 1400 * BPS),
                    0,
                    WAD
                ),
                7000 * BPS
        ...
        uint256 reopenQuoteGate = benignQuoteGate;
        if (continuationVeto > 0) {
            reopenQuoteGate = wmul(
                reopenQuoteGate,
                _oneMinus(clamp(wmul(continuationVeto, 9000 * BPS), 0, WAD))
            );
        }
        reopenQuoteGate = wmul(
            reopenQuoteGate,
            _oneMinus(clamp(wmul(flowImbalance, 3200 * BPS), 0, WAD))
        );
        uint256 counterflowReopenBias = counterflowRelease > 0
            ? clamp(
                counterflowRelease +
                    wmul(reopenCalmConfirmation, 3200 * BPS),
                0,
                WAD
            )
            : wmul(reopenCalmConfirmation, 6800 * BPS);
        uint256 safeSideReopenCut =
            wmul(
                wmul(counterflowReopenBias, reopenQuoteGate),
                gap >= 4 ? 760 * BPS : 420 * BPS
            ) +
            wmul(
                wmul(divergenceMemory, reopenQuoteGate),
                gap >= 4 ? 110 * BPS : 55 * BPS
            );
        uint256 reopenCap = wmul(widthFlex, 2200 * BPS);
        if (safeSideReopenCut > reopenCap) {
            safeSideReopenCut = reopenCap;
        }
        if (counterflowRelease > 0) {
            safeSideReopenCut = clamp(
                safeSideReopenCut + wmul(safeSideReopenCut, counterflowRelease),
                0,
                reopenCap
            );
        }
        if (continuationAligned) {
            safeSideReopenCut = wmul(safeSideReopenCut, 6500 * BPS);
        }
        if (counterflowRelease == 0 && gap < 4) {
            safeSideReopenCut = 0;
        }
        if (toxicBidSide) {
            askOpportunityCut += passiveRecaptureCut + calmDivergenceBonus + healingRebate + safeSideReopenCut;
        } else {
            bidOpportunityCut += passiveRecaptureCut + calmDivergenceBonus + healingRebate + safeSideReopenCut;
        }
        bidOpportunityCut = clamp(
            bidOpportunityCut,
            0,
            bidProtection + wmul(widthFlex, 1200 * BPS)
        );
        askOpportunityCut = clamp(
            askOpportunityCut,
            0,
            askProtection + wmul(widthFlex, 1200 * BPS)
        );
    ...
    function getName() external pure override returns (string memory) {
        return "CounterflowReopenQuoteMap";
    }
