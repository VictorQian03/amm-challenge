...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 inventoryPassiveSupport = _max(
            calmMemory,
            wmul(
                passiveRecaptureMemory,
                _oneMinus(clamp(wmul(flowPressure, 2200 * BPS), 0, WAD))
            )
        );
        uint256 refillPassiveSupport = wmul(
            passiveRecaptureMemory,
            _oneMinus(
                clamp(
        ...
        uint256 inventoryCenteringOffset = wmul(
            wmul(inventoryPassiveSupport, inventoryGate),
            760 * BPS
        );
        ...
        uint256 inventoryCenteringShare =
            widthFlex == 0 ? 0 : wdiv(inventoryCenteringOffset, widthFlex);
        uint256 coordinationGate = _oneMinus(
            clamp(
                wmul(inventoryCenteringShare, 2600 * BPS) +
                    wmul(oneSidedFlow, 1200 * BPS) +
                    wmul(flowPressure, 1000 * BPS),
                0,
                WAD
            )
        );
        uint256 coordinationSuppressedSupport = wmul(
            refillPassiveSupport,
            _oneMinus(coordinationGate)
        );
        uint256 toxicFloorStress = clamp(
            wmul(_max(oneSidedFlow, divergenceMemory), 2800 * BPS) +
                wmul(flowPressure, 1800 * BPS) +
                wmul(hazardMemory, 1200 * BPS),
            0,
            WAD
        );
        toxicFloorStress = wmul(
            toxicFloorStress,
            _oneMinus(clamp(wmul(calmMemory, 2000 * BPS), 0, WAD))
        );
        uint256 toxicFloorReinvestment = wmul(
            wmul(coordinationSuppressedSupport, toxicFloorStress),
            900 * BPS
        );
        uint256 toxicFloorCap = wmul(widthFlex, 700 * BPS);
        if (toxicFloorReinvestment > toxicFloorCap) {
            toxicFloorReinvestment = toxicFloorCap;
        }
        if (toxicBidSide) {
            bidProtection += toxicFloorReinvestment;
        } else {
            askProtection += toxicFloorReinvestment;
        }
        ...
        uint256 passiveRecaptureCut = wmul(
            wmul(refillPassiveSupport, coordinationGate),
            1550 * BPS
        );
        ...
        uint256 refillCalmConfirmation = wmul(refillPassiveSupport, coordinationGate);
        if (gap >= 4) {
        ...
        refillAuctionBudget = wmul(refillAuctionBudget, coordinationGate);
        ...
        uint256 refillAuctionCut =
            ...
            wmul(wmul(refillAuctionBudget, refillPassiveSupport), 220 * BPS);
        if (toxicBidSide) {
            askOpportunityCut +=
                passiveRecaptureCut +
                calmDivergenceBonus +
    ...
    function getName() external pure override returns (string memory) {
        return "PassiveFloorReinvestment";
    }
