...
/// @title Benign Microtrade Safe Band
/// @notice Caps incremental one-sided protection only for tiny, quiet, no-tail trades.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 incrementalProtection =
            directionalBurstFee + oneSidedProtection + inventoryCenteringOffset;
        if (
            tailBucket == 0 &&
            tradeSize < 3 * BPS &&
            divergenceMemory < 3 * BPS &&
            flowPressure < 500 * BPS
        ) {
            uint256 microtradeCap = wmul(
                sharedSpread > BASE_FEE ? sharedSpread - BASE_FEE : 0,
                650 * BPS
            );
            if (incrementalProtection > microtradeCap) {
                incrementalProtection = microtradeCap;
            }
        }
        if (currentSpot >= latentSpot) {
            bidProtection += incrementalProtection;
        } else {
            askProtection += incrementalProtection;
        }
    ...
    function getName() external pure override returns (string memory) {
        return "BenignMicrotradeSafeBand";
    }
