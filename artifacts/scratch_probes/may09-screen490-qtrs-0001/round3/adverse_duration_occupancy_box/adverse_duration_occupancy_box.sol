...
/// @title Adverse Duration Occupancy Box
/// @notice Adds short repeated continuation stress only to toxic-side protection.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 adverseOccupancyProtection = 0;
        if (continuationAligned && gap <= 3) {
            uint256 occupancySignal = wmul(
                _max(hazardMemory, _max(divergenceMemory, spotJump)),
                _oneMinus(gapShort)
            );
            adverseOccupancyProtection = wmul(occupancySignal, 1150 * BPS);
        }
    ...
    function getName() external pure override returns (string memory) {
        return "AdverseDurationOccupancyBox";
    }
