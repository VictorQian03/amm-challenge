...
/// @title Credit Stack Exclusivity Compiler
/// @notice Allows only one safe-side credit family when active risk is present.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 safeSideCredit = passiveRecaptureCut +
            calmDivergenceBonus +
            healingRebate +
            inventoryCenteringOffset +
            refillAuctionCut;
        if (tailBucket > 0 || hazardMemory > 1100 * BPS || flowPressure > 650 * BPS) {
            safeSideCredit = passiveRecaptureCut;
            if (calmDivergenceBonus > safeSideCredit) {
                safeSideCredit = calmDivergenceBonus;
            }
            if (healingRebate > safeSideCredit) {
                safeSideCredit = healingRebate;
            }
            if (inventoryCenteringOffset > safeSideCredit) {
                safeSideCredit = inventoryCenteringOffset;
            }
            if (refillAuctionCut > safeSideCredit) {
                safeSideCredit = refillAuctionCut;
            }
        }
        if (toxicBidSide) {
            askOpportunityCut += safeSideCredit;
        } else {
            bidOpportunityCut += safeSideCredit;
        }
    ...
    function getName() external pure override returns (string memory) {
        return "CreditStackExclusivityCompiler";
    }
