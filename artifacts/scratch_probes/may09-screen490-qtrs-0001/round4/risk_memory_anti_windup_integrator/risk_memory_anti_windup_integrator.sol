...
/// @title Risk Memory Anti-Windup Integrator
/// @notice Tests bounded stale-risk unwinding in hazard and divergence memory writes.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 volAcceleration = volObservation > volMemory ? volObservation - volMemory : 0;
        ...
        uint256 blendedHazardMemory = _blend(hazardMemory, hazardObservation, ALPHA_HAZARD);
        if (hazardObservation < hazardMemory && tailBucket == 0 && spotJump < 8 * BPS && gap >= 2) {
            uint256 hazardUnwind = wmul(hazardMemory - hazardObservation, gap >= 4 ? 1800 * BPS : 900 * BPS);
            blendedHazardMemory = blendedHazardMemory > hazardUnwind ? blendedHazardMemory - hazardUnwind : hazardObservation;
            if (blendedHazardMemory < hazardObservation) {
                blendedHazardMemory = hazardObservation;
            }
        }
        hazardMemory = blendedHazardMemory;
        calmMemory = _blend(calmMemory, calmObservation, ALPHA_CALM);
        uint256 blendedDivergenceMemory = _blend(divergenceMemory, divergence, ALPHA_DIVERGENCE);
        if (divergence < divergenceMemory && tailBucket == 0 && spotJump < 8 * BPS && gap >= 2) {
            uint256 divergenceUnwind = wmul(divergenceMemory - divergence, gap >= 4 ? 1600 * BPS : 800 * BPS);
            blendedDivergenceMemory = blendedDivergenceMemory > divergenceUnwind
                ? blendedDivergenceMemory - divergenceUnwind
                : divergence;
            if (blendedDivergenceMemory < divergence) {
                blendedDivergenceMemory = divergence;
            }
        }
        divergenceMemory = blendedDivergenceMemory;
        ...
        uint256 accelerationProtection = wmul(volAcceleration, 950 * BPS);
        if (accelerationProtection > 9 * BPS) {
            accelerationProtection = 9 * BPS;
        }
        ...
        if (currentSpot >= latentSpot) {
            bidProtection += directionalBurstFee +
                oneSidedProtection +
                inventoryCenteringOffset +
                accelerationProtection;
        } else {
            askProtection += directionalBurstFee +
                oneSidedProtection +
                inventoryCenteringOffset +
                accelerationProtection;
        }
    ...
    function getName() external pure override returns (string memory) {
        return "RiskMemoryAntiWindupIntegrator";
    }
