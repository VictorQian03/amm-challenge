...
/// @title Majorization Risk Vector Filter
/// @notice Tests a one-sided dominance boundary before side-risk protection is sized.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 askRiskSignal =
            ...
            askFlowRisk;
        uint256 toxicRiskSignal = toxicBidSide ? bidRiskSignal : askRiskSignal;
        uint256 safeRiskSignal = toxicBidSide ? askRiskSignal : bidRiskSignal;
        uint256 sharedExtremeEvidence = _max(hazardMemory, _max(monotoneEvidenceFloor, tailBucketFloor));
        bool extremeSharedEvidence = tailBucket >= 2 || sharedExtremeEvidence > 18 * BPS;
        if (!extremeSharedEvidence && safeRiskSignal > 0) {
            uint256 safeRiskLimit = toxicRiskSignal > 0 ? wmul(toxicRiskSignal, 4200 * BPS) + 2 * BPS : 2 * BPS;
            if (safeRiskSignal > safeRiskLimit) {
                safeRiskSignal = safeRiskLimit;
            }
            if (toxicBidSide) {
                askRiskSignal = safeRiskSignal;
            } else {
                bidRiskSignal = safeRiskSignal;
            }
        }
    ...
    function getName() external pure override returns (string memory) {
        return "MajorizationRiskVectorFilter";
    }
