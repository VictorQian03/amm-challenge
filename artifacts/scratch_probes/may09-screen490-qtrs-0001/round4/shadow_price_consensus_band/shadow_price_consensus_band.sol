...
/// @title Shadow Price Consensus Band
/// @notice Tests a one-way shadow-price consensus breach as monotone hazard evidence.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = spot; // last unshocked shadow price
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 shadowSpot = slots[11] == 0 ? latentSpot : slots[11];
        uint256 shadowDivergence =
            shadowSpot == 0 ? 0 : wdiv(absDiff(currentSpot, shadowSpot), shadowSpot);
        bool aboveConsensus = currentSpot >= latentSpot && currentSpot >= shadowSpot;
        bool belowConsensus = currentSpot < latentSpot && currentSpot < shadowSpot;
        uint256 consensusBreach = aboveConsensus || belowConsensus ? shadowDivergence : 0;
        if (consensusBreach <= 5 * BPS) {
            consensusBreach = 0;
        } else {
            consensusBreach -= 5 * BPS;
        }
        ...
        uint256 consensusEvidence = wmul(consensusBreach, 7200 * BPS);
        if (monotoneEvidenceFloor < consensusEvidence) {
            monotoneEvidenceFloor = consensusEvidence;
        }
        ...
        if (gap >= 2 && tailBucket == 0 && hazardObservation < 10 * BPS && spotJump < 6 * BPS) {
            shadowSpot = _blend(shadowSpot, currentSpot, gap >= 4 ? 850 * BPS : 450 * BPS);
        }
    ...
    function getName() external pure override returns (string memory) {
        return "ShadowPriceConsensusBand";
    }
