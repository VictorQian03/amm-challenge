...
/// @title False Positive Protection Scorecard
/// @notice Tests a delayed false-positive ledger that gates only extra acceleration protection.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // prior extra acceleration protection
        slots[12] = 0; // prior protected bid-side flag
        slots[13] = 0; // false-positive protection memory
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 priorExtraProtection = slots[11];
        uint256 falsePositiveMemory = wmul(
            slots[13],
            _gapAdjustedDecay(9000 * BPS, gapShort, 1600 * BPS)
        );
        if (priorExtraProtection > 0 && spotJump > 0) {
            bool priorBidProtected = slots[12] > 0;
            bool priorJustified = priorBidProtected ? currentSpot >= lastSpot : currentSpot <= lastSpot;
            uint256 falsePositiveSample = priorJustified ? 0 : priorExtraProtection;
            falsePositiveMemory = _blend(falsePositiveMemory, falsePositiveSample, 2200 * BPS);
        }
        ...
        uint256 extraProtectionAllowance = _oneMinus(clamp(falsePositiveMemory * 140, 0, 5000 * BPS));
        accelerationProtection = wmul(accelerationProtection, extraProtectionAllowance);
        ...
        slots[11] = accelerationProtection;
        slots[12] = currentSpot >= latentSpot ? WAD : 0;
        slots[13] = falsePositiveMemory;

        return (bidFee, askFee);
    }

    function getName() external pure override returns (string memory) {
        return "FalsePositiveProtectionScorecard";
    }
