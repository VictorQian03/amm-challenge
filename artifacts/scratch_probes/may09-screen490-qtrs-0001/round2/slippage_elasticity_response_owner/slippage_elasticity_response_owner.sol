...
/// @title Slippage Elasticity Response Owner
/// @notice Routes realized price response into side-risk without changing tail rights or downstream cuts.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 responseDenom = tradeSize > 2 * BPS ? tradeSize : 2 * BPS;
        uint256 responseElasticity = clamp(
            wdiv(_max(spotJump, divergence), responseDenom),
            0,
            WAD
        );
        ...
        uint256 responseRiskState = wmul(
            _max(volMemory, divergenceMemory),
            clamp(responseElasticity + 2500 * BPS, 0, WAD)
        );
        uint256 sideHazard = hazardMemory + wmul(flowImbalance, responseRiskState);
        ...
        if (flowPressure > 500 * BPS && responseElasticity > 1800 * BPS) {
            ...
            if (extensionSignal > 3 * BPS) {
                uint256 toxicFlowSignal = flowPressure +
                    ...
                    wmul(responseElasticity, 450 * BPS);
    ...
    function getName() external pure override returns (string memory) {
        return "SlippageElasticityResponseOwner";
    }
