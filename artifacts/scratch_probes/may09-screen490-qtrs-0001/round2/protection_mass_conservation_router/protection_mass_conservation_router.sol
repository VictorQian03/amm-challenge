...
/// @title Protection Mass Conservation Router
/// @notice Reroutes existing protection mass toward the signed toxic side without increasing total protection.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 totalProtection = bidProtection + askProtection;
        if (totalProtection > 0) {
            uint256 toxicShare = continuationAligned ? 6200 * BPS : 5400 * BPS;
            toxicShare = clamp(
                toxicShare +
            ...
            uint256 toxicProtection = wmul(totalProtection, toxicShare);
            if (toxicProtection > totalProtection) {
                toxicProtection = totalProtection;
            }
            if (toxicBidSide) {
                bidProtection = toxicProtection;
                askProtection = totalProtection - toxicProtection;
            } else {
                askProtection = toxicProtection;
                bidProtection = totalProtection - toxicProtection;
            }
        }
    ...
    function getName() external pure override returns (string memory) {
        return "ProtectionMassConservationRouter";
    }
