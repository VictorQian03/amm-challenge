...
/// @title Adverse Benign Orthogonal Basis
/// @notice Separates one-way adverse evidence from benign diagnostics without giving benign evidence fee rights.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 alignedExtension = continuationAligned ? _max(divergenceMemory, spotJump) : 0;
        uint256 adverseBasis = wmul(
            alignedExtension,
            clamp(flowPressure + flowImbalance, 0, WAD)
        );
        uint256 benignBasis = 0;
        if (!continuationAligned && divergenceMemory < 4 * BPS) {
            benignBasis = wmul(
                _oneMinus(clamp(flowImbalance * 2, 0, WAD)),
                _oneMinus(clamp(divergenceMemory * 18, 0, WAD))
            );
        }
        ...
        sideHazard += wmul(adverseBasis, 1800 * BPS);
        if (benignBasis > 0) {
            sideHazard += wmul(wmul(benignBasis, tailBucketFloor), 450 * BPS);
        }
    ...
    function getName() external pure override returns (string memory) {
        return "AdverseBenignOrthogonalBasis";
    }
