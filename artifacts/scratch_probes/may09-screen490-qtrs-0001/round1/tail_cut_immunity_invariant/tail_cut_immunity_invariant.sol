...
/// @title Tail Cut Immunity Invariant
/// @notice Preserves tail-sourced protection from downstream opportunity cuts.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 bidTailReserve = 0;
        uint256 askTailReserve = 0;
        if (tailBucket > 0) {
            bidTailReserve = bidProtection < tailBucketFloor ? bidProtection : tailBucketFloor;
            askTailReserve = askProtection < tailBucketFloor ? askProtection : tailBucketFloor;
        }
        ...
        uint256 bidCuttableProtection =
            bidProtection > bidTailReserve ? bidProtection - bidTailReserve : 0;
        uint256 askCuttableProtection =
            askProtection > askTailReserve ? askProtection - askTailReserve : 0;
        bidOpportunityCut = clamp(
            bidOpportunityCut,
            0,
            bidCuttableProtection + wmul(baselineFlex, 1200 * BPS)
        );
        askOpportunityCut = clamp(
            askOpportunityCut,
            0,
            askCuttableProtection + wmul(baselineFlex, 1200 * BPS)
        );
    ...
    function getName() external pure override returns (string memory) {
        return "TailCutImmunityInvariant";
    }
