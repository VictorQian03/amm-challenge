...
/// @title Short Long Tail Consumer Split
/// @notice Gives short-gap tail evidence protection rights while long-gap tails only suppress calm diagnostics.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 shortTailBucket = gap < 4 ? tailBucket : 0;
        uint256 longTailBucket = gap >= 4 ? tailBucket : 0;
        uint256 tailBucketFloor = shortTailBucket == 0
            ? 0
            : shortTailBucket == 1
                ? 4 * BPS
                : shortTailBucket == 2
        ...
        if (longTailBucket > 0) {
            calmObservation = wmul(
                calmObservation,
                _oneMinus(clamp(longTailBucket * 900 * BPS, 0, 3500 * BPS))
            );
        }
        ...
        uint256 tailProtectionCap = shortTailBucket == 0
            ? 11 * BPS
            : shortTailBucket == 1
                ? 17 * BPS
                : shortTailBucket == 2
    ...
    function getName() external pure override returns (string memory) {
        return "ShortLongTailConsumerSplit";
    }
