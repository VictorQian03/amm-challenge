...
/// @title Toxic Side Cap Floor Decoupler
/// @notice Splits tail protection into toxic-side floors and non-toxic-side caps.
contract Strategy is AMMStrategyBase {
        ...
        uint256 tailProtectionCap = tailBucket == 0
            ...
            : tailBucket == 1
                ...
                : tailBucket == 2
                    ? 26 * BPS
                    : 38 * BPS;
        uint256 toxicTailCap = tailProtectionCap + tailBucketFloor;
        uint256 toxicTailFloor = tailBucketFloor;
        if (toxicBidSide) {
            if (bidProtection > toxicTailCap) {
                bidProtection = toxicTailCap;
            }
            if (bidProtection < toxicTailFloor) {
                bidProtection = toxicTailFloor;
            }
            if (askProtection > tailProtectionCap) {
                askProtection = tailProtectionCap;
            }
        } else {
            if (askProtection > toxicTailCap) {
                askProtection = toxicTailCap;
            }
            if (askProtection < toxicTailFloor) {
                askProtection = toxicTailFloor;
            }
    ...
    function getName() external pure override returns (string memory) {
        return "ToxicSideCapFloorDecoupler";
    }
