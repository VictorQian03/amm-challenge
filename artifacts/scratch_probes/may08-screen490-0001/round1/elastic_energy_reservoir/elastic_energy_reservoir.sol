...
/// @title Elastic Energy Reservoir
/// @notice Tests a bounded stored-energy floor from spot and latent displacement disagreement.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 elasticStretch = absDiff(spotJumpVol, divergenceVol);
        uint256 elasticBody = _max(spotJumpVol, divergenceVol);
        uint256 elasticSupport = tradeSize + divergenceMemory;
        if (elasticBody > elasticSupport) {
            elasticBody = elasticSupport;
        }
        uint256 elasticFloor = elasticStretch > 3 * BPS
            ? clamp(wmul(elasticBody, 3000 * BPS), 0, 8 * BPS)
            : 0;
        if (monotoneEvidenceFloor < elasticFloor) {
            monotoneEvidenceFloor = elasticFloor;
        }
