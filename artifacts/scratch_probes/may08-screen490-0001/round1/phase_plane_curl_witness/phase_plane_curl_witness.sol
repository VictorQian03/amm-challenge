...
/// @title Phase Plane Curl Witness
/// @notice Tests a bounded curl witness between divergence memory and flow pressure.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 priorDivergence = slots[8];
        uint256 priorFlowPressure = slots[9];
        uint256 curlCross = absDiff(
            wmul(divergence, priorFlowPressure),
            wmul(flowPressure, priorDivergence)
        );
        uint256 curlSlope = _max(
            absDiff(divergence, priorDivergence),
            absDiff(flowPressure, priorFlowPressure)
        );
        uint256 curlWitness = clamp(
            wmul(curlCross, 6200 * BPS) + wmul(curlSlope, 2200 * BPS),
            0,
            9 * BPS
        );
        ...
        uint256 bidSideHazard = sideHazard;
        uint256 askSideHazard = sideHazard;
        if (curlWitness > 0) {
            if (toxicBidSide) {
                bidSideHazard = clamp(sideHazard + curlWitness, 0, WAD);
            } else {
                askSideHazard = clamp(sideHazard + curlWitness, 0, WAD);
            }
        }
        ...
        uint256 bidRiskSignal =
            wmul(sellShare, bidSideHazard) +
            wmul(richSignal, 8500 * BPS) +
            bidFlowRisk;
        uint256 askRiskSignal =
            wmul(buyShare, askSideHazard) +
