...
/// @title Max Plus Risk Assembler
/// @notice Tests max-plus side-risk composition in place of additive signal stacking.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 bidBaseRisk = wmul(sellShare, sideHazard);
        uint256 bidDivergenceRisk = wmul(richSignal, 8500 * BPS);
        uint256 bidConsensus =
            bidBaseRisk < bidDivergenceRisk ? bidBaseRisk : bidDivergenceRisk;
        uint256 bidRiskSignal =
            _max(bidBaseRisk, bidDivergenceRisk) +
            wmul(bidConsensus, 1600 * BPS) +
            bidFlowRisk;

        uint256 askBaseRisk = wmul(buyShare, sideHazard);
        uint256 askDivergenceRisk = wmul(cheapSignal, 8500 * BPS);
        uint256 askConsensus =
            askBaseRisk < askDivergenceRisk ? askBaseRisk : askDivergenceRisk;
        uint256 askRiskSignal =
            _max(askBaseRisk, askDivergenceRisk) +
            wmul(askConsensus, 1600 * BPS) +
            askFlowRisk;
