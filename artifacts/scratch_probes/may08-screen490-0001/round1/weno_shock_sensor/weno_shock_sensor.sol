...
/// @title WENO Shock Sensor
/// @notice Tests a local smoothness shock feature as a bounded monotone evidence lift.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        monotoneEvidenceFloor = clamp(
            monotoneEvidenceFloor + _wenoShockLift(tradeSize, spotJumpVol, divergenceVol),
            0,
            WAD
        );
    ...
    function _wenoShockLift(
        uint256 tradeSize,
        uint256 spotJumpVol,
        uint256 divergenceVol
    ) internal pure returns (uint256) {
        uint256 leftSlope = absDiff(tradeSize, spotJumpVol);
        uint256 rightSlope = absDiff(spotJumpVol, divergenceVol);
        uint256 endpointSwing = absDiff(tradeSize, divergenceVol);
        uint256 localPeak = _max(tradeSize, _max(spotJumpVol, divergenceVol));
        uint256 localRoughness = _max(endpointSwing, _max(leftSlope, rightSlope));

        if (localPeak <= 4 * BPS || localRoughness <= BPS) {
            return 0;
        }

        uint256 curvature = absDiff(leftSlope, rightSlope);
        uint256 shockLift =
            wmul(localRoughness, 2800 * BPS) +
            wmul(curvature, 1700 * BPS);
        if (localPeak > 16 * BPS) {
            shockLift += wmul(localPeak - 16 * BPS, 320 * BPS);
        }

        return clamp(shockLift, 0, 6 * BPS);
    }
