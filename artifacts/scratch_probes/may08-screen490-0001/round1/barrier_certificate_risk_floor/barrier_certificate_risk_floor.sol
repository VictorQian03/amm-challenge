...
/// @title Barrier Certificate Risk Floor
/// @notice Tests a signed barrier-load certificate inside the side-risk path.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 barrierPriceLoad = _max(divergence, spotJump);
        uint256 barrierFlowLoad = _max(flowPressure, oneSidedFlow);
        uint256 barrierPriceExcess =
            barrierPriceLoad > 5 * BPS ? barrierPriceLoad - 5 * BPS : 0;
        uint256 barrierFlowExcess =
            barrierFlowLoad > 4 * BPS ? barrierFlowLoad - 4 * BPS : 0;
        uint256 barrierLoad = 0;
        if (barrierPriceExcess > 0 && barrierFlowExcess > 0) {
            barrierLoad = clamp(
                wmul(barrierPriceExcess, 2600 * BPS) +
                    wmul(barrierFlowExcess, 2200 * BPS) +
                    wmul(wmul(barrierPriceLoad, barrierFlowLoad), 9000 * BPS),
                0,
                13 * BPS
            );
        }
        uint256 bidBarrierRisk = toxicBidSide ? barrierLoad : 0;
        uint256 askBarrierRisk = toxicBidSide ? 0 : barrierLoad;
    ...
    function getName() external pure override returns (string memory) {
        return "BarrierCertificateRiskFloor";
    }
