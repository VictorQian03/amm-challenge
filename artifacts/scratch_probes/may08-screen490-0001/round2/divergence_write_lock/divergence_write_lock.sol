...
/// @title Divergence Write Lock
/// @notice Tests locking same-swap divergence decay behind event-clean write conditions.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        bool eventCleanForDivergenceWrite =
            !weaklyInconsistentEvent &&
            feasibilityResidual == 0 &&
            tailBucket == 0 &&
            flowImbalance < 1800 * BPS &&
            oneSidedFlow < 4 * BPS;
        if (gap >= 4 && eventCleanForDivergenceWrite) {
