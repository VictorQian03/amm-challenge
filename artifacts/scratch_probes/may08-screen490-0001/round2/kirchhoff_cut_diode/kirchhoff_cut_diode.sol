...
/// @title Kirchhoff Cut Diode
/// @notice Tests a bounded conductance gate over existing subtractive cut paths.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 cutConductance = _oneMinus(
            clamp(
                wmul(eventSignal, 1800 * BPS) +
                    wmul(feasibilityResidual, 7000 * BPS) +
                    wmul(oneSidedFlow, 2600 * BPS),
                0,
                7200 * BPS
            )
        );
        bidOpportunityCut = wmul(bidOpportunityCut, cutConductance);
        askOpportunityCut = wmul(askOpportunityCut, cutConductance);
        passiveRecaptureCut = wmul(passiveRecaptureCut, cutConductance);
        calmDivergenceBonus = wmul(calmDivergenceBonus, cutConductance);
        healingRebate = wmul(healingRebate, cutConductance);
        inventoryCenteringOffset = wmul(inventoryCenteringOffset, cutConductance);
        refillAuctionCut = wmul(refillAuctionCut, cutConductance);
        if (toxicBidSide) {
