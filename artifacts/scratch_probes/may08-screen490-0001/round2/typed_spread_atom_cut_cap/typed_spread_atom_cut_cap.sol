...
/// @title Typed Spread Atom Cut Cap
/// @notice Tests cut caps that only consume benign shared width, not protected atoms.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 baseSpreadAtom = BASE_FEE;
        uint256 riskSpreadAtom =
            wmul(volMemory, 1800 * BPS) +
            wmul(hazardMemory, 1900 * BPS);
        uint256 sharedSpread = baseSpreadAtom + riskSpreadAtom;
        ...
        uint256 protectedSharedWidth = baseSpreadAtom + eventCarry;
        uint256 benignCutEligibleWidth = sharedSpread > protectedSharedWidth
            ? sharedSpread - protectedSharedWidth
            : 0;
        uint256 bidRiskProtection = wmul(bidRiskSignal, 5400 * BPS);
        uint256 askRiskProtection = wmul(askRiskSignal, 5400 * BPS);
        uint256 tailProtectionCap = tailBucket == 0
            ...
            : tailBucket == 1
                ...
                : tailBucket == 2
                    ? 26 * BPS
                    : 38 * BPS;
        if (bidRiskProtection > tailProtectionCap) {
            bidRiskProtection = tailProtectionCap;
        }
        if (askRiskProtection > tailProtectionCap) {
            askRiskProtection = tailProtectionCap;
        }
        ...
        uint256 bidSideProtection = 0;
        uint256 askSideProtection = 0;
        if (currentSpot >= latentSpot) {
            bidSideProtection = directionalBurstFee + oneSidedProtection + inventoryCenteringOffset;
        } else {
            askSideProtection = directionalBurstFee + oneSidedProtection + inventoryCenteringOffset;
        }
        uint256 bidProtection = bidRiskProtection + bidSideProtection;
        uint256 askProtection = askRiskProtection + askSideProtection;
