...
/// @title Latent State Gap Bias Candidate
/// @notice Preserve the incumbent latent-state defense but release stale pressure after quiet gaps.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        if (gap >= 5) {
            ...
            bidPressure = wmul(bidPressure, 9200 * BPS);
            askPressure = wmul(askPressure, 9200 * BPS);
            cooldown = wmul(cooldown, 8800 * BPS);
            sizeMemory = wmul(sizeMemory, 9400 * BPS);
        }
        ...
        if (currentSpot >= latentSpot) {
            bidFee = clampFee(bidFee + BPS + wmul(latentGap, 500 * BPS));
        } else {
            askFee = clampFee(askFee + BPS + wmul(latentGap, 500 * BPS));
        }
        ...
        if (trade.isBuy) {
            ...
            if (cooldown < 6 * BPS && gap >= 3 && askFee > 4 * BPS) {
                askFee -= 4 * BPS;
            } else if (cooldown < 6 * BPS && askFee > 3 * BPS) {
                askFee -= 3 * BPS;
            }
            slots[5] = 1;
        } else {
            ...
            if (cooldown < 6 * BPS && gap >= 3 && bidFee > 4 * BPS) {
                bidFee -= 4 * BPS;
            } else if (cooldown < 6 * BPS && bidFee > 3 * BPS) {
                bidFee -= 3 * BPS;
            }
            slots[5] = 2;
        }
