...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 reserveTieBreakGate = 0;
        if (gap >= 2) {
            uint256 reserveValue = wmul(trade.reserveX, latentSpot);
            uint256 reserveTotal = reserveValue + trade.reserveY;
            uint256 reserveSkew = reserveTotal == 0
                ? 0
                : wdiv(absDiff(reserveValue, trade.reserveY), reserveTotal);
            reserveTieBreakGate = wmul(quietGate, calmMemory);
            reserveTieBreakGate = wmul(
                reserveTieBreakGate,
                _oneMinus(
                    clamp(
                        wmul(divergenceMemory, 1700 * BPS) +
            ...
            reserveTieBreakGate = wmul(reserveTieBreakGate, reserveSkew);
            if (reserveTieBreakGate > 0) {
                uint256 reserveTieBreakCut = wmul(reserveTieBreakGate, 1800 * BPS);
                if (trade.reserveY >= reserveValue) {
                    bidOpportunityCut += reserveTieBreakCut;
                } else {
                    askOpportunityCut += reserveTieBreakCut;
                }
            }
        }
