...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // stress memory
        slots[12] = 0; // stress recovery memory
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 stressObservation = _max(hazardObservation, oneSidedFlow);
        stressObservation = _max(stressObservation, divergenceVol);
        uint256 previousStress = slots[11];
        uint256 stressRecoveryObservation = previousStress > stressObservation
            ? previousStress - stressObservation
            : 0;
        uint256 stressMemory = _blend(
            wmul(previousStress, _gapAdjustedDecay(9000 * BPS, gapShort, 1300 * BPS)),
            stressObservation,
            ALPHA_STRESS
        );
        uint256 stressRecoveryMemory = _blend(slots[12], stressRecoveryObservation, ALPHA_STRESS);
        ...
        if (gap >= 4 && quietGate > 9300 * BPS && previousStress > 8 * BPS && stressRecoveryObservation > 3 * BPS) {
            healingRebate += wmul(stressRecoveryMemory, 550 * BPS);
        }
        if (currentSpot >= latentSpot) {
        ...
        slots[11] = stressMemory;
        slots[12] = stressRecoveryMemory;
