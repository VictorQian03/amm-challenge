...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        if (eventSignal > 5 * BPS) {
            uint256 special = wmul(eventSignal - 5 * BPS, 820 * BPS);
            if (trade.isBuy) {
                bidFee += special;
                askFee += special / 4;
            } else {
        ...
        if (dangerSignal > 2 * BPS) {
            directionalTilt = BPS + wmul(dangerSignal - 2 * BPS, 820 * BPS);
        }
        ...
        if (dangerSignal >= 2 * BPS && dangerSignal > reversionSignal) {
            sameSideBoost = BPS + wmul(dangerSignal - 2 * BPS, 520 * BPS);
