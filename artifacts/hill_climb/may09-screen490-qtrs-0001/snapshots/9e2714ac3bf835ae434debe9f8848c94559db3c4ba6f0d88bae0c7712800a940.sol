...
/// @title Flow Share Entropy Risk Gate
/// @notice Uses nonlinear flow concentration as a read-only side-risk gate.
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 flowConcentrationGate = wmul(
            flowImbalance,
            clamp(flowImbalance + 3000 * BPS, 0, WAD)
        );
        ...
        uint256 sideHazard =
            hazardMemory + wmul(flowConcentrationGate, _max(volMemory, divergenceMemory));
        ...
        uint256 bidRiskSignal =
            ...
            wmul(flowConcentrationGate, wmul(richSignal, 900 * BPS)) +
            wmul(richSignal, 8500 * BPS) +
            bidFlowRisk;
        uint256 askRiskSignal =
            wmul(buyShare, sideHazard) +
            wmul(flowConcentrationGate, wmul(cheapSignal, 900 * BPS)) +
    ...
    function getName() external pure override returns (string memory) {
        return "FlowShareEntropyRiskGate";
    }
