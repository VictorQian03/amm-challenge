...
contract Strategy is AMMStrategyBase {
    ...
    uint256 internal constant ALPHA_ARRIVAL = 16 * BPS;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // expected arrival cadence memory
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 expectedArrival = slots[11];
        uint256 gapSurprisal = absDiff(gapLong, expectedArrival);
        ...
        uint256 benignCompetitiveGate = 0;
        if (gap >= 4) {
            uint256 arrivalQuietGate = _oneMinus(
                clamp(
                    wmul(gapSurprisal, 9000 * BPS) +
            ...
            benignCompetitiveGate = wmul(arrivalQuietGate, quietGate);
            benignCompetitiveGate = wmul(benignCompetitiveGate, _max(calmMemory, passiveRecaptureMemory));
        }
        uint256 benignCompetitiveBand = wmul(
            gapLong,
            wmul(benignCompetitiveGate, wmul(sharedSpread, 600 * BPS))
        );
        bidFee = bidFee > benignCompetitiveBand ? bidFee - benignCompetitiveBand : MIN_FEE;
        askFee = askFee > benignCompetitiveBand ? askFee - benignCompetitiveBand : MIN_FEE;
        ...
        slots[11] = _blend(expectedArrival, gapLong, ALPHA_ARRIVAL);
