...
contract Strategy is AMMStrategyBase {
    ...
    uint256 internal constant DECAY_ALPHA = 9400 * BPS;
    ...
    uint256 internal constant ALPHA_ALPHA = 16 * BPS;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // slow flow-alpha memory
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 flowAlphaMemory = wmul(
            slots[11],
            _gapAdjustedDecay(DECAY_ALPHA, gapLong, 900 * BPS)
        );
        uint256 flowAlphaObservation = wmul(flowImbalance, _max(divergenceMemory, spotJump));
        flowAlphaMemory = _blend(flowAlphaMemory, flowAlphaObservation, ALPHA_ALPHA);
        uint256 oneSidedFlow = wmul(flowImbalance, _max(volMemory, hazardMemory));
        bool continuationBuy = buyFlow >= sellFlow;
        bool toxicContinuation =
            (currentSpot >= latentSpot && continuationBuy) ||
            (currentSpot < latentSpot && !continuationBuy);
        bool benignCounterflow =
            (currentSpot >= latentSpot && !continuationBuy) ||
            (currentSpot < latentSpot && continuationBuy);
        ...
        if (gap >= 3) {
            ...
            if (toxicContinuation) {
                quietRecenter = wmul(
                    quietRecenter,
                    _oneMinus(clamp(wmul(flowAlphaMemory, 2200 * BPS), 0, WAD))
                );
            } else if (benignCounterflow) {
                quietRecenter = clamp(
                    quietRecenter + wmul(flowAlphaMemory, 450 * BPS),
                    0,
                    WAD
                );
            }
