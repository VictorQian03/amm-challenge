...
contract Strategy is AMMStrategyBase {
    ...
    uint256 internal constant DECAY_SHELL = 9550 * BPS;
    ...
    uint256 internal constant ALPHA_SHELL = 18 * BPS;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // continuation shell memory
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        if (flowPressure > 500 * BPS) {
            ...
            if (extensionSignal > 3 * BPS) {
                toxicFlowSignal = flowPressure + wmul(extensionSignal, 2200 * BPS);
        ...
        uint256 alignedFlow = toxicBidSide ? buyFlow : sellFlow;
        uint256 counterFlow = toxicBidSide ? sellFlow : buyFlow;
        bool alignedContinuation = alignedFlow >= counterFlow;
        uint256 eventSignal = volObservation + hazardObservation;
        if (eventSignal > WAD) {
            eventSignal = WAD;
        }
        uint256 shellUsageObservation = clamp(
            wmul(eventSignal, 5200 * BPS) +
                wmul(oneSidedFlow, 6500 * BPS) +
                wmul(toxicFlowSignal, 2200 * BPS),
            0,
            WAD
        );
        uint256 shellBudget = wmul(
            slots[11],
            _gapAdjustedDecay(DECAY_SHELL, gapLong, 450 * BPS)
        );
        shellBudget = _blend(shellBudget, shellUsageObservation, ALPHA_SHELL);
        if (shellUsageObservation < 7 * BPS) {
            uint256 protectedInterval = wmul(calmMemory, wmul(quietGate, gapLong));
            shellBudget = wmul(
                shellBudget,
                _oneMinus(clamp(wmul(protectedInterval, 200 * BPS), 0, WAD))
            );
        }
        ...
        uint256 eventCarry = wmul(eventSignal, 200 * BPS);
        ...
        if (eventSignal > 8 * BPS) {
            ...
            directionalBurstFee = wmul(burstSignal, 1500 * BPS);
        }
        sharedSpread += eventCarry + wmul(shellBudget, 700 * BPS);
        ...
        uint256 oneSidedProtection = wmul(oneSidedFlow, 2200 * BPS);
        uint256 continuationShell = 0;
        if (alignedContinuation) {
            continuationShell =
                wmul(shellBudget, 1100 * BPS) +
                wmul(oneSidedFlow, 900 * BPS);
        }
        uint256 healingRebate = wmul(directionalBurstFee, 3200 * BPS);
        ...
        if (toxicBidSide) {
            bidProtection += directionalBurstFee + oneSidedProtection + continuationShell;
        } else {
            askProtection += directionalBurstFee + oneSidedProtection + continuationShell;
        }
        ...
        bidOpportunityCut = clamp(
            bidOpportunityCut,
            0,
            bidProtection + wmul(baselineFlex, 1000 * BPS)
        );
        askOpportunityCut = clamp(
            askOpportunityCut,
            0,
            askProtection + wmul(baselineFlex, 1000 * BPS)
        );
