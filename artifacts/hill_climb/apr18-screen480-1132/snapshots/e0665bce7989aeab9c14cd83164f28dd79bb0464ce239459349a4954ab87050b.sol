        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        bool stressLane =
            ...
            (hazardMemory > 900 * BPS ||
                oneSidedFlow > 700 * BPS ||
                divergenceMemory > 7 * BPS);
        bool refillLane =
            gap >= 2 &&
            quietGate > 3500 * BPS &&
            passiveRecaptureMemory > 180 * BPS &&
            hazardMemory < 1200 * BPS &&
            flowPressure < 900 * BPS &&
            oneSidedFlow < 1000 * BPS;
        uint256 safeSideBaseCut = passiveRecaptureCut + calmDivergenceBonus;
        uint256 safeSideStressCut =
            safeSideBaseCut + healingRebate + inventoryCenteringOffset;
        uint256 safeSideNeutralCut =
            safeSideBaseCut + wmul(inventoryCenteringOffset, 3500 * BPS);
        uint256 safeSideRefillCut =
            safeSideBaseCut +
            refillAuctionCut +
            wmul(inventoryCenteringOffset, 1800 * BPS);
        uint256 safeSideCut = stressLane
            ? safeSideStressCut
            : refillLane
                ? safeSideRefillCut
                : safeSideNeutralCut;
        if (toxicBidSide) {
            askOpportunityCut += safeSideCut;
        } else {
            bidOpportunityCut += safeSideCut;
        }
