...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // one-sided hazard regime memory
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        bool alignedContinuation = toxicBidSide ? buyFlow >= sellFlow : sellFlow > buyFlow;
        uint256 hazardModeObservation = 0;
        if (
            alignedContinuation &&
            flowPressure > 500 * BPS &&
            extensionSignal > 3 * BPS &&
            hazardMemory > 700 * BPS
        ) {
            uint256 directionShare = toxicBidSide ? buyShare : sellShare;
            uint256 regimePressure = clamp(
                wmul(flowPressure, 9000 * BPS) +
                    wmul(hazardMemory, 7500 * BPS) +
                    wmul(extensionSignal, 11000 * BPS),
                0,
                WAD
            );
            hazardModeObservation = wmul(wmul(directionShare, regimePressure), _oneMinus(quietGate));
        }
        uint256 hazardMode = _blend(slots[11], hazardModeObservation, 2600 * BPS);
        if (hazardModeObservation == 0) {
            hazardMode = wmul(hazardMode, gap >= 4 ? 9200 * BPS : 9700 * BPS);
        }

        uint256 bidSideRisk = wmul(sellShare, sideHazard);
        uint256 askSideRisk = wmul(buyShare, sideHazard);
        uint256 bidBaseRisk = wmul(richSignal, 8500 * BPS) + bidFlowRisk;
        uint256 askBaseRisk = wmul(cheapSignal, 8500 * BPS) + askFlowRisk;
        uint256 bidRiskSignal = bidSideRisk + bidBaseRisk;
        uint256 askRiskSignal = askSideRisk + askBaseRisk;
        if (alignedContinuation) {
            uint256 toxicRiskSignal =
                sideHazard +
                wmul(extensionSignal, 7500 * BPS) +
                wmul(oneSidedFlow, 8500 * BPS) +
                _max(bidFlowRisk, askFlowRisk);
            if (toxicRiskSignal > WAD) {
                toxicRiskSignal = WAD;
            }
            ...
            if (toxicBidSide) {
                bidRiskSignal =
                    bidBaseRisk +
                    wmul(bidSideRisk, _oneMinus(hazardMode)) +
                    wmul(toxicRiskSignal, hazardMode);
            } else {
                askRiskSignal =
                    askBaseRisk +
                    wmul(askSideRisk, _oneMinus(hazardMode)) +
                    wmul(toxicRiskSignal, hazardMode);
            }
        }
