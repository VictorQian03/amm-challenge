...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 bidOpportunityCut = wmul(bidOpportunitySignal, 7000 * BPS);
        uint256 askOpportunityCut = wmul(askOpportunitySignal, 7000 * BPS);
        uint256 passiveRecaptureCut = wmul(passiveRecaptureMemory, 1250 * BPS);
        ...
        uint256 bidFlexCap = wmul(baselineFlex, 1200 * BPS);
        uint256 askFlexCap = bidFlexCap;
        uint256 narrowRecoveryGate = wmul(opportunityGate, quietGate);
        if (narrowRecoveryGate > 0) {
            uint256 gatedHealingIncrement = wmul(
                wmul(directionalBurstFee, 200 * BPS),
                narrowRecoveryGate
            );
            uint256 gatedFlexCapIncrement = wmul(
                wmul(baselineFlex, 300 * BPS),
                narrowRecoveryGate
            );
            if (currentSpot >= latentSpot) {
                askOpportunityCut += gatedHealingIncrement;
                askFlexCap += gatedFlexCapIncrement;
            } else {
                bidOpportunityCut += gatedHealingIncrement;
                bidFlexCap += gatedFlexCapIncrement;
            }
        }
        bidOpportunityCut = clamp(
            bidOpportunityCut,
            0,
            bidProtection + bidFlexCap
        );
        askOpportunityCut = clamp(
            askOpportunityCut,
            0,
            askProtection + askFlexCap
        );
