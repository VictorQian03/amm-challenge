...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 healingRebate = wmul(directionalBurstFee, 3000 * BPS);
        if (currentSpot >= latentSpot) {
        ...
        bidOpportunityCut = clamp(
            bidOpportunityCut,
            0,
            bidProtection + wmul(baselineFlex, 1500 * BPS)
        );
        askOpportunityCut = clamp(
            askOpportunityCut,
            0,
            askProtection + wmul(baselineFlex, 1500 * BPS)
        );
