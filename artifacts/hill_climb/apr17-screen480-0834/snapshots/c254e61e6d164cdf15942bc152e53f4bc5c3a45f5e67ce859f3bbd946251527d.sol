...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        if (gap >= 2 && baselineFlex > 0) {
            uint256 recoveryGate = wmul(
            ...
            uint256 nonToxicRecoveryCut = wmul(wmul(baselineFlex, 750 * BPS), recoveryGate);
            uint256 nonToxicCapLift = wmul(wmul(baselineFlex, 350 * BPS), recoveryGate);
            if (currentSpot >= latentSpot) {
                askOpportunityCut += nonToxicRecoveryCut;
                askFlexCap += nonToxicCapLift;
            } else {
                bidOpportunityCut += nonToxicRecoveryCut;
                bidFlexCap += nonToxicCapLift;
            }
        }
