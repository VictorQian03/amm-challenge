...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 stressSignal = _max(spotJumpVol, divergenceVol);
        uint256 persistentCluster = wmul(_max(volMemory, hazardMemory), _oneMinus(gapShort));
        uint256 clusterObservation = _max(stressSignal, persistentCluster);
        uint256 volObservation = _max(
            tradeSize,
            wmul(stressSignal, 5500 * BPS) + wmul(persistentCluster, 6500 * BPS)
        );
        uint256 hazardObservation = _max(
            divergenceVol,
            volObservation + wmul(clusterObservation, 4500 * BPS)
        );
        uint256 calmObservation = wmul(
            gapLong,
            _oneMinus(clamp(hazardObservation * 5 + wmul(clusterObservation, 1200 * BPS), 0, WAD))
        );
