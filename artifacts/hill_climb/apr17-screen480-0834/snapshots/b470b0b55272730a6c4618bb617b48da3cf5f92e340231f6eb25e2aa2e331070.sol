...
contract Strategy is AMMStrategyBase {
    ...
    uint256 internal constant MAX_CALM_CREDIT = 8 * BPS;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // earned calm-credit balance
        slots[12] = 0; // benign confirmation streak
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        bool benignConfirmation =
            gap >= 4 &&
            quietGate > 5500 * BPS &&
            hazardMemory < 1100 * BPS &&
            flowPressure < 700 * BPS &&
            spotJump < 7 * BPS &&
            divergenceMemory < 9 * BPS;
        uint256 benignConfirmStreak = benignConfirmation
            ? (slots[12] < 4 ? slots[12] + 1 : 4)
            : 0;
        uint256 calmCreditBalance = wmul(
            slots[11],
            benignConfirmation ? 9500 * BPS : gap >= 2 ? 7600 * BPS : 6200 * BPS
        );
        if (benignConfirmStreak >= 2) {
            uint256 creditFill = wmul(
                wmul(gapLong, quietGate),
                _oneMinus(
                    clamp(
                        wmul(hazardMemory, 4000 * BPS) +
                            wmul(flowPressure, 3600 * BPS) +
                            wmul(divergenceMemory, 2800 * BPS),
                        0,
                        WAD
                    )
                )
            );
            creditFill = wmul(creditFill, benignConfirmStreak >= 3 ? 4 * BPS : 2 * BPS);
            calmCreditBalance = clamp(calmCreditBalance + creditFill, 0, MAX_CALM_CREDIT);
        }
        ...
        uint256 sharedRebate = wmul(wmul(calmMemory, quietGate), 140 * BPS);
        ...
        uint256 activeOpportunitySignal = currentSpot >= latentSpot
            ? askOpportunitySignal
            : bidOpportunitySignal;
        uint256 activeOpportunityCut = currentSpot >= latentSpot
            ? askOpportunityCut
            : bidOpportunityCut;
        uint256 creditSpend = 0;
        if (
            activeOpportunitySignal > 0 &&
            gap >= 2 &&
            quietGate > 4500 * BPS &&
            hazardMemory < 1400 * BPS &&
            flowPressure < 850 * BPS
        ) {
            uint256 relativeSpendCap = wmul(activeOpportunityCut, 3500 * BPS);
            uint256 absoluteSpendCap = gap >= 4 ? 4 * BPS : 2 * BPS;
            creditSpend = wmul(activeOpportunitySignal, gap >= 4 ? 6500 * BPS : 3800 * BPS);
            if (creditSpend > calmCreditBalance) {
                creditSpend = calmCreditBalance;
            }
            if (creditSpend > relativeSpendCap) {
                creditSpend = relativeSpendCap;
            }
            if (creditSpend > absoluteSpendCap) {
                creditSpend = absoluteSpendCap;
            }
        }
        if (currentSpot >= latentSpot) {
            askOpportunityCut += creditSpend;
        } else {
            bidOpportunityCut += creditSpend;
        }
        uint256 creditDebit = creditSpend + wmul(creditSpend, 7000 * BPS);
        calmCreditBalance =
            calmCreditBalance > creditDebit ? calmCreditBalance - creditDebit : 0;
        ...
        slots[11] = calmCreditBalance;
        slots[12] = benignConfirmStreak;
