...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        if (eventSignal > 8 * BPS) {
            ...
            eventCarry += wmul(burstSignal, 350 * BPS);
            directionalBurstFee = wmul(burstSignal, 1550 * BPS);
        }
        ...
        uint256 oneSidedProtection = wmul(oneSidedFlow, 1700 * BPS);
        uint256 healingRebate = wmul(directionalBurstFee, 2500 * BPS);
        if (currentSpot >= latentSpot) {
        ...
        bidOpportunityCut = clamp(bidOpportunityCut, 0, bidProtection);
        askOpportunityCut = clamp(askOpportunityCut, 0, askProtection);
