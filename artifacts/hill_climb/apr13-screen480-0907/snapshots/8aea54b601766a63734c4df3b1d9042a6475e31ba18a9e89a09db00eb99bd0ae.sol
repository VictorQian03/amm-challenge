...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // calm confirmation counter
        slots[12] = 0; // bounded calm-relief envelope
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        bool continuationUp =
            currentSpot >= latentSpot && buyFlow > sellFlow && spotJump > divergenceMemory;
        bool continuationDown =
            currentSpot < latentSpot && sellFlow > buyFlow && spotJump > divergenceMemory;
        bool calmReleaseBreak =
            continuationUp ||
            continuationDown ||
            hazardMemory > 1200 * BPS ||
            flowPressure > 700 * BPS ||
            divergenceMemory > 10 * BPS;
        bool calmReleaseConfirm =
            gap >= 4 &&
            quietGate > 6800 * BPS &&
            hazardMemory < 1100 * BPS &&
            flowPressure < 550 * BPS &&
            divergenceMemory < 8 * BPS;
        uint256 calmConfirmCounter = slots[11];
        if (calmReleaseBreak) {
            calmConfirmCounter = 0;
        } else if (calmReleaseConfirm) {
            calmConfirmCounter = calmConfirmCounter < 2 ? calmConfirmCounter + 1 : 2;
        } else if (calmConfirmCounter > 0) {
            calmConfirmCounter -= 1;
        }

        uint256 calmReleaseEnvelope = slots[12];
        uint256 envelopeDecay = calmReleaseBreak
            ? 6200 * BPS
            : calmConfirmCounter > 1
                ? 9300 * BPS
                : calmConfirmCounter > 0
                    ? 8800 * BPS
                    : 7600 * BPS;
        calmReleaseEnvelope = wmul(calmReleaseEnvelope, envelopeDecay);
        if (calmConfirmCounter > 1) {
            uint256 calmEnvelopeObservation =
                passiveRecaptureMemory > 14 * BPS ? passiveRecaptureMemory - 14 * BPS : 0;
            calmEnvelopeObservation += wmul(wmul(opportunityGate, quietGate), 8 * BPS);
            calmEnvelopeObservation += wmul(
                _oneMinus(clamp(divergenceMemory * 12, 0, WAD)),
                3 * BPS
            );
            calmReleaseEnvelope = _blend(calmReleaseEnvelope, calmEnvelopeObservation, 4200 * BPS);
        }
        uint256 calmEnvelopeCap =
            calmReleaseConfirm &&
                gap >= 5 &&
                hazardMemory < 950 * BPS &&
                flowPressure < 500 * BPS
                ? 26 * BPS
                : 18 * BPS;
        calmReleaseEnvelope = clamp(calmReleaseEnvelope, 0, calmEnvelopeCap);
        ...
        if (currentSpot >= latentSpot) {
            ...
            bidFee = bidFee > healingRebate ? bidFee - healingRebate : MIN_FEE;
        }

        uint256 bidOpportunityCut = wmul(bidOpportunitySignal, 8200 * BPS);
        uint256 askOpportunityCut = wmul(askOpportunitySignal, 8200 * BPS);
        uint256 passiveReleaseThreshold = calmConfirmCounter > 1 ? 14 * BPS : 16 * BPS;
        uint256 passiveReleaseSignal =
            passiveRecaptureMemory > passiveReleaseThreshold
                ? passiveRecaptureMemory - passiveReleaseThreshold
                : 0;
        uint256 passiveRecaptureCut = wmul(
            passiveReleaseSignal,
            calmConfirmCounter > 1 ? 1650 * BPS : 900 * BPS
        );
        uint256 calmEnvelopeCut = wmul(
            calmReleaseEnvelope,
            calmConfirmCounter > 1 ? 2400 * BPS : 1200 * BPS
        );
        uint256 calmDivergenceBonus = 0;
        if (calmConfirmCounter > 0 && hazardMemory < 1100 * BPS && flowPressure < 650 * BPS && gap >= 2) {
            uint256 calmDivergenceSignal =
                divergenceMemory > 8 * BPS ? divergenceMemory - 8 * BPS : 0;
            calmDivergenceBonus = wmul(
                calmDivergenceSignal,
                calmConfirmCounter > 1
                    ? (gap >= 4 ? 650 * BPS : 350 * BPS)
                    : (gap >= 4 ? 250 * BPS : 150 * BPS)
            );
        }
        if (currentSpot >= latentSpot) {
            askOpportunityCut += passiveRecaptureCut + calmEnvelopeCut + calmDivergenceBonus;
        } else {
            bidOpportunityCut += passiveRecaptureCut + calmEnvelopeCut + calmDivergenceBonus;
        }
        ...
        slots[11] = calmConfirmCounter;
        slots[12] = calmReleaseEnvelope;
