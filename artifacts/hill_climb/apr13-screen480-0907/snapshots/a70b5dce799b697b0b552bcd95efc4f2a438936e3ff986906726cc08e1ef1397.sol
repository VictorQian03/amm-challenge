...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 passiveReleaseSignal =
            passiveRecaptureMemory > 16 * BPS ? passiveRecaptureMemory - 16 * BPS : 0;
        bool ultraCalmRelease =
            calmConfirmCounter == 0 &&
            gap >= 3 &&
            quietGate > 7800 * BPS &&
            hazardMemory < 950 * BPS &&
            flowPressure < 500 * BPS &&
            divergenceMemory < 7 * BPS;
        uint256 passiveReleaseRate = 0;
        if (calmConfirmCounter > 1) {
            passiveReleaseRate = 1750 * BPS;
        } else if (calmConfirmCounter > 0) {
            passiveReleaseRate = 1050 * BPS;
        } else if (ultraCalmRelease) {
            passiveReleaseRate = gap >= 4 ? 575 * BPS : 350 * BPS;
        }
        uint256 passiveRecaptureCut = wmul(passiveReleaseSignal, passiveReleaseRate);
