...
contract Strategy is AMMStrategyBase {
    ...
    uint256 internal constant DECAY_FEE_FLOOR = 9050 * BPS;
    ...
    uint256 internal constant ALPHA_FEE_FLOOR = 28 * BPS;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // filtered protective fee-floor accumulator
        slots[12] = spot; // fee-floor anchor spot
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 floorAccumulator = wmul(
            slots[11],
            _gapAdjustedDecay(DECAY_FEE_FLOOR, gapLong, 1800 * BPS)
        );
        uint256 floorAnchor = slots[12];
        if (gap >= 6) {
            floorAnchor = currentSpot;
        }
        ...
        uint256 sharedSpread =
            ...
            wmul(oneSidedFlow, 1550 * BPS);

        uint256 eventSignal = volObservation + hazardObservation;
        if (eventSignal > WAD) {
            eventSignal = WAD;
        }
        uint256 floorObservation = floorAnchor == 0
            ? _max(eventSignal, divergenceMemory)
            : _max(
                eventSignal,
                wdiv(absDiff(currentSpot, floorAnchor), floorAnchor)
            );
        if (floorObservation > 5 * BPS) {
            floorObservation += wmul(floorObservation - 5 * BPS, floorObservation);
        }
        floorObservation = clamp(floorObservation, 0, WAD);
        floorAccumulator = _blend(floorAccumulator, floorObservation, ALPHA_FEE_FLOOR);

        uint256 floorAnchorBlend = gap >= 4 ? 1600 * BPS : gap >= 2 ? 800 * BPS : 250 * BPS;
        uint256 floorAnchorGate = _oneMinus(
            clamp(
                wmul(floorAccumulator, 5000 * BPS) +
                    wmul(flowPressure, 2000 * BPS) +
        ...
        if (gap >= 2) {
            floorAnchor = _blend(floorAnchor, currentSpot, wmul(floorAnchorBlend, floorAnchorGate));
        }
        ...
        if (eventSignal > 8 * BPS) {
            ...
            eventCarry += wmul(burstSignal, 320 * BPS);
            directionalBurstFee = wmul(burstSignal, 700 * BPS);
        }
        sharedSpread += eventCarry;

        uint256 sharedRebateGate = _oneMinus(
            clamp(
                wmul(floorAccumulator, 5000 * BPS) +
        ...
        uint256 sharedRebate = wmul(wmul(calmMemory, 260 * BPS), sharedRebateGate);
        sharedSpread = sharedSpread > sharedRebate ? sharedSpread - sharedRebate : MIN_FEE;
        uint256 protectiveFloor =
            BASE_FEE +
            wmul(floorAccumulator, 1400 * BPS) +
            wmul(hazardMemory, 500 * BPS);
        if (sharedSpread < protectiveFloor) {
            sharedSpread = protectiveFloor;
        }
        ...
        slots[11] = floorAccumulator;
        slots[12] = floorAnchor;
