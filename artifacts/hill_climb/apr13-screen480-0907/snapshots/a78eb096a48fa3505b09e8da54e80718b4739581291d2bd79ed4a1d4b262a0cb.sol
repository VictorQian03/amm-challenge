...
contract Strategy is AMMStrategyBase {
    ...
    uint256 internal constant DECAY_MARKOUT = 9050 * BPS;
    ...
    uint256 internal constant ALPHA_MARKOUT = 32 * BPS;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // bid-side markout memory
        slots[12] = 0; // ask-side markout memory
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 bidMarkoutMemory = wmul(
            slots[11],
            _gapAdjustedDecay(DECAY_MARKOUT, gapShort, 1700 * BPS)
        );
        uint256 askMarkoutMemory = wmul(
            slots[12],
            _gapAdjustedDecay(DECAY_MARKOUT, gapShort, 1700 * BPS)
        );
        ...
        uint256 upwardExtension = 0;
        uint256 downwardExtension = 0;
        if (currentSpot >= lastSpot) {
            upwardExtension = lastSpot == 0 ? 0 : wdiv(currentSpot - lastSpot, lastSpot);
        } else {
            downwardExtension = lastSpot == 0 ? 0 : wdiv(lastSpot - currentSpot, lastSpot);
        }
        uint256 toxicCalmGate = _oneMinus(clamp(wmul(calmMemory, 7500 * BPS), 0, WAD));
        uint256 bidMarkoutObservation = 0;
        if (upwardExtension > 2 * BPS && sellShare >= 5200 * BPS) {
            bidMarkoutObservation =
                upwardExtension +
                wmul(richSignal, 7000 * BPS) +
                wmul(flowPressure, 3500 * BPS);
            if (bidMarkoutObservation > WAD) {
                bidMarkoutObservation = WAD;
            }
            bidMarkoutObservation = wmul(bidMarkoutObservation, toxicCalmGate);
        }
        uint256 askMarkoutObservation = 0;
        if (downwardExtension > 2 * BPS && buyShare >= 5200 * BPS) {
            askMarkoutObservation =
                downwardExtension +
                wmul(cheapSignal, 7000 * BPS) +
                wmul(flowPressure, 3500 * BPS);
            if (askMarkoutObservation > WAD) {
                askMarkoutObservation = WAD;
            }
            askMarkoutObservation = wmul(askMarkoutObservation, toxicCalmGate);
        }
        bidMarkoutMemory = _blend(bidMarkoutMemory, bidMarkoutObservation, ALPHA_MARKOUT);
        askMarkoutMemory = _blend(askMarkoutMemory, askMarkoutObservation, ALPHA_MARKOUT);
        ...
        if (flowPressure > 500 * BPS) {
            ...
            if (extensionSignal > 3 * BPS) {
                ...
                flowDirectionalRisk = wmul(toxicFlowSignal, 260 * BPS);
            }
        }
        ...
        uint256 bidMarkoutBlock = wmul(bidMarkoutMemory, 1200 * BPS);
        uint256 askMarkoutBlock = wmul(askMarkoutMemory, 1200 * BPS);
        ...
        bidOpportunityCut = bidOpportunityCut > bidMarkoutBlock ? bidOpportunityCut - bidMarkoutBlock : 0;
        askOpportunityCut = askOpportunityCut > askMarkoutBlock ? askOpportunityCut - askMarkoutBlock : 0;
        ...
        slots[11] = bidMarkoutMemory;
        slots[12] = askMarkoutMemory;
