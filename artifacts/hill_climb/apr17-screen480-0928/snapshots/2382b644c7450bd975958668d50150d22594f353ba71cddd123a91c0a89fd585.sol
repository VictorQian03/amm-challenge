...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 corridorCalmConfirmation = passiveRecaptureMemory;
        if (gap >= 4) {
            corridorCalmConfirmation = wmul(
                corridorCalmConfirmation + wmul(calmMemory, 2500 * BPS),
                8800 * BPS
            );
        } else if (gap >= 2) {
            corridorCalmConfirmation = wmul(
                corridorCalmConfirmation + wmul(calmMemory, 1800 * BPS),
                5200 * BPS
            );
        } else {
            corridorCalmConfirmation = 0;
        }
        if (corridorCalmConfirmation > WAD) {
            corridorCalmConfirmation = WAD;
        }
        uint256 corridorToxicity = clamp(
            wmul(hazardMemory, 9500 * BPS) +
                wmul(flowPressure, 8500 * BPS) +
                wmul(volMemory, 5000 * BPS) +
                wmul(divergenceMemory, 7000 * BPS),
            0,
            WAD
        );
        uint256 corridorBudget = wmul(
            corridorCalmConfirmation,
            _oneMinus(corridorToxicity)
        );
        corridorBudget = wmul(
            corridorBudget,
            _oneMinus(clamp(wmul(flowImbalance, 5000 * BPS), 0, WAD))
        );
        uint256 corridorCut =
            wmul(corridorBudget, 1800 * BPS) +
            wmul(wmul(corridorBudget, calmMemory), 1100 * BPS);
        if (currentSpot >= latentSpot) {
            askFee = askFee > corridorCut ? askFee - corridorCut : MIN_FEE;
        } else {
            bidFee = bidFee > corridorCut ? bidFee - corridorCut : MIN_FEE;
        }
