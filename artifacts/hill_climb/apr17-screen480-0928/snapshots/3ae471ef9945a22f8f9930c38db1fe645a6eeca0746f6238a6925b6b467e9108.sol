...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 corridorReconvergence = _oneMinus(
            clamp(wmul(divergenceMemory, 11500 * BPS), 0, WAD)
        );
        uint256 corridorGate = wmul(quietGate, corridorReconvergence);
        if (gap >= 4) {
            corridorGate = wmul(corridorGate, gapLong);
        } else if (gap >= 2) {
            corridorGate = wmul(corridorGate, 4500 * BPS);
        } else {
            corridorGate = 0;
        }
        if (directionalBurstFee > 0 || flowDirectionalRisk > 0) {
            corridorGate = 0;
        }
        uint256 corridorBaseCut =
            wmul(passiveRecaptureMemory, 1250 * BPS) +
            wmul(calmMemory, 225 * BPS);
        uint256 corridorFlex = sharedSpread > BASE_FEE ? sharedSpread - BASE_FEE : 0;
        uint256 corridorCap = wmul(corridorFlex, 3500 * BPS);
        uint256 corridorCut = wmul(corridorBaseCut, corridorGate);
        if (corridorCut > corridorCap) {
            corridorCut = corridorCap;
        }
        if (currentSpot >= latentSpot) {
            askOpportunityCut += passiveRecaptureCut + calmDivergenceBonus + corridorCut;
        } else {
            bidOpportunityCut += passiveRecaptureCut + calmDivergenceBonus + corridorCut;
        }
