...
contract Strategy is AMMStrategyBase {
    ...
    uint256 internal constant DECAY_FLOOR = 9550 * BPS;
    ...
    uint256 internal constant ALPHA_FLOOR = 10 * BPS;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // persistent filtered floor memory
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 floorAccumulator = wmul(
            slots[11],
            _gapAdjustedDecay(DECAY_FLOOR, gapLong, 500 * BPS)
        );
        ...
        uint256 persistentVolObservation = _max(
            clusterObservation,
            wmul(_max(volObservation, divergenceVol), 7000 * BPS)
        );
        uint256 floorObservation = persistentVolObservation + wmul(hazardObservation, 3500 * BPS);
        if (floorObservation > WAD) {
            floorObservation = WAD;
        }
        ...
        floorAccumulator = _blend(floorAccumulator, floorObservation, ALPHA_FLOOR);
        ...
        if (gap >= 4) {
            ...
            quietRelease = wmul(wmul(quietGate, reconvergenceGate), benignGapGate);
        ...
        if (quietRelease > 0) {
            floorAccumulator = wmul(
                floorAccumulator,
                _oneMinus(clamp(wmul(quietRelease, 900 * BPS), 0, WAD))
            );
        }
        ...
        uint256 askRiskSignal =
            ...
            askFlowRisk;
        uint256 persistentFloorSignal = _max(floorAccumulator, wmul(volMemory, 6500 * BPS));
        uint256 fastSideRelay = _max(hazardObservation, sideHazard);
        fastSideRelay = _max(fastSideRelay, oneSidedFlow);
        if (fastSideRelay > WAD) {
            fastSideRelay = WAD;
        }
        ...
        uint256 sharedSpread =
            BASE_FEE +
            wmul(persistentFloorSignal, 2500 * BPS) +
            wmul(volMemory, 500 * BPS) +
            wmul(hazardMemory, 850 * BPS) +
            wmul(divergenceMemory, 450 * BPS);

        uint256 burstSignal = fastSideRelay + wmul(_max(spotJump, divergence), 2800 * BPS);
        if (burstSignal > WAD) {
            burstSignal = WAD;
        }
        uint256 directionalBurstFee = 0;
        if (burstSignal > 8 * BPS) {
            directionalBurstFee = wmul(burstSignal - 8 * BPS, 1150 * BPS);
        }
