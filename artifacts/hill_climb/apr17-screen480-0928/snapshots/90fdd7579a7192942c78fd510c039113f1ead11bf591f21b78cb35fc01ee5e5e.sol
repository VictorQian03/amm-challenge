...
contract Strategy is AMMStrategyBase {
    ...
    uint256 internal constant DECAY_FLOOR = 9800 * BPS;
    ...
    uint256 internal constant ALPHA_FLOOR = 18 * BPS;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 floorObservation = _max(clusterObservation, eventSignal);
        ...
        if (quietRelease > 0) {
            floorAccumulator = wmul(
                floorAccumulator,
                _oneMinus(clamp(wmul(quietRelease, 500 * BPS), 0, WAD))
            );
        }
        ...
        uint256 floorCarry = wmul(floorAccumulator, 450 * BPS);
        if (floorAccumulator > 8 * BPS) {
            floorCarry += wmul(floorAccumulator - 8 * BPS, 750 * BPS);
        }
