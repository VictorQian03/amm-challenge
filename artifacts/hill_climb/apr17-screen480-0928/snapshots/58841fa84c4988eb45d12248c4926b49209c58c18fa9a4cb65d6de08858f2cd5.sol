...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 floorSpread =
            BASE_FEE +
            wmul(volMemory, 1350 * BPS) +
            wmul(hazardMemory, 1700 * BPS) +
            wmul(divergenceMemory, 400 * BPS);
        uint256 stressShell =
            wmul(shellBudget, 1700 * BPS) +
            wmul(eventSignal, 160 * BPS);
        uint256 directionalBurstFee = 0;
        if (eventSignal > 8 * BPS) {
            uint256 burstSignal = eventSignal - 8 * BPS;
            stressShell += wmul(burstSignal, 250 * BPS);
        ...
        bidFee = sharedSpread;
        askFee = sharedSpread;
        uint256 healingRebate = wmul(directionalBurstFee, 4000 * BPS);
        if (alignedContinuation) {
            uint256 extensionSignal = clamp(
                _max(bidRiskSignal, askRiskSignal),
                0,
                WAD
            );
            uint256 extensionShell = wmul(extensionSignal, 3400 * BPS);
            if (toxicBidSide) {
