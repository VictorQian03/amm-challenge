...
contract Strategy is AMMStrategyBase {
    ...
    uint256 internal constant ALPHA_SHELL = 20 * BPS;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // drawdown-budget shell memory
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 shellUsageObservation = clamp(
            wmul(eventSignal, 8000 * BPS) +
                wmul(oneSidedFlow, 9000 * BPS) +
                wmul(toxicFlowSignal, 3000 * BPS),
            0,
            WAD
        );
        uint256 shellBudget = wmul(
            slots[11],
            _gapAdjustedDecay(DECAY_SHELL, gapLong, 500 * BPS)
        );
        shellBudget = _blend(shellBudget, shellUsageObservation, ALPHA_SHELL);
        if (shellUsageObservation < 8 * BPS) {
            uint256 protectedInterval = wmul(calmMemory, wmul(quietGate, gapLong));
            shellBudget = wmul(
                shellBudget,
                _oneMinus(clamp(wmul(protectedInterval, 250 * BPS), 0, WAD))
            );
        }
        ...
        uint256 floorSpread =
            ...
            wmul(hazardMemory, 1950 * BPS) +
            wmul(divergenceMemory, 450 * BPS);
        uint256 stressShell =
            wmul(shellBudget, 2000 * BPS) +
            wmul(eventSignal, 220 * BPS);
        uint256 directionalBurstFee = 0;
        if (eventSignal > 8 * BPS) {
            uint256 burstSignal = eventSignal - 8 * BPS;
            stressShell += wmul(burstSignal, 300 * BPS);
            directionalBurstFee = wmul(burstSignal, 900 * BPS);
        }
        uint256 sharedSpread = floorSpread + stressShell;

        uint256 sharedRebate = wmul(calmMemory, 300 * BPS);
        sharedSpread = sharedSpread > sharedRebate ? sharedSpread - sharedRebate : MIN_FEE;

        uint256 bidProtection = wmul(bidRiskSignal, 3000 * BPS);
        uint256 askProtection = wmul(askRiskSignal, 3000 * BPS);
        ...
        if (alignedContinuation) {
            uint256 extensionSignal = _max(bidRiskSignal, askRiskSignal);
            uint256 extensionShell =
                wmul(clamp(extensionSignal, 0, WAD), 2400 * BPS) +
                wmul(oneSidedFlow, 2200 * BPS);
            if (toxicBidSide) {
                bidFee += extensionShell + directionalBurstFee;
                askFee = askFee > healingRebate ? askFee - healingRebate : MIN_FEE;
            } else {
                askFee += extensionShell + directionalBurstFee;
