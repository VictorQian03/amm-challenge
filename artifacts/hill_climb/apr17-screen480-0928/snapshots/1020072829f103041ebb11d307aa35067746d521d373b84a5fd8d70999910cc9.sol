...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 floorCarry = wmul(floorAccumulator, 1100 * BPS);
        if (floorAccumulator > 7 * BPS) {
            floorCarry += wmul(floorAccumulator - 7 * BPS, 900 * BPS);
        }
        sharedSpread += floorCarry;
        ...
        if (eventSignal > 8 * BPS) {
            directionalBurstFee = wmul(eventSignal - 8 * BPS, 1050 * BPS);
        }
