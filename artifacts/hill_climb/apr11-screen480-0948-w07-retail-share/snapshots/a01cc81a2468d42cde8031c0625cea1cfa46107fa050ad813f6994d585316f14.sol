...
contract Strategy is AMMStrategyBase {
    ...
    uint256 internal constant ALPHA_RETAIL = 15 * BPS;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // retail-share memory
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 retailShareObservation = 0;
        if (gap >= 2 && tradeSize <= 8 * BPS) {
            uint256 retailQuiet = _oneMinus(
                clamp(
                    wmul(flowPressure, 2500 * BPS) +
                        wmul(hazardMemory, 1800 * BPS) +
                        wmul(divergence, 3200 * BPS) +
                        wmul(spotJump, 1200 * BPS),
                    0,
                    WAD
                )
            );
            uint256 retailSize = _oneMinus(clamp(wmul(tradeSize, 7000 * BPS), 0, WAD));
            retailShareObservation = wmul(retailQuiet, retailSize);
        }
        uint256 retailShareMemory = _blend(slots[11], retailShareObservation, ALPHA_RETAIL);
        if (gap >= 4) {
            retailShareMemory = wmul(retailShareMemory, 9400 * BPS);
        } else if (gap >= 2) {
            retailShareMemory = wmul(retailShareMemory, 9700 * BPS);
        }
        ...
        uint256 retailShareCut = wmul(retailShareMemory, 240 * BPS);
        ...
        if (currentSpot >= latentSpot) {
            ...
            askOpportunityCut += retailShareCut;
        } else {
            bidOpportunityCut += passiveRecaptureCut + calmDivergenceBonus;
            bidOpportunityCut += retailShareCut;
        }
