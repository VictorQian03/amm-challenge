...
contract Strategy is AMMStrategyBase {
    ...
    uint256 internal constant ALPHA_ARRIVAL = 20 * BPS;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // count-like arrival cadence memory
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 arrivalMemory = wmul(
            slots[11],
            _gapAdjustedDecay(DECAY_FLOW, gapShort, 1900 * BPS)
        );
        uint256 arrivalObservation = _oneMinus(_gapRatio(gap, 6));
        ...
        arrivalMemory = _blend(arrivalMemory, arrivalObservation, ALPHA_ARRIVAL);
        ...
        uint256 arrivalSmallTradeGate = _oneMinus(clamp(wmul(tradeSize, 2800 * BPS), 0, WAD));
        uint256 arrivalSmallTradeSignal = wmul(arrivalMemory, arrivalSmallTradeGate);
        ...
        uint256 arrivalRebate = wmul(wmul(arrivalSmallTradeSignal, quietGate), 120 * BPS);
        sharedSpread = sharedSpread > arrivalRebate ? sharedSpread - arrivalRebate : MIN_FEE;
