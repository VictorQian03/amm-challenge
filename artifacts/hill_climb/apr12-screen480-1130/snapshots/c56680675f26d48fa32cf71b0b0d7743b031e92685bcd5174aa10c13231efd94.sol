...
contract Strategy is AMMStrategyBase {
    ...
    uint256 internal constant ALPHA_BUCKET = 10 * BPS;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 weakSliceFill = 0;
        if (gap >= 4 && hazardMemory < 1100 * BPS) {
            weakSliceFill = wmul(gapLong, quietGate);
        }
        ...
        uint256 sharedRebate = weakSliceBucket > 25 * BPS ? 25 * BPS : weakSliceBucket;
        sharedSpread = sharedSpread > sharedRebate ? sharedSpread - sharedRebate : MIN_FEE;
        weakSliceBucket = weakSliceBucket > wmul(sharedRebate, 7000 * BPS)
            ? weakSliceBucket - wmul(sharedRebate, 7000 * BPS)
            : 0;
