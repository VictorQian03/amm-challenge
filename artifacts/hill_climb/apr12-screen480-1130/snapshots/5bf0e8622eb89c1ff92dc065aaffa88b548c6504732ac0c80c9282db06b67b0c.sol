...
contract Strategy is AMMStrategyBase {
    ...
    uint256 internal constant SLEW_UP_BASE = 70 * BPS;
    uint256 internal constant SLEW_UP_URGENCY = 450 * BPS;
    uint256 internal constant SLEW_DOWN_BASE = 14 * BPS;
    uint256 internal constant SLEW_DOWN_RELAX = 40 * BPS;
    uint256 internal constant SLEW_DOWN_CALM_TIGHTEN = 24 * BPS;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = BASE_FEE; // previous bid fee
        slots[12] = BASE_FEE; // previous ask fee
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 calmReleaseGate = wmul(
            opportunityGate,
            _oneMinus(clamp(eventSignal * 6, 0, WAD))
        );
        bidFee = _applySlew(
            slots[11],
            bidFee,
            _max(eventSignal, bidRiskSignal),
            calmReleaseGate
        );
        askFee = _applySlew(
            slots[12],
            askFee,
            _max(eventSignal, askRiskSignal),
            calmReleaseGate
        );
    ...
    function _applySlew(
        uint256 previousFee,
        uint256 desiredFee,
        uint256 urgencySignal,
        uint256 calmReleaseGate
    ) internal pure returns (uint256) {
        if (previousFee == 0) {
            return desiredFee;
        }

        urgencySignal = clamp(urgencySignal, 0, WAD);
        calmReleaseGate = clamp(calmReleaseGate, 0, WAD);

        if (desiredFee >= previousFee) {
            uint256 upCap = SLEW_UP_BASE + wmul(urgencySignal, SLEW_UP_URGENCY);
            uint256 maxRaisedFee = previousFee + upCap;
            return desiredFee < maxRaisedFee ? desiredFee : maxRaisedFee;
        }

        uint256 downCap = SLEW_DOWN_BASE + wmul(_oneMinus(urgencySignal), SLEW_DOWN_RELAX);
        uint256 calmTighten = wmul(calmReleaseGate, SLEW_DOWN_CALM_TIGHTEN);
        downCap = downCap > calmTighten ? downCap - calmTighten : SLEW_DOWN_BASE;

        if (previousFee <= downCap) {
            return desiredFee;
        }

        uint256 minRelaxedFee = previousFee - downCap;
        return desiredFee > minRelaxedFee ? desiredFee : minRelaxedFee;
    }
}
