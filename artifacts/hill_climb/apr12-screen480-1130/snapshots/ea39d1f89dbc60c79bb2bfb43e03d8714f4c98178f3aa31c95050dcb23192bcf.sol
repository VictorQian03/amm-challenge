...
contract Strategy is AMMStrategyBase {
    ...
    uint256 internal constant ROUTER_ALPHA_FAST = 2800 * BPS;
    uint256 internal constant ROUTER_ALPHA_SLOW = 1800 * BPS;
    uint256 internal constant ROUTER_ENTER = 6200 * BPS;
    uint256 internal constant ROUTER_EXIT = 3800 * BPS;
    uint256 internal constant PROTECT_MODE = 1;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 3500 * BPS; // router score
        slots[12] = 0; // mode state: 0 competitive, 1 protective
        slots[13] = 0; // last mode-change timestamp
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 protectSignal = clamp(
            wmul(hazardMemory, 5200 * BPS) +
                ...
                wmul(oneSidedFlow, 1900 * BPS) +
                wmul(divergenceMemory, 1500 * BPS) +
                wmul(_max(bidFlowRisk, askFlowRisk), 7000 * BPS),
            0,
            WAD
        );
        uint256 competeSignal = clamp(
            wmul(calmMemory, 4200 * BPS) +
                wmul(quietGate, 2000 * BPS) +
                wmul(gapLong, 1400 * BPS) +
                wmul(passiveRecaptureMemory, 1800 * BPS),
            0,
            WAD
        );
        uint256 routerTarget = 5000 * BPS;
        if (protectSignal >= competeSignal) {
            routerTarget += wmul(protectSignal - competeSignal, 5000 * BPS);
        } else {
            uint256 routerRelease = wmul(competeSignal - protectSignal, 5000 * BPS);
            routerTarget = routerTarget > routerRelease ? routerTarget - routerRelease : 0;
        }
        uint256 routerScore = _blend(
            slots[11],
            routerTarget,
            gap >= 2 ? ROUTER_ALPHA_FAST : ROUTER_ALPHA_SLOW
        );
        uint256 modeState = slots[12];
        uint256 lastModeChange = slots[13];
        bool canSwitch = lastModeChange == 0 || trade.timestamp >= lastModeChange + 2;
        if (modeState == PROTECT_MODE) {
            if (canSwitch && routerScore < ROUTER_EXIT) {
                modeState = 0;
                lastModeChange = trade.timestamp;
            }
        } else if (canSwitch && routerScore > ROUTER_ENTER) {
            modeState = PROTECT_MODE;
            lastModeChange = trade.timestamp;
        }
        uint256 modeBlend = modeState == PROTECT_MODE
            ? clamp(6000 * BPS + wmul(routerScore, 4000 * BPS), 6000 * BPS, WAD)
            : clamp(wmul(routerScore, 4000 * BPS), 0, 4000 * BPS);

        uint256 competitiveSpread =
            BASE_FEE +
            wmul(volMemory, 1200 * BPS) +
            wmul(hazardMemory, 1200 * BPS) +
            wmul(divergenceMemory, 250 * BPS) +
            wmul(oneSidedFlow, 900 * BPS);
        uint256 competitiveRebate =
            wmul(calmMemory, 360 * BPS) +
            wmul(passiveRecaptureMemory, 220 * BPS);
        competitiveSpread = competitiveSpread > competitiveRebate
            ? competitiveSpread - competitiveRebate
            : MIN_FEE;
        uint256 competitiveBidFee = competitiveSpread + wmul(bidRiskSignal, 3000 * BPS);
        uint256 competitiveAskFee = competitiveSpread + wmul(askRiskSignal, 3000 * BPS);
        uint256 competitiveBidCut = wmul(bidOpportunitySignal, 9200 * BPS);
        uint256 competitiveAskCut = wmul(askOpportunitySignal, 9200 * BPS);
        uint256 passiveCompetitiveCut = wmul(passiveRecaptureMemory, 1700 * BPS);
        uint256 quietDivergenceCut = 0;
        if (hazardMemory < 1100 * BPS && flowPressure < 650 * BPS && gap >= 2) {
            quietDivergenceCut = wmul(divergenceMemory, gap >= 4 ? 650 * BPS : 450 * BPS);
        }
        if (currentSpot >= latentSpot) {
            competitiveAskCut += passiveCompetitiveCut + quietDivergenceCut;
        } else {
            competitiveBidCut += passiveCompetitiveCut + quietDivergenceCut;
        }
        competitiveBidFee = competitiveBidFee > competitiveBidCut
            ? competitiveBidFee - competitiveBidCut
            : MIN_FEE;
        competitiveAskFee = competitiveAskFee > competitiveAskCut
            ? competitiveAskFee - competitiveAskCut
            : MIN_FEE;

        uint256 protectiveSpread =
            BASE_FEE +
            wmul(volMemory, 2200 * BPS) +
            wmul(hazardMemory, 3400 * BPS) +
            wmul(divergenceMemory, 900 * BPS) +
            wmul(oneSidedFlow, 2100 * BPS) +
            eventCarry +
            wmul(eventSignal, 250 * BPS);
        uint256 protectiveBidFee = protectiveSpread + wmul(bidRiskSignal, 5600 * BPS);
        uint256 protectiveAskFee = protectiveSpread + wmul(askRiskSignal, 5600 * BPS);
        uint256 protectiveDirectionalTilt =
            directionalBurstFee +
            wmul(_max(richSignal, cheapSignal), 900 * BPS);
        uint256 protectiveHealing = wmul(protectiveDirectionalTilt, 2200 * BPS);
        if (currentSpot >= latentSpot) {
            protectiveBidFee += protectiveDirectionalTilt;
            protectiveAskFee = protectiveAskFee > protectiveHealing
                ? protectiveAskFee - protectiveHealing
                : MIN_FEE;
        } else {
            protectiveAskFee += protectiveDirectionalTilt;
            protectiveBidFee = protectiveBidFee > protectiveHealing
                ? protectiveBidFee - protectiveHealing
                : MIN_FEE;
        }
