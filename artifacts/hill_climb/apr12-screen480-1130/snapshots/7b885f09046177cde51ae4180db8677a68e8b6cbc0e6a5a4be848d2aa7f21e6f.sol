...
contract Strategy is AMMStrategyBase {
    ...
    uint256 internal constant ALPHA_BUCKET = 8 * BPS;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // weak-slice rebate bucket
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 weakSliceBucket = wmul(
            slots[11],
            _gapAdjustedDecay(DECAY_CALM, gapLong, 1200 * BPS)
        );
        if (
            gap >= 5 &&
            hazardMemory < 900 * BPS &&
            flowPressure < 450 * BPS &&
            divergenceMemory < 9 * BPS
        ) {
            uint256 weakSliceFill = wmul(gapLong, wmul(calmMemory, quietGate));
            weakSliceBucket = _blend(weakSliceBucket, weakSliceFill, ALPHA_BUCKET);
        }
        if (weakSliceBucket > 18 * BPS) {
            weakSliceBucket = 18 * BPS;
        }
        ...
        uint256 weakSlicePayout = 0;
        if (gap >= 4 && hazardMemory < 1000 * BPS && flowPressure < 500 * BPS) {
            weakSlicePayout = weakSliceBucket > 9 * BPS ? 9 * BPS : weakSliceBucket;
        }
        ...
        if (currentSpot >= latentSpot) {
            ...
            if (askOpportunitySignal > 3 * BPS) {
                askOpportunityCut += weakSlicePayout;
            }
        } else {
            bidOpportunityCut += passiveRecaptureCut + calmDivergenceBonus;
            if (bidOpportunitySignal > 3 * BPS) {
                bidOpportunityCut += weakSlicePayout;
            }
        }
        if (weakSlicePayout > 0) {
            weakSliceBucket = weakSliceBucket > wmul(weakSlicePayout, 9000 * BPS)
                ? weakSliceBucket - wmul(weakSlicePayout, 9000 * BPS)
                : 0;
        }
