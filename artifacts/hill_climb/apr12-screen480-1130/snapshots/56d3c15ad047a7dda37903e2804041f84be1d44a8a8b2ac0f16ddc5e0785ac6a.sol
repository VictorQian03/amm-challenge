...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // passive release streak
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 passiveReleaseStreak = slots[11];
        ...
        uint256 postRecenterDivergence = divergence;
        if (gap >= 4) {
            postRecenterDivergence =
        ...
        bool continuationUp =
            currentSpot >= latentSpot && buyFlow > sellFlow && spotJump > postRecenterDivergence;
        bool continuationDown =
            currentSpot < latentSpot && sellFlow > buyFlow && spotJump > postRecenterDivergence;
        bool releaseBreak =
            continuationUp ||
            continuationDown ||
            hazardMemory > 1200 * BPS ||
            flowPressure > 700 * BPS ||
            postRecenterDivergence > 10 * BPS;
        bool releaseConfirm =
            gap >= 4 &&
            quietGate > 6800 * BPS &&
            postRecenterDivergence < 5 * BPS &&
            flowPressure < 550 * BPS &&
            hazardMemory < 1100 * BPS;
        bool releaseStrongConfirm =
            releaseConfirm &&
            calmMemory > 1700 * BPS &&
            postRecenterDivergence < 3 * BPS &&
        ...
        if (gap >= 3 && (continuationUp || continuationDown)) {
            passiveRecaptureObservation = wmul(passiveRecaptureObservation, 8200 * BPS);
        }
        if (gap >= 4) {
        ...
        if (releaseBreak) {
            passiveReleaseStreak = 0;
        } else if (releaseStrongConfirm) {
            passiveReleaseStreak = passiveReleaseStreak < 2 ? passiveReleaseStreak + 1 : 2;
        } else if (releaseConfirm) {
            passiveReleaseStreak = passiveReleaseStreak == 0 ? 1 : 2;
        } else if (passiveReleaseStreak > 0) {
            passiveReleaseStreak -= 1;
        }
        ...
        uint256 passiveRecaptureCut = wmul(passiveRecaptureMemory, 450 * BPS);
        if (passiveReleaseStreak > 0) {
            passiveRecaptureCut += wmul(
                passiveRecaptureMemory,
                passiveReleaseStreak > 1 ? 1100 * BPS : 400 * BPS
            );
        }
        uint256 calmDivergenceBonus = 0;
        if (passiveReleaseStreak > 0 && hazardMemory < 1100 * BPS && flowPressure < 650 * BPS && gap >= 2) {
            calmDivergenceBonus = wmul(
                divergenceMemory,
                passiveReleaseStreak > 1
                    ? (gap >= 4 ? 650 * BPS : 400 * BPS)
