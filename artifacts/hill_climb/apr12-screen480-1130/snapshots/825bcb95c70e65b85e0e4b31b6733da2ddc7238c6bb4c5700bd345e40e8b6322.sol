...
contract Strategy is AMMStrategyBase {
    ...
    uint256 internal constant DECAY_TOXIC = 9000 * BPS;
    ...
    uint256 internal constant ALPHA_TOXIC = 18 * BPS;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        slots[11] = 0; // toxic-side budget memory
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 toxicBudgetMemory = wmul(
            slots[11],
            _gapAdjustedDecay(DECAY_TOXIC, gapShort, 1400 * BPS)
        );
        ...
        uint256 responseAgreement = 0;
        if (currentSpot >= lastSpot) {
            responseAgreement = buyFlow >= sellFlow ? WAD : 0;
        } else {
            responseAgreement = sellFlow > buyFlow ? WAD : 0;
        }
        uint256 toxicBudgetObservation = wmul(flowPressure, responseAgreement);
        toxicBudgetObservation = wmul(
            toxicBudgetObservation,
            _oneMinus(clamp(hazardMemory * 6, 0, WAD))
        );
        toxicBudgetMemory = _blend(toxicBudgetMemory, toxicBudgetObservation, ALPHA_TOXIC);
        ...
        uint256 toxicSideBudget = wmul(toxicBudgetMemory, 2500 * BPS);
        if (toxicSideBudget > 3500 * BPS) {
            toxicSideBudget = 3500 * BPS;
        }
        ...
        if (flowDirectionalRisk > 0) {
            if (currentSpot >= latentSpot) {
                if (buyFlow >= sellFlow) {
                    askFlowRisk = flowDirectionalRisk + toxicSideBudget;
                }
            } else if (sellFlow > buyFlow) {
                bidFlowRisk = flowDirectionalRisk + toxicSideBudget;
            }
        }
