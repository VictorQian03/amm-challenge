...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 quietCarryCompression = 0;
        if (gap >= 3) {
            quietCarryCompression = wmul(
                quietGate,
                wmul(
                    wmul(volObservation, 650 * BPS) + wmul(hazardObservation, 1100 * BPS),
                    50 * BPS
                )
            );
        }
        ...
        sharedSpread = sharedSpread > quietCarryCompression ? sharedSpread - quietCarryCompression : MIN_FEE;
