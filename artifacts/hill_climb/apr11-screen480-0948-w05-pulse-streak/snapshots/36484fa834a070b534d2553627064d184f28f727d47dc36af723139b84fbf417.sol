...
/// @title Quiet Step Pulse V2 Cluster
/// @notice Lower calm carry with tighter protection on same-step and short-gap burst clusters.
contract Strategy is AMMStrategyBase {
    uint256 internal constant BASE_FEE = 34 * BPS;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 common = clampFee(
            BASE_FEE + wmul(carryShock, 680 * BPS) + wmul(sizeMemory, 220 * BPS)
        );
    ...
    function getName() external pure override returns (string memory) {
        return "QuietStepPulseV2Cluster";
    }
}
