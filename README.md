...
For long-running probe-heavy rounds, use the optional subagent pattern in [docs/hill_climb.md](docs/hill_climb.md) when the user explicitly asks for parallel help; the critic runs after the proposer has produced a concrete batch and before source edits. This is operator guidance, not harness state; the harness still only measures, records, and protects retained evals.
