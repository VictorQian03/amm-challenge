...
Probe-heavy rounds still need a memo surface: `status`, `history`, and `show-eval` summarize retained evals only, so keep live scratch-batch synthesis in `docs/plans/active/`, move retired run notes to `docs/plans/completed/`, and store probe artifacts outside the retained lane.
Use `docs/plans/active/<run_id>.md` as the stable live run index and split long run notes into 5-round chunk files named `docs/plans/active/<run_id>-round01-05.md`, `...-round06-10.md`, `...-round11-15.md`, and so on.
...
Use the profile/failure-tag read surfaces to retire exhausted basins such as `over_open_leak`, `over_tighten_clamp`, `frontier_neighbor`, and `crossover_regression`; add precise new basin vocabulary when repeated evidence no longer fits the old labels. Treat `max_fee_jump` as a diagnostic, not an automatic anti-pattern.
When importing outside evidence for saturated runs, bias web searches toward out-of-distribution vocabulary instead of more incumbent-local AMM, LOB, hazard, refill, recapture, inventory, opportunity, or fee-band terms; layer 5/6 is admissible only as a bounded diagnostic with a new evidence owner and tight floor-loss kill thresholds.
For higher proposal hit rates, require a proposal scorecard before source work: primary owner, allowed consumers, forbidden consumers, expected metric movement, nearest negative example, and kill signature. Outside vocabulary alone is not novelty if it still routes through an old hazard, protection, release, or side-risk consumer path.
For long-running probe-heavy rounds, use the optional subagent pattern in [docs/hill_climb.md](docs/hill_climb.md) when the user explicitly asks for parallel help; the proposer/critic loop must leave at least four critic-accepted, genuinely distinct strategy design improvements before workers start source edits. This is operator guidance, not harness state; the harness still only measures, records, and protects retained evals.
