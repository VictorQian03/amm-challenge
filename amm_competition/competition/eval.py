...
from amm_competition.competition.config import resolve_stage_preset
...
def compute_scorecard(
    match_result: MatchResult, *, stage: str | None = None
) -> dict[str, Any]:
    ...
    return {
        ...
        "gate": _build_gate_summary(overall, stage=stage),
    }
...
def _summarize_records(records: list[ScorecardRecord]) -> dict[str, float | int | None]:
    ...
    retail_volume_share = (
        None if total_retail <= 0 else submission_retail / total_retail
    )
    arb_to_retail_volume_ratio = (
    ...
    retail_flow_share = (
        None
        if total_submission_flow <= 0
        else submission_retail / total_submission_flow
    )
    arb_flow_share = (
        None if total_submission_flow <= 0 else submission_arb / total_submission_flow
    )
...
def _build_gate_summary(
    ...
    *,
    ...
    required_fields_present = all(
        overall.get(field) is not None for field in REQUIRED_GATE_METRIC_FIELDS
    )
    failures: list[str] = []
    ...
    if not required_fields_present:
        failures.append("required scorecard fields are absent")
    else:
        for metric_name, minimum in thresholds.items():
            ...
            if float(value) < minimum:
                failures.append(
                    f"{metric_name}={float(value):.6f} is below stage threshold {minimum:.6f}"
                )

    return {
        "stage": stage,
        "required_fields_present": required_fields_present,
        "required_metric_fields": list(REQUIRED_GATE_METRIC_FIELDS),
        "optional_metric_fields": list(OPTIONAL_DIAGNOSTIC_METRIC_FIELDS),
        "thresholds": thresholds,
        "passed": required_fields_present and not failures,
        "failures": failures,
    }
