...
class ProtectedSurfaceChecker:
    ...
    def __init__(
        ...
        *,
        ...
        self._manifest_relpath = self.manifest_path.relative_to(
            self.repo_root
        ).as_posix()
    ...
    def ensure_runtime_eval_allowed(self) -> None:
        ...
        raise ProtectedSurfaceError(
            self._dirty_tree_message("hill-climb eval", changes)
        )
    ...
    def verify_recorded_fingerprint(
        ...
        *,
        ...
        if not isinstance(recorded_payload, dict):
            raise ProtectedSurfaceError(
                f"Run '{run_id}' does not record a valid protected_surface_fingerprint in run.json. "
                "This retained run predates evaluator freeze fingerprinting; start a fresh run_id "
        ...
        details.append(
            "Start a fresh run_id and baseline for this evaluator version before continuing."
        )
        raise ProtectedSurfaceError("\n".join(details))
    ...
    def _dirty_tree_message(self, mode: str, changes: ProtectedSurfaceChanges) -> str:
        ...
        lines.extend(
            [
                ...
                "Do not append to an existing retained run after changing the evaluator surface; "
                "start a fresh run_id and baseline instead.",
            ]
        )
        return "\n".join(lines)
